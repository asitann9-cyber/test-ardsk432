...
🔥 YENİ: Z-Score UI entegrasyonu tamamlandı
⚡ TESTNET SENKRONIZASYONU: Pozisyon fiyatları testnet'ten alınıyor
"""
...
from ui.components import create_layout, create_ai_score_bar, create_zscore_indicator
from trading.analyzer import batch_analyze_with_ai
from data.fetch_data import get_current_price  # Sadece mainnet tablosu için kullanılacak
...
def auto_scan_worker():
    """Otomatik tarama işleyicisi - config.current_data'yı günceller"""
...
# CSS stilleri (değişmedi, kısalık için atlandı)
app.index_string = '''
<!DOCTYPE html>
<html>
    <head>
...
def update_performance_metrics(n_intervals):
    """⚡ TESTNET SENKRONIZASYONU: Gerçekleşmemiş PnL için pozisyon fiyatlarını kullan"""
    try:
        ...
        for symbol, position in list(open_positions.items()):
            # ⚡ DEĞİŞTİRİLDİ: Pozisyondan current_price'ı al (testnet'ten geliyor)
...
def update_trading_tables(n_intervals):
    """⚡ TESTNET SENKRONIZASYONU DÜZELTMESİ: Pozisyondan current_price al"""
    positions_data = []
    ...
    try:
        for symbol, position in list(current_positions.items()):
            # ⚡ DEĞİŞTİRİLDİ: Pozisyondan current_price'ı al
            # get_current_live_positions() zaten mark_price'ı current_price olarak kaydediyor
            current_price = position.get('current_price', position['entry_price'])
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct, max_zscore):  
    ...
    max_zscore = float(max_zscore or 5.0)
    ...
    if max_zscore < 5.0:
        if 'max_zscore' in df.columns:
            df = df[df['max_zscore'] <= max_zscore]
...
if __name__ == "__main__":
    setup_csv_files()
    logger.info("🚀 AI Crypto Analytics + Live Trading + Z-Score Sistemi başlatılıyor...")
    logger.info("⚡ HİBRİT SİSTEM: Tablo mainnet | Live pozisyonlar testnet")
