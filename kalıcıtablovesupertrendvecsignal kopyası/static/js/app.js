 ...
 * 🆕 YENİ: Threshold Yönetimi - TAM ENTEGRASYON
 */
...
class AnalysisApp {
  ...
  saveSettings() {
    /**
     * Ayarları LocalStorage'a kaydet ve backend'e bildir
     */
    try {
      const inputElement = document.getElementById('min-ratio-threshold');
      ...
      // 🆕 Sunucuya bildir (hemen)
      this.updateServerThreshold(newThreshold);
      
      // 🆕 ÖNEMLI: Eğer analiz aktifse, yeniden çalıştır
      if (this.isAutoUpdateActive && this.selectedSymbols.length > 0) {
        ...
        console.log('💡 Threshold değişti. Yeni analiz başlatmak için "Analiz Başlat" butonuna tıklayın.');
        this.showStatus('settings-status', 
          `✅ Ayarlar kaydedildi! Yeni threshold: ${newThreshold}% - Yeni analize başlamak için "Analiz Başlat" butonuna tıklayın.`, 
          'info');
      }
  ...
  // =====================================================
  // ANALYSIS FUNCTIONS - 🆕 THRESHOLD ENTEGRASYONU
  // =====================================================
  ...
  async startSupertrendAnalysis(isAutoUpdate = false) {
    ...
    try {
      ...
      // 🆕 Threshold değerini isteğe ekle
      const requestBody = {
        timeframe: timeframe,
        min_ratio_threshold: this.minRatioThreshold  // Frontend'deki threshold'u backend'e gönder
      };
      
      console.log(`📊 Analiz başlatılıyor - Threshold: ${this.minRatioThreshold}%`);
      ...
      if (data.success) {
        ...
        console.log(`🎯 Threshold'u aşan: ${data.high_priority_count || 0} emtia (${data.active_threshold}%)`);
        ...
        if (!isAutoUpdate) {
          ...
          this.showStatus('analysis-status', 
            `✅ Analiz başlatıldı - Threshold: ${data.active_threshold}% (${data.high_priority_count || 0} emtia kalıcı listeye eklendi) - Her 30 saniyede otomatik güncellenecek (${now})`, 
            'success');
        } else {
          const now = new Date().toLocaleTimeString('tr-TR');
          this.showStatus('analysis-status', 
            `🔄 Otomatik güncelleme aktif - Son: ${now} (${data.results.length} emtia, Threshold: ${data.active_threshold}%)`, 
            'info');
        }
      } else {
        this.showStatus('analysis-status', `❌ ${data.error}`, 'error');
