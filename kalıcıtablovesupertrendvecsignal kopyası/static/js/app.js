 ...
 * 🆕 YENİ: C-Signal ±20 L/S Badge Render
 */
...
class AnalysisApp {
  ...
  renderCSignalBadge(signalStatus) {
    /**
     * C-Signal ±20 badge HTML render
     * 
     * @param {string} signalStatus - 'L', 'S', veya null
  ...
  // =====================================================
  // PERMANENT LIST FUNCTIONS - 🆕 C-SIGNAL BADGE
  // =====================================================
  ...
  async loadPermanentHighRatio() {
    try {
      ...
      if (data.success) {
        ...
        // 🆕 Aktif C-Signal sayısını güncelle
        ...
        if (cSignalCountElement) {
          ...
          cSignalCountElement.textContent = `${activeCSignals} aktif C-Signal`;
  ...
  async startSupertrendAnalysis(isAutoUpdate = false) {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log(`🎯 Threshold'u aşan: ${data.high_priority_count || 0} emtia`);
        ...
        if (!isAutoUpdate) {
          ...
          this.showStatus('analysis-status', 
            `✅ Analiz başlatıldı - ${data.high_priority_count || 0} emtia kalıcı listeye eklendi${cSignalInfo} - Her 30 saniyede otomatik güncellenecek (${now})`, 
            'success');
        } else {
          const now = new Date().toLocaleTimeString('tr-TR');
          this.showStatus('analysis-status', 
            `🔄 Otomatik güncelleme aktif - Son: ${now} (${data.results.length} emtia)`, 
            'info');
        }
      } else {
        this.showStatus('analysis-status', `❌ ${data.error}`, 'error');
  ...
  async init() {
    console.log('🎯 Supertrend + C-Signal ±20 System initializing...');
    ...
    try {
      ...
      console.log('✅ Supertrend + C-Signal ±20 System initialized');
