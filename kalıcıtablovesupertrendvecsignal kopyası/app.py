            ...
            if results:
                ...
                if c_signal_alerts:
                    ...
                    if telegram_service.is_configured():
                        ...
                        # Alert sayısına göre tek veya toplu gönder
                        if len(c_signal_alerts) == 1:
                            ...
                            if symbol_data and telegram_service.should_send_c_signal_alert(symbol_data):
                                ...
                                if success:
                                    ...
                                    logger.info(f"📱 {alert['symbol']} C-Signal alert Telegram'a gönderildi")
                        
                        elif len(c_signal_alerts) > 1:
                            # Birden fazla alert - toplu mesaj
                            sent_count = telegram_service.send_batch_c_signal_alerts(c_signal_alerts)
                            
                            if sent_count > 0:
                                memory_storage.increment_c_signal_alerts()
                                logger.info(f"📱 {sent_count} C-Signal alert toplu olarak Telegram'a gönderildi")
                    else:
                ...
                return jsonify({
                    ...
                    "message": f"{len(formatted_results)} sembol analiz edildi - {high_priority_count} sembol kalıcı listeye eklendi - {len(c_signal_alerts)} C-Signal alert (Telegram'a gönderildi)"
                })
            else:
...
def main():
    ...
    try:
        ...
        print("   🔔 C-Signal TELEGRAM BİLDİRİMLERİ AKTİF!")
        ...
        print("   🔔 TELEGRAM'A OTOMATIK BİLDİRİM!")
        print("="*70)
