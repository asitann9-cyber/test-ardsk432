...
🆕 YENİ: Panel üzerinden ayarlanabilir minimum ratio threshold
"""
import pandas as pd
import logging
...
def register_routes(app, analysis_service, memory_storage, binance_service, telegram_service):
    ...
    def analyze_supertrend():
        ...
        try:
            ...
            # ✅ EKLEME: Mevcut threshold'u logla
            ...
            if results:
                ...
                # ✅ DÜZELTME: Ratio >= threshold olan emtiaları kalıcı listeye ekle
                high_priority_count = 0
                for result in results:
                    ratio = abs(result.get('ratio_percent', 0))
                    
                    # Debug log
                    logger.debug(f"Symbol: {result['symbol']}, Ratio: {ratio}%, Threshold: {current_threshold}%")
                    
                    if ratio >= current_threshold:  # ✅ Güncel threshold ile karşılaştır
                ...
                return jsonify({
                    ...
                    "timeframe": timeframe,
                    "high_priority_count": high_priority_count,  # ✅ EKLEME
                    "current_threshold": current_threshold,  # ✅ EKLEME
                    "message": f"{len(formatted_results)} sembol analiz edildi - {high_priority_count} sembol kalıcı listeye eklendi (Threshold: {current_threshold}%)"
                })
            else:
    ...
    # =====================================================
    # 🆕 YENİ ENDPOINT: MİNİMUM RATİO THRESHOLD AYARLAMA
    # =====================================================
    ...
    def update_threshold():
        ...
        Panel üzerinden ayarlanabilir kalıcı liste kriteri.
        """
        try:
            data = request.get_json()
            ...
            # ✅ DÜZELTME: Class attribute olarak güncelle
            AnalysisService.MIN_RATIO_THRESHOLD = float(min_ratio_threshold)
            
            # ✅ EKLEME: Mevcut permanent symbolleri yeniden değerlendir
            permanent_symbols = memory_storage.get_permanent_symbols()
            ...
            for symbol_data in permanent_symbols:
                current_ratio = abs(symbol_data.get('max_ratio_percent', 0))
...
def main():
    ...
    try:
        ...
        print("   ⚙️ Panel Üzerinden Ayarlanabilir Threshold (YENİ!)")
        print("="*70)
