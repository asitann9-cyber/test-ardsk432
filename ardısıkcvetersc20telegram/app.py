...
def register_routes(app, analysis_service, memory_storage, binance_service, telegram_service):
    ...
    # =====================================================
    # 🆕 YENİ: MANUEL TÜR DEĞİŞTİRME ENDPOINT'İ - KALICI KORUMA İLE
    # =====================================================
    @app.route('/api/consecutive/update-symbol-type', methods=['POST'])
    def update_symbol_type():
        """Kalıcı listedeki sembolün türünü manuel güncelle ve KALICI OLARAK KORUMA ALTINA AL"""
        try:
            data = request.get_json()
            symbol = data.get('symbol')
            ...
            if updated_permanent_symbol:
                ...
                if reverse_momentum.get('has_reverse_momentum', False):
                    if telegram_service.should_send_alert(updated_symbol, reverse_momentum):
                        success_telegram = telegram_service.send_reverse_momentum_alert(updated_symbol)
                        if success_telegram:
                            updated_symbol['last_telegram_alert'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                            memory_storage.update_permanent_symbol(symbol, {'last_telegram_alert': updated_symbol['last_telegram_alert']})
            ...
            return jsonify({
                "success": True,
                "message": f"🔒 {symbol} türü {old_type} → {new_type} değiştirildi ve KİLİTLENDİ! Artık otomatik güncellemelerde korunacak.",
                "symbol": symbol,
                "old_type": old_type,
                "new_type": new_type,
                "is_locked": True,
                "reverse_momentum_detected": reverse_momentum.get('has_reverse_momentum', False) if 'reverse_momentum' in locals() else False
            })
