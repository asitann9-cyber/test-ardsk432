...
class AnalysisApp {
  ...
  showSearchSuggestions(searchTerm) {
    ...
    matches.forEach(result => {
      ...
      // Tür simgesi
      const typeIcon = result.consecutive_type === 'Long' ? '🟢' : 
                      result.consecutive_type === 'Short' ? '🔴' : '⚪';
        ...
        <div class="search-suggestion-item" 
          ...
          <div>
            ...
            <div style="font-size: 11px; color: var(--text-muted);">
              ${typeIcon} ${result.consecutive_type} - ${result.consecutive_count} ardışık - ${result.percentage_change}%
            </div>
          </div>
  ...
  updatePermanentTable(permanentSymbols) {
    ...
    permanentSymbols.forEach(symbol => {
      ...
      // 🆕 MANUEL TÜR DEĞİŞTİRME DROPDOWN
      const typeDropdownHtml = `
        <select onchange="app.updateSymbolType('${symbol.symbol}', this.value)" 
          ...
          <option value="Long" ${symbol.max_consecutive_type === 'Long' ? 'selected' : ''}>🟢 Long</option>
          <option value="Short" ${symbol.max_consecutive_type === 'Short' ? 'selected' : ''}>🔴 Short</option>
        </select>
      `;
  ...
  async init() {
    console.log('🚨 Consecutive Candles + C-Signal + Reverse Momentum + Autocomplete System initializing...');
    ...
    try {
      ...
      console.log('✅ Consecutive Candles + C-Signal + Reverse Momentum + Autocomplete System initialized');
