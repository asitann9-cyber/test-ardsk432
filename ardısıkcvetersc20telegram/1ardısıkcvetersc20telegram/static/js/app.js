...
class AnalysisApp {
  ...
  updateConsecutiveTable(results) {
    ...
    results.forEach(result => {
      ...
      if (result.shouldHighlight) {
        row.style.backgroundColor = 'rgba(255, 235, 59, 0.2)'; // Sarı vurgu
        row.style.border = '1px solid #ffeb3b';
      }
      ...
      const addButtonHtml = `
        <button class="btn btn-warning" 
                ...
                onclick="app.addToPermanentList('${result.symbol}', '${document.getElementById('timeframe').value}')"
  ...
  bindEventListeners() {
    ...
    // YENİ: Arama kutusu event listener
