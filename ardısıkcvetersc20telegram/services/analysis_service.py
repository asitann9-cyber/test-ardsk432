...
class AnalysisService:
    ...
    def detect_reverse_momentum(symbol_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - Ardışık Long + C-Signal negatif = Ters momentum (C↓)
        - Ardışık Short + C-Signal pozitif = Ters momentum (C↑)
        ...
        try:
            ...
            # TERS MOMENTUM TESPİTİ
            if consecutive_type == 'Long' and c_signal_value < 0:
                # Ardışık Long ama C-Signal negatif = Düşüş sinyali
                strength = 'Strong' if abs(c_signal_value) >= 2.0 else 'Medium' if abs(c_signal_value) >= 1.0 else 'Weak'
                return {
            ...
            elif consecutive_type == 'Short' and c_signal_value > 0:
                # Ardışık Short ama C-Signal pozitif = Yükseliş sinyali
                strength = 'Strong' if c_signal_value >= 2.0 else 'Medium' if c_signal_value >= 1.0 else 'Weak'
                return {
                    'has_reverse_momentum': True,
    ...
    def is_high_priority_symbol(result: Dict[str, Any]) -> bool:
        ...
        return consecutive_count >= 5 or percentage_change >= 10.0