...
Modüler yapıda organize edilmiş Dash web uygulaması + Live Trading Bot + WebSocket
🔥 LIVE TRADER ENTEGRASYONu TAMAMLANDI + Config Senkronizasyonu + WebSocket Real-time
"""
...
from config import (
    initialize, DASH_CONFIG, TABLE_REFRESH_INTERVAL, RT_TABLE_REFRESH_INTERVAL,
...
# 🔗 WEBSOCKET IMPORT - YENİ
try:
    from websocket import (
        start_websocket_streams, stop_websocket_streams, is_websocket_active,
        get_websocket_data, update_websocket_symbols, get_top3_for_live_bot
    )
    WEBSOCKET_AVAILABLE = True
except ImportError:
    WEBSOCKET_AVAILABLE = False
    print("⚠️ WebSocket modülü bulunamadı")
...
def auto_scan_worker():
    """🔥 DÜZELTME: Otomatik tarama işleyicisi - config.current_data'yı günceller + WebSocket'e gönderir"""
    ...
    while auto_scan_active:
        try:
            ...
            # 🔗 YENİ: WebSocket için top 10 gönder
            if WEBSOCKET_AVAILABLE and not result_data.empty:
                top10_symbols = result_data.head(10)['symbol'].tolist()
                config.update_websocket_symbols(top10_symbols)
                logger.info(f"🔗 WebSocket'e top 10 gönderildi: {', '.join(top10_symbols[:3])}...")
...
# 🔗 YENİ: WebSocket Kontrol Callback
@app.callback(
    [Output('btn-websocket', 'children'),
     Output('btn-websocket', 'className')],
    Input('btn-websocket', 'n_clicks')
)
def toggle_websocket(n_clicks):
    """WebSocket butonunu kontrol et"""
    if not WEBSOCKET_AVAILABLE:
        return "❌ WebSocket Yok", "control-button websocket-button"
    
    if n_clicks > 0:
        if is_websocket_active():
            stop_websocket_streams()
            return "🔗 WebSocket Başlat", "control-button websocket-button"
        else:
            # Mevcut top 10 sembol ile başlat
            if config.current_data is not None and not config.current_data.empty:
                top10_symbols = config.current_data.head(10)['symbol'].tolist()
                success = start_websocket_streams(top10_symbols)
                if success:
                    return "🔗 WebSocket Durdur", "control-button websocket-button active"
...
def update_websocket_status(n_intervals):
    ...
    if not WEBSOCKET_AVAILABLE:
        return html.Div("❌ WebSocket Modülü Yok", style={'color': '#ef4444'})
    
    if is_websocket_active():
        # WebSocket aktif - sembolleri göster
        ws_symbols = config.websocket_top10_symbols
        return html.Div([
            html.Span("🔗 WebSocket Aktif", style={'color': '#06b6d4', 'fontWeight': 'bold'}),
            html.Br(),
            html.Small(f"İzlenen: {len(ws_symbols)} sembol", style={'color': '#9ca3af'}),
            html.Br(),
            html.Small(f"Top 3: {', '.join(ws_symbols[:3])}" if len(ws_symbols) >= 3 else f"Semboller: {', '.join(ws_symbols)}", 
                      style={'color': '#06b6d4'})
        ])
    else:
        return html.Div([
            html.Span("🔴 WebSocket Durduruldu", style={'color': '#ef4444', 'fontWeight': 'bold'}),
            html.Br(),
            html.Small("Real-time veri yok", style={'color': '#9ca3af'})
        ])
...
def update_trading_tables(n_intervals):
    ...
    # Açık pozisyonlar - 🔥 GÜVENLİ ITERATION
    try:
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):  
    """🔥 YENİ: Config entegre sinyal güncelleme (log_volume uyumlu)"""
    ...
    if WEBSOCKET_AVAILABLE and is_websocket_active():
        status += f" | 🔗 WebSocket aktif"
...
# 🔗 YENİ: WebSocket Real-time Tablo Callback
@app.callback(
    [Output('websocket-table', 'data'),
     Output('websocket-stats', 'children')],
    [Input('websocket-interval', 'n_intervals')]
)
def update_websocket_table(n_intervals):
    """WebSocket real-time tablosunu güncelle"""
    if not WEBSOCKET_AVAILABLE or not is_websocket_active():
        return [], html.Div("🔴 WebSocket pasif", style={'color': '#ef4444', 'textAlign': 'center'})
    
    try:
        # WebSocket real-time verisini al
        ws_data = config.get_websocket_realtime_data()
        
        if ws_data is None or (hasattr(ws_data, 'empty') and ws_data.empty):
            return [], html.Div("📡 WebSocket veri bekleniyor...", style={'color': '#f59e0b', 'textAlign': 'center'})
        
        # WebSocket tablosu için veri hazırla
        ws_table_data = []
        for _, row in ws_data.head(10).iterrows():  # Top 10
            ai_score_bar = create_ai_score_bar(row.get('ai_score', 0))
            
            ws_table_data.append({
                'symbol': row.get('symbol', ''),
                'side': row.get('run_type', '').upper(),
                'ai_score': ai_score_bar,
                'realtime_price': f"${row.get('realtime_price', 0):.6f}",
                'price_change': f"{row.get('price_change_24h', 0):+.2f}%",
                'momentum_1m': f"{row.get('momentum_1m', 0):+.2f}%",
                'combined_score': f"{row.get('combined_score', 0):.0f}",
                'status': row.get('rt_status', '🔴 Offline')
            })
        
        # WebSocket istatistikleri
        ws_count = len(ws_data)
        live_count = len([r for r in ws_table_data if '🟢' in r.get('status', '')])
        
        ws_stats = html.Div([
            html.Span(f"📡 {ws_count} WebSocket sinyali | ", style={'color': '#06b6d4'}),
            html.Span(f"🟢 {live_count} canlı | ", style={'color': '#22c55e'}),
            html.Span(f"⏰ {datetime.now(LOCAL_TZ).strftime('%H:%M:%S')}", style={'color': '#9ca3af'})
        ], style={'textAlign': 'center'})
        
        return ws_table_data, ws_stats
        
    except Exception as e:
        logger.error(f"❌ WebSocket tablo hatası: {e}")
        return [], html.Div(f"❌ WebSocket hata: {str(e)[:50]}...", style={'color': '#ef4444', 'textAlign': 'center'})


# Ana çalıştırma
if __name__ == "__main__":
    setup_csv_files()
    logger.info("🚀 AI Crypto Analytics + Live Trading + WebSocket başlatılıyor...")
    ...
    if WEBSOCKET_AVAILABLE:
        logger.info("✅ WebSocket modülü hazır")
        logger.info("🔗 Real-time fiyat akışı için WebSocket kullanılacak")
    else:
        logger.warning("⚠️ WebSocket modülü bulunamadı - real-time veri yok")
