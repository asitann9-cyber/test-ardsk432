...
🔥 YENİ DEVISO RATIO ENTEGRASYONu + WebSocket Top 10 Gönderimi
"""
...
from config import (
    ...
    current_data, saved_signals, update_websocket_symbols
)
...
def analyze_symbol_with_ai(symbol: str, interval: str) -> Dict:
    """
    🔥 GÜNCELLENDI: C-Signal momentum eklendi
    ...
    try:
        ...
        result = {
            ...
            'c_signal_momentum': metrics.get('c_signal_momentum', 0.0),  # 🔥 YENİ EKLENEN
            'ai_score': ai_score,  
        ...
        logger.debug(f"✅ {symbol}: Sinyal onaylandı - AI:{ai_score:.0f}%, Deviso:{deviso_ratio:.2f}, Trend:{trend_direction}")
        return result
...
def batch_analyze_with_ai(interval: str) -> pd.DataFrame:
    ...
    🔗 YENİ: WebSocket'e top 10 sembol gönderimi eklendi
    ...
    logger.info(f"🤖 {len(symbols)} sembol için YENİ DEVISO ile AI analiz başlatılıyor...")
    
    # Yeni analiz sonuçları
    fresh_results = []
    processed_count = 0
    deviso_success_count = 0  # 🔥 YENİ: Deviso başarı sayacı
            ...
            try:
                ...
                if processed_count % 100 == 0:
                    ...
                    logger.info(
                        ...
                        f"Deviso Başarı: {success_rate:.1f}% ({deviso_success_count})"
                    )
    ...
    logger.info("✅ YENİ DEVISO AI Analiz tamamlandı:")
    ...
    # 🔗 YENİ: WebSocket'e top 10 sembol gönder
    if len(df) > 0:
        top_10_symbols = df.head(10)['symbol'].tolist()
        try:
            update_websocket_symbols(top_10_symbols)
            logger.info(f"🔗 WebSocket'e top 10 gönderildi: {', '.join(top_10_symbols[:3])}...")
        except Exception as e:
            logger.debug(f"WebSocket güncelleme hatası: {e}")
        
        top_signal = df.iloc[0]
        logger.info(
            f"🏆 En yüksek AI skoru: {top_signal['ai_score']:.0f}% - {top_signal['symbol']} "
            f"(Deviso: {top_signal['deviso_ratio']:.2f})"
        )
        ...
        for i, row in df.head(5).iterrows():
            logger.debug(
                ...
                f"Trend={row.get('trend_strength',0)}"
            )
...
def filter_signals(df: pd.DataFrame, filters: Dict) -> pd.DataFrame:
    ...
    🔥 YENİ: Deviso ratio filtreleri eklendi
...
def get_top_signals(df: pd.DataFrame, count: int = 10) -> pd.DataFrame:
    ...
    🔥 YENİ: Deviso ratio da sıralama kriterinde
...
def analyze_signal_quality(df: pd.DataFrame) -> Dict:
    ...
    🔥 YENİ: Deviso ratio kalite metrikleri eklendi
    ...
    return {
        ...
        'deviso_quality': deviso_quality,  # 🔥 YENİ
        'trend_distribution': trend_distribution  # 🔥 YENİ
    }
...
def get_signal_summary() -> Dict:
    """
    Sinyal özetini al
    🔥 YENİ: Deviso istatistikleri eklendi
    ...
    return {
        ...
        'deviso_stats': deviso_stats  # 🔥 YENİ
    }


# 🔗 YENİ: WebSocket entegrasyon fonksiyonları
def get_top10_for_websocket(df: pd.DataFrame) -> List[str]:
    """
    WebSocket için top 10 sembol listesi al
    
    Args:
        df (pd.DataFrame): Sinyal verileri
        
    Returns:
        List[str]: Top 10 sembol listesi
    """
    if df.empty:
        return []
    ...
    # En iyi 10'u al
    top10_df = df.head(10)
    symbols = top10_df['symbol'].tolist()
    
    logger.info(f"🔗 WebSocket için top 10 hazırlandı: {', '.join(symbols[:3])}...")
    return symbols


def update_websocket_with_signals():
    """
    Mevcut sinyalleri WebSocket'e gönder
    """
    global current_data
    
    if current_data is None or current_data.empty:
        logger.debug("🔗 WebSocket güncellemesi atlandı - veri yok")
        return
    
    try:
        top10_symbols = get_top10_for_websocket(current_data)
        if top10_symbols:
            update_websocket_symbols(top10_symbols)
            logger.info(f"🔗 WebSocket güncellendi: {len(top10_symbols)} sembol")
    except Exception as e:
        logger.error(f"❌ WebSocket güncelleme hatası: {e}")
