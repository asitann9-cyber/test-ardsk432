"""
🔧 Kripto AI Sistemi - Konfigürasyon Dosyası - SADECE LIVE TRADING + WEBSOCKET
Tüm ayarlar ve sabitler burada tanımlanır
🔥 PAPER TRADING KALDIRILDI - Sadece Live Trading
🔗 YENİ: WebSocket real-time veri akışı eklendi
...
# =============================================================================
# 🔗 WEBSOCKET AYARLARI - BASIT
# =============================================================================

# WebSocket Parametreleri
WEBSOCKET_ACTIVE = False
WEBSOCKET_RECONNECT_DELAY = 5  # saniye
RT_TABLE_REFRESH_INTERVAL = 1000  # milisaniye (UI için)
...
DASH_CONFIG = {
    ...
    'host': "127.0.0.1",
    'port': 8050,
    'title': "🤖 AI Crypto Analytics + Live Trading Bot + WebSocket"
}
...
# ⚠️ GEÇİCİ: App.py uyumluluğu için - KULLANILMAYACAK
...
# =============================================================================
# 🔗 WEBSOCKET GLOBAL DEĞİŞKENLERİ - BASIT
# =============================================================================

# WebSocket Durumu
websocket_active = False
websocket_top10_symbols = []  # AI'dan gelen top 10 sembol
websocket_prices = {}  # {symbol: {price, change, volume}}
websocket_realtime_data = None  # Real-time tablo (DataFrame)
...
# ⚠️ GEÇİCİ COMPATIBILITY FONKSİYONLARI - App.py için
...
# =============================================================================
# 🔗 WEBSOCKET KONTROL FONKSİYONLARI - BASIT
# =============================================================================

def update_websocket_symbols(symbols):
    """WebSocket izlenen sembolleri güncelle"""
    global websocket_top10_symbols
    websocket_top10_symbols = symbols
    logging.getLogger("crypto-analytics").info(f"🔗 WebSocket semboller güncellendi: {len(symbols)} adet")

def get_websocket_realtime_data():
    """WebSocket real-time tablosunu al"""
    global websocket_realtime_data
    return websocket_realtime_data

def update_websocket_realtime_data(data):
    """WebSocket real-time tablosunu güncelle"""
    global websocket_realtime_data
    websocket_realtime_data = data

def update_websocket_prices(symbol, price_data):
    """WebSocket fiyat verisi güncelle"""
    global websocket_prices
    websocket_prices[symbol] = price_data

def get_websocket_prices():
    """Tüm WebSocket fiyatları al"""
    return websocket_prices

def get_websocket_top3_for_livebot():
    """Live Bot için en iyi 3 sinyali al"""
    if websocket_realtime_data is None:
        return []
    
    # En iyi 3'ü al
    top3 = websocket_realtime_data.head(3)
    return top3.to_dict('records') if not top3.empty else []
...
def initialize():
    ...
    logger.info("🚀 Kripto AI sistemi - SADECE LIVE TRADING + WEBSOCKET")
    ...
    logger.info("🔗 WebSocket Real-time veri akışı hazır")
    
    # API key kontrolü
    if BINANCE_API_KEY and BINANCE_SECRET_KEY:
        logger.info("✅ Binance API anahtarları yüklendi")
        logger.info(f"🔗 WebSocket endpoint: {WS_BASE}")
    else:
