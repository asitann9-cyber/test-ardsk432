        ...
        try:
            ...
            if results:
                ...
                for symbol_data in permanent_symbols:
                    # C-Signal ile güncelle
                    updated_symbol = analysis_service.update_symbol_with_c_signal(symbol_data, symbol_data.get('timeframe', '4h'))
...
def update_permanent_c_signals(memory_storage, analysis_service, telegram_service):
    """Kalıcı listedeki tüm emtialar için C-Signal değerlerini güncelle + Ters momentum tespit et + Telegram bildirimi"""
    current_time = datetime.now().strftime('%H:%M')
    permanent_symbols = memory_storage.get_permanent_symbols()
    ...
    for symbol_data in permanent_symbols:
        try:
            ...
            # C-Signal ile güncelle
            updated_symbol = analysis_service.update_symbol_with_c_signal(symbol_data, timeframe)
