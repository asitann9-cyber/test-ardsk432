#!/usr/bin/env python3
"""
Supertrend + C-Signal + Devis'So Trend Analiz Sistemi
Ana Flask Uygulaması - Clean Architecture Implementation - LONG & SHORT
"""
import pandas as pd
import logging
...
def register_routes(app, analysis_service, memory_storage, binance_service, telegram_service):
    ...
    def analyze_supertrend():
        """Supertrend analizi gerçekleştir + Devis'So entegrasyonu - LONG & SHORT"""
        try:
            data = request.get_json()
            ...
            logger.info(f"{len(selected_symbols)} sembol için {timeframe} Supertrend + Devis'So analizi başlatılıyor (LONG & SHORT)...")
            ...
            if results:
                ...
                # GÜNCELLEME: LONG & SHORT sistemlerini kalıcı listeye ekle
                for result in results:
                    if analysis_service.is_high_priority_symbol(result):
                        ...
                        # Kalıcı listeye ekle (Devis'So çizgi değerleri dahil)
                        memory_storage.add_permanent_symbol(result)
                
                # Kalıcı listeki Devis'So analizi güncelle + Telegram bildirimi (LONG & SHORT)
                permanent_symbols = memory_storage.get_permanent_symbols()
                deviso_contact_count = 0
                
                for symbol_data in permanent_symbols:
                    # Devis'So analizi ile güncelle (MANUEL TÜR KORUNARAK)
                    updated_symbol = analysis_service.update_symbol_with_deviso_analysis(
                        symbol_data, 
                        symbol_data.get('timeframe', '4h'),
                        preserve_manual_type=symbol_data.get('manual_type_override', False)
                    )
                    
                    # Devis'So çizgi teması varsa Telegram bildirimi gönder (LONG & SHORT)
                    deviso_status = updated_symbol.get('deviso_status', {})
                    if deviso_status.get('has_line_contact', False):
                        deviso_contact_count += 1
                        
                        # Spam önleme kontrolü
                        if telegram_service.should_send_deviso_alert(updated_symbol, deviso_status):
                            success = telegram_service.send_deviso_line_contact_alert(updated_symbol)
                            if success:
                                updated_symbol['last_telegram_alert'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                memory_storage.update_permanent_symbol(symbol_data['symbol'], {
                                    'last_telegram_alert': updated_symbol['last_telegram_alert']
                                })
                
                # Sonuçları formatla (final_ratio, change_momentum, deviso_lines dahil)
                formatted_results = []
                ...
                for i, result in enumerate(results, 1):
                    ...
                    # Devis'So çizgi değerleri
                    deviso_lines = result.get('deviso_lines', {})
                    ...
                    formatted_results.append({
                        ...
                        'deviso_lines': deviso_lines,  # YENİ: Devis'So çizgi değerleri
                ...
                return jsonify({
                    ...
                    "timeframe": timeframe,
                    "deviso_contact_count": deviso_contact_count,
                    "message": f"{len(formatted_results)} sembol analiz edildi - Supertrend + Devis'So Trend (LONG & SHORT) + Telegram bildirimleri ({deviso_contact_count} çizgi teması)"
                })
            else:
                return jsonify({"success": False, "error": "Analiz sonucu bulunamadı"})
                
        except Exception as e:
            logger.error(f"Supertrend + Devis'So analiz API hatası: {e}")
            return jsonify({"success": False, "error": str(e)}), 500

    @app.route('/api/consecutive/permanent-high-consecutive', methods=['GET'])
    def get_permanent_high_ratio():
        """Kalıcı %100+ ratio emtialar listesi - Devis'So Trend + çizgi değerleri ile (LONG & SHORT)"""
        try:
            permanent_symbols = memory_storage.get_permanent_symbols()
            
            # Devis'So çizgi temas sayısını hesapla
            deviso_contact_count = sum(1 for s in permanent_symbols 
            ...
            for i, symbol_data in enumerate(permanent_symbols, 1):
                ...
                formatted_permanent.append({
                    ...
                    # DEVİS'SO TREND BİLGİLERİ
                    'deviso_lines': {
                        'ma_blue': deviso_lines.get('ma_blue', 0.0),
                        'ma_yellow': deviso_lines.get('ma_yellow', 0.0),
                        'ma_purple': deviso_lines.get('ma_purple', 0.0),
                        'last_update': deviso_lines.get('last_update', 'N/A')
                    },
                    'deviso_contact': deviso_status.get('has_line_contact', False),
                    'contact_level': deviso_status.get('contact_level', 'None'),
                    'signal_strength': deviso_status.get('signal_strength', 'None'),
            ...
            return jsonify({
                ...
                "message": f"Kalıcı listede {len(formatted_permanent)} emtia (LONG: %100+, SHORT: %50- + Devis'So Trend: {deviso_contact_count} çizgi teması) + Telegram bildirimleri"
            })
    ...
    # =====================================================
    # MANUEL EMTİA EKLEME ENDPOINT'İ - DEVİS'SO TREND İÇİN (LONG & SHORT)
    # =====================================================
    @app.route('/api/consecutive/add-to-permanent', methods=['POST'])
    def add_symbol_to_permanent():
        """Manuel olarak emtia kalıcı listeye ekleme - Devis'So Trend ile (LONG & SHORT destekli)"""
        try:
            data = request.get_json()
            ...
            # Güncel veriyi al - Supertrend + Devis'So analizi
            ...
            # Manuel ekleme için sembol verisi oluştur (Devis'So çizgi değerleri dahil)
            manual_symbol_data = {
                'symbol': symbol,
                'timeframe': timeframe,
                ...
                'deviso_lines': current_analysis.get('deviso_lines', {}),  # YENİ: Devis'So çizgi değerleri
            ...
            # Devis'So analizi ile güncelle
            permanent_symbol = memory_storage.get_permanent_symbol(symbol)
            if permanent_symbol:
                updated_symbol = analysis_service.update_symbol_with_deviso_analysis(permanent_symbol, timeframe)
            ...
            logger.info(f"✅ {symbol} manuel olarak kalıcı listeye eklendi (Devis'So Trend - LONG & SHORT)")
            
            return jsonify({
                "success": True,
                "message": f"✅ {symbol} kalıcı listeye eklendi! (Devis'So Trend entegrasyonu LONG & SHORT + çizgi takibi)",
                "symbol_data": {
                    'symbol': symbol,
                    ...
                    'timeframe': timeframe,
                    'deviso_lines': manual_symbol_data['deviso_lines']
                }
            })
    ...
    # =====================================================
    # MANUEL TÜR DEĞİŞTİRME - DEVİS'SO TREND İÇİN UYARLANMIŞ (LONG & SHORT)
    # =====================================================
    @app.route('/api/consecutive/update-symbol-type', methods=['POST'])
    def update_symbol_type():
        """Kalıcı listedeki sembolün trend türünü manuel güncelle + Devis'So sıfırlama (LONG & SHORT)"""
        try:
            data = request.get_json()
            symbol = data.get('symbol')
            ...
            # Telegram bildirimi gönder (manuel değişiklik bildirimi - LONG & SHORT)
            updated_permanent_symbol = memory_storage.get_permanent_symbol(symbol)
            if updated_permanent_symbol:
                deviso_status = updated_permanent_symbol.get('deviso_status', {})
                if telegram_service.should_send_deviso_alert(updated_permanent_symbol, deviso_status):
                    success_telegram = telegram_service.send_deviso_line_contact_alert(updated_permanent_symbol)
                    if success_telegram:
                        memory_storage.update_permanent_symbol(symbol, {
                            'last_telegram_alert': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                        })
            
            logger.info(f"🔒 {symbol} trend türü manuel olarak {old_type} -> {new_type} DEĞİŞTİRİLDİ ve KİLİTLENDİ (LONG & SHORT sistemi)")
            
            return jsonify({
                "success": True,
                "message": f"🔒 {symbol} trend türü {old_type} → {new_type} değiştirildi ve KİLİTLENDİ! Devis'So sistemi sıfırlandı! (LONG & SHORT destekli)",
                "symbol": symbol,
                "old_type": old_type,
                "new_type": new_type,
                "is_locked": True,
...
def update_permanent_deviso_analysis(memory_storage, analysis_service, telegram_service):
    """
    Kalıcı listeki sembollerin Devis'So analizini güncelle (LONG & SHORT)
    """
    current_time = datetime.now().strftime('%H:%M')
    permanent_symbols = memory_storage.get_permanent_symbols()
    ...
    for symbol_data in permanent_symbols:
        try:
            ...
            # Devis'So çizgi teması varsa Telegram bildirimi (LONG & SHORT)
            deviso_status = updated_symbol.get('deviso_status', {})
            if deviso_status.get('has_line_contact', False):
                if telegram_service.should_send_deviso_alert(updated_symbol, deviso_status):
                    success = telegram_service.send_deviso_line_contact_alert(updated_symbol)
            ...
            logger.debug(f"Devis'So analizi güncelleme hatası {symbol}: {e}")

def main():
    """Ana uygulama başlatma fonksiyonu - Devis'So Trend entegrasyonu ile (LONG & SHORT)"""
    try:
        print("\n" + "="*70)
        print("🎯 Supertrend + C-Signal + Devis'So Trend Analiz Sistemi")
        print("="*70)
        print("📊 ÖZELLİKLER:")
        ...
        print("   🆕 Devis'So Trend Çizgi Takibi (Ters Momentum yerine)")
        ...
        print("   🎯 DEVİS'SO ÇIZGI TEMAS SİSTEMİ - LONG & SHORT")
        print("="*70)
        ...
        print("="*70)
        print("🎯 DEVİS'SO TREND ÇİZGİ TAKİBİ (LONG & SHORT!):")
        print("   • Pine Script mantığıyla Devis'So indikatörü hesaplama")
        print("   • 3 Ana Çizgi: Mavi (ma_all_signals), Sarı (ma_average), Pembe (ma_all_signals2)")
        print("   🟢 LONG Trend: Mavi > Sarı > Pembe sıralaması")
        print("   • Seviye 1: Fiyat MAVİ çizgiye değdiğinde → Telegram bildirimi")
        print("   • Seviye 2: Mavi delindi, SARI çizgiye değdiğinde → Telegram bildirimi")
        print("   • Seviye 3: Sarı delindi, PEMBE çizgiye değdiğinde → Telegram bildirimi")
        print("   🔴 SHORT Trend: Pembe > Sarı > Mavi sıralaması")
        print("   • Seviye 1: Fiyat MAVİ çizgiye çıktığında → Telegram bildirimi")
        print("   • Seviye 2: Mavi aşıldı, SARI çizgiye çıktığında → Telegram bildirimi")
        print("   • Seviye 3: Sarı aşıldı, PEMBE çizgiye çıktığında → Telegram bildirimi")
        print("   • Kalıcı tabloda çizgi fiyatları (USDT) görünür")
        print("   • Çizgi değerleri her güncellemede yenilenir")
        print("="*70)
        print("🎯 KALICI LİSTE KRİTERİ (LONG & SHORT):")
        print("   🟢 LONG: Supertrend Ratio >= %100 + Bullish trend = Otomatik kalıcı liste")
        print("   🔴 SHORT: Supertrend Ratio <= -%50 + Bearish trend = Otomatik kalıcı liste")
        print("   • Manuel ekleme ve çıkarma imkanı")
        print("   • Devis'So çizgi değerleri + temas takibi")
        print("   • C-Signal hesaplama (RSI momentum)")
        print("="*70)
        print("🔧 GÜNCELLENDİ - DEVİS'SO ENTEGRASYONU (LONG & SHORT):")
        print("   • TERS MOMENTUM sistemi → DEVİS'SO TREND çizgi takibi")
        print("   • analysis_service.py: Devis'So hesaplama eklendi + SHORT sistem")
        print("   • telegram_service.py: Çizgi temas bildirimleri + SHORT seviyeleri")
        print("   • app.py: Devis'So endpoint entegrasyonu + LONG & SHORT")
        print("   • memory_storage.py: Çizgi değerleri ve temas durumu")
        print("   • Kalıcı tabloda: Mavi/Sarı/Pembe çizgi fiyatları")
        print("="*70)
        print("📱 TELEGRAM BİLDİRİMLERİ (LONG & SHORT):")
        print("   • Seviye bazlı çizgi temas uyarıları")
        print("   • Manuel trend değiştirme bildirimleri")
        print("   • Spam önleme (5 dakika minimum interval)")
        print("   🟢 LONG trend için çizgi bildirimleri aktif")
        print("   🔴 SHORT trend için çizgi bildirimleri aktif")
        print("="*70)
        ...
        logger.info("Devis'So Trend entegrasyonu ile sistem başlatılıyor (LONG & SHORT)")
        ...
        logger.info("👋 Devis'So Trend Analiz Sistemi kapatıldı")
        print("\n👋 Sistem kapatıldı")
