 ...
 * Frontend JavaScript Application - DEVİS'SO ÇIZGI TAKİBİ ÖZELLİKLİ
 */
...
class AnalysisApp {
  ...
  // =====================================================
  // FORMATTING FUNCTIONS - DEVİS'SO TREND İÇİN GÜNCELLENDİ
  // =====================================================
  ...
  // =====================================================
  // YENİ: DEVİS'SO ÇIZGI FORMATLAMA FONKSİYONLARI
  // =====================================================

  formatDevisoLines(devisoLines) {
    /**
     * Devis'So çizgi değerlerini formatla
     * @param {Object} devisoLines - Çizgi değerleri objekti
     * @returns {Object} Formatlanmış çizgi değerleri
     */
    if (!devisoLines || typeof devisoLines !== 'object') {
      return {
        ma_blue: '0.00000',
        ma_yellow: '0.00000', 
        ma_purple: '0.00000'
    ...
    return {
      ma_blue: this.formatDevisoPrice(devisoLines.ma_blue),
      ma_yellow: this.formatDevisoPrice(devisoLines.ma_yellow),
      ma_purple: this.formatDevisoPrice(devisoLines.ma_purple)
    };
  }

  formatDevisoPrice(price) {
    /**
     * Devis'So çizgi fiyatını USDT formatında göster
  ...
  formatDevisoContactStatus(devisoStatus) {
    /**
     * Devis'So çizgi temas durumunu formatla
     * @param {Object} devisoStatus - Temas durumu objekti
     * @returns {string} Formatlanmış HTML
     */
    if (!devisoStatus || !devisoStatus.has_line_contact) {
      return '<span style="color: var(--text-muted); font-size: 11px;">-</span>';
    }
    
    const contactLevel = devisoStatus.contact_level || 'None';
    const signalStrength = devisoStatus.signal_strength || 'None';
    
    let levelEmoji = '⚪';
    let levelClass = 'deviso-contact-none';
    let levelText = 'Temas Yok';
    
    switch(contactLevel) {
      case 'Level_1_Blue':
        levelEmoji = '🔵';
        levelClass = 'deviso-contact-blue';
        levelText = 'Seviye 1 - Mavi';
        break;
      case 'Level_2_Yellow':
        levelEmoji = '🟡';
        levelClass = 'deviso-contact-yellow';
        levelText = 'Seviye 2 - Sarı';
        break;
      case 'Level_3_Purple':
        levelEmoji = '🟣';
        levelClass = 'deviso-contact-purple';
        levelText = 'Seviye 3 - Pembe';
        break;
      case 'Manual_Reset':
        levelEmoji = '🚨';
        levelClass = 'deviso-contact-manual';
        levelText = 'Manuel Sıfırlama';
        break;

      case 'Level_1_Blue_Short':
    levelEmoji = '🔵';
    levelClass = 'deviso-contact-blue';
    levelText = 'SHORT Seviye 1 - Mavi';
    break;
  case 'Level_2_Yellow_Short':
    levelEmoji = '🟡';
    levelClass = 'deviso-contact-yellow';
    levelText = 'SHORT Seviye 2 - Sarı';
    break;
  case 'Level_3_Purple_Short':
    levelEmoji = '🟣';
    levelClass = 'deviso-contact-purple';
    levelText = 'SHORT Seviye 3 - Pembe';
    break;
  // ⬆️ YENİ EKLENEN SHORT SEVİYELERİ ⬆️
  case 'Manual_Reset':
    levelEmoji = '🚨';
    levelClass = 'deviso-contact-manual';
    levelText = 'Manuel Sıfırlama';
    ...
    return `
      <div class="deviso-contact-indicator ${levelClass}" style="padding: 3px 6px; border-radius: 8px; font-size: 10px; font-weight: bold; text-align: center;">
        <div>${levelEmoji} ${levelText}</div>
        <div style="margin-top: 2px;">${strengthEmoji} ${signalStrength}</div>
      </div>
    `;
  }
  ...
  formatReverseMomentum(reverseMomentum, reverseType, signalStrength) {
    /**
     * DEPRECATED: Geriye uyumluluk için - Devis'So contact formatla
     */
    console.warn('formatReverseMomentum() DEPRECATED - formatDevisoContactStatus() kullanın');
    
    // Eski formatı yeni formata dönüştür
    const devisoStatus = {
      has_line_contact: reverseMomentum || false,
      contact_level: reverseType === 'MANUEL' ? 'Manual_Reset' : 'None',
      signal_strength: signalStrength || 'None'
    };
    
    return this.formatDevisoContactStatus(devisoStatus);
  }
  ...
  // =====================================================
  // 🆕 MANUEL TÜR DEĞİŞTİRME FONKSİYONU - DEVİS'SO ENTEGRE
  // =====================================================
  ...
  // =====================================================
// PERMANENT LIST FUNCTIONS - DEVİS'SO TREND İÇİN GÜNCELLENDİ
// =====================================================

displayDevisoContactAlerts(permanentSymbols) {
  const alertsContainer = document.getElementById('deviso-contact-alerts');
  const alertContent = document.getElementById('deviso-alert-content');
  const countSpan = document.getElementById('deviso-contact-count');
  
  if (!alertsContainer || !alertContent || !countSpan) return;
  
  const devisoContactSymbols = permanentSymbols.filter(s => s.deviso_contact);
  
  if (devisoContactSymbols.length === 0) {
    alertsContainer.classList.remove('show');
    return;
  }
  
  countSpan.textContent = devisoContactSymbols.length;
  
  let alertHtml = '';
  devisoContactSymbols.forEach(symbol => {
    let alertType, alertClass;
    
    const contactLevel = symbol.contact_level || 'None';
    
    if (contactLevel === 'Level_1_Blue') {
      alertType = '🔵';
      alertClass = 'deviso-contact-blue';
    } else if (contactLevel === 'Level_2_Yellow') {
      alertType = '🟡';
      alertClass = 'deviso-contact-yellow';
    } else if (contactLevel === 'Level_3_Purple') {
      alertType = '🟣';
      alertClass = 'deviso-contact-purple';
    } else if (contactLevel === 'Level_1_Blue_Short') {
      alertType = '🔵';
      alertClass = 'deviso-contact-blue';
    } else if (contactLevel === 'Level_2_Yellow_Short') {
      alertType = '🟡';
      alertClass = 'deviso-contact-yellow';
    } else if (contactLevel === 'Level_3_Purple_Short') {
      alertType = '🟣';
      alertClass = 'deviso-contact-purple';
    } else if (contactLevel === 'Manual_Reset') {
      alertType = '🚨';
      alertClass = 'deviso-contact-manual';
    } else {
      alertType = '⚠️';
      alertClass = 'deviso-contact-other';
    }
    
    alertHtml += `
      <div class="deviso-alert-item">
        ...
        <span class="deviso-contact-indicator ${alertClass}" style="margin: 0 8px;">
          ${alertType} ${contactLevel} - ${symbol.signal_strength}
        </span>
  ...
  async loadPermanentHighRatio() {
    try {
      ...
      if (data.success) {
        ...
        this.displayDevisoContactAlerts(data.permanent_symbols);
        
        const devisoContactCount = data.deviso_contact_count || 0;
        const statusMessage = devisoContactCount > 0 
          ? `🏆 ${data.count} kalıcı emtia + 🎯 ${devisoContactCount} DEVİS'SO ÇIZGI TEMASI tespit edildi!`
          : `🏆 ${data.count} kalıcı emtia yüklendi + Devis'So çizgi takibi aktif`;
        
        this.showStatus('permanent-status', statusMessage, devisoContactCount > 0 ? 'warning' : 'success');
  ...
  updatePermanentTable(permanentSymbols) {
    ...
    if (permanentSymbols.length === 0) {
      tbody.innerHTML = '<tr><td colspan="11" style="padding: 20px; text-align: center; color: var(--text-muted);">🏆 Henüz %100+ ratio olan emtia yok</td></tr>';
      return;
    }
    ...
    permanentSymbols.forEach(symbol => {
      ...
      // DEVİS'SO ÇIZGI TEMASI KONTROLÜ - SATIR VURGULAMA
      if (symbol.deviso_contact) {
        row.classList.add('deviso-contact-alert');
      }
      ...
      // Devis'So çizgi değerlerini formatla
      const devisoLines = this.formatDevisoLines(symbol.deviso_lines);
      
      // Devis'So temas durumunu formatla
      const devisoContactFormatted = this.formatDevisoContactStatus({
        has_line_contact: symbol.deviso_contact,
        contact_level: symbol.contact_level,
      ...
      row.innerHTML = `
        ...
        <td class="deviso-line-blue" style="color: #2196F3; font-weight: bold; font-size: 11px;">${devisoLines.ma_blue}</td>
        <td class="deviso-line-yellow" style="color: #FF9800; font-weight: bold; font-size: 11px;">${devisoLines.ma_yellow}</td>
        <td class="deviso-line-purple" style="color: #9C27B0; font-weight: bold; font-size: 11px;">${devisoLines.ma_purple}</td>
        <td>${devisoContactFormatted}</td>
  ...
  // =====================================================
  // ANALYSIS FUNCTIONS - DEVİS'SO TREND İÇİN GÜNCELLENDİ
  // =====================================================
  ...
  async startSupertrendAnalysis(isAutoUpdate = false) {
    ...
    try {
      if (!isAutoUpdate) {
        this.showStatus('analysis-status', `🎯 ${this.selectedSymbols.length} emtia için ${timeframe} Supertrend + Devis'So analizi başlatılıyor...`, 'info', true);
      } else {
        this.updateCounter++;
        this.showStatus('analysis-status', `🔄 Otomatik güncelleme #${this.updateCounter} - ${this.selectedSymbols.length} emtia analiz ediliyor + Devis'So çizgi takibi...`, 'info', true);
      }
      ...
      if (data.success) {
        ...
        if (!isAutoUpdate) {
          ...
          this.showStatus('analysis-status', `✅ Analiz başlatıldı - Her 30 saniyede otomatik güncellenecek + Devis'So çizgi takibi (${now})`, 'success');
        } else {
          const now = new Date().toLocaleTimeString('tr-TR');
          this.showStatus('analysis-status', `🔄 Otomatik güncelleme aktif - Son: ${now} (${data.results.length} emtia + ${devisoContactCount} çizgi teması)`, 'info');
        }
      } else {
        this.showStatus('analysis-status', `❌ ${data.error}`, 'error');
      }
    } catch (error) {
      console.error('Supertrend + Devis\'So analiz hatası:', error);
      if (!isAutoUpdate) {
  ...
  // =====================================================
  // EVENT LISTENERS - DEVİS'SO TREND İÇİN GÜNCELLENDİ
  // =====================================================
  ...
  bindEventListeners() {
    ...
    // Analysis - Supertrend + Devis'So analizi
  ...
  // =====================================================
  // INITIALIZATION - DEVİS'SO TREND İÇİN GÜNCELLENDİ
  // =====================================================

  async init() {
    console.log('🎯 Supertrend + C-Signal + Devis\'So Trend + Autocomplete System initializing...');
    ...
    try {
      ...
      console.log('✅ Supertrend + C-Signal + Devis\'So Trend + Autocomplete System initialized');
  ...
  displayReverseMomentumAlerts(permanentSymbols) {
    /**
     * DEPRECATED: Geriye uyumluluk için - Devis'So alert göster
     */
    console.warn('displayReverseMomentumAlerts() DEPRECATED - displayDevisoContactAlerts() kullanın');
    this.displayDevisoContactAlerts(permanentSymbols);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 DOM yüklendi, Devis\'So Trend sistemi başlatılıyor...');
  app = new AnalysisApp();
  app.init();
});
