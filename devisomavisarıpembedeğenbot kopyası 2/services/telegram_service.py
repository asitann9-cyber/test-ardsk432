...
Telegram API ile mesaj gönderme ve Devis'So çizgi temas bildirimleri - LONG & SHORT
"""
...
🔗 Supertrend + C-Signal + Devis'So Trend Sistemi (LONG & SHORT)

#Test #TelegramBot #DevisoTrend #LongShort
        """.strip()
    ...
    def send_deviso_line_contact_alert(self, symbol_data: Dict[str, Any]) -> bool:
        """
        Devis'So çizgi temas uyarısı gönder - LONG VE SHORT trend için
    ...
    def _format_deviso_contact_message(self, symbol: str, deviso_status: Dict[str, Any], 
        ...
        """
        Devis'So çizgi temas mesajını formatla - LONG VE SHORT için
        ...
        try:
            ...
            # SHORT seviye kontrolü (YENİ)
            elif contact_level == 'Level_1_Blue_Short':
                level_emoji = "🔵"
                level_text = "SEVİYE 1 - MAVİ ÇİZGİ"
                level_description = "İlk temas noktası"
                trend_text = "SHORT TREND"
            elif contact_level == 'Level_2_Yellow_Short':
                level_emoji = "🟡"
                level_text = "SEVİYE 2 - SARI ÇİZGİ"
                level_description = "Mavi çizgi aşıldı"
                trend_text = "SHORT TREND"
            elif contact_level == 'Level_3_Purple_Short':
                level_emoji = "🟣"
                level_text = "SEVİYE 3 - PEMBE ÇİZGİ"
                level_description = "Sarı çizgi aşıldı - KRİTİK!"
                trend_text = "SHORT TREND"
            
            # Manuel ve diğer durumlar
...
{level_emoji} <b>{symbol}</b> - {trend_text}
📈 {level_text}
💡 {level_description}
...
#DevisoTrend #{symbol} #{trend_text} #{contact_level}
            """.strip()
...
🔥 High Ratio: <b>{high_ratio_count}</b> (LONG: %100+, SHORT: %50-)
🎯 Devis'So Temas: <b>{deviso_contact_count}</b>

#AnalizÖzeti #Supertrend #DevisoTrend #LongShort
            """.strip()
    ...
    def should_send_deviso_alert(self, symbol_data: Dict[str, Any], deviso_status: Dict[str, Any], 
        ...
        """
        Devis'So çizgi temas uyarısı gönderilmeli mi kontrol et (spam önleme) - LONG VE SHORT için
        ...
        # LONG VE SHORT trend için bildirim gönder (YENİ!)
        trend_direction = symbol_data.get('max_supertrend_type', 'None')
        if trend_direction not in ['Bullish', 'Bearish']:  # Bu satır değişti!
            return False
