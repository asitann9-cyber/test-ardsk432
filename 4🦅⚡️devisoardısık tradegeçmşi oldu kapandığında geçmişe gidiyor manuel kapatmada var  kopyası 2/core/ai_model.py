...
🔥 YENİ: Z-Score ceza sistemi eklendi
"""
...
class CryptoMLModel:
    """🔥 YENİ: Dengeli AI model sistemi + Z-Score ceza sistemi"""
    ...
    def predict_score(self, metrics: Dict) -> float:
        """🔥 Güncellenmiş: Z-Score ceza sistemi ile skorlama"""
        ...
        try:
            ...
            # 🔥 YENİ: Z-Score ceza sistemi uygula
            final_score = self._apply_zscore_penalty(base_score, metrics)
    ...
    def _apply_zscore_penalty(self, base_score: float, metrics: Dict) -> float:
        """
        🔥 YENİ: Z-Score ceza sistemi
        Aşırı şişmiş metriklere ceza uygular
        
        Args:
            base_score (float): Temel AI skoru
            metrics (Dict): Metrikler (z-score dahil)
        ...
        try:
            ...
            # Z-Score ceza tablosu
            if max_zscore >= 3.0:
                penalty = 25  # Çok aşırı şişmiş
                logger.debug(f"🚨 Ağır Z-Score cezası: -{penalty} (max_zscore: {max_zscore:.2f})")
            elif max_zscore >= 2.5:
                penalty = 20  # Aşırı şişmiş
                logger.debug(f"⚠️ Orta Z-Score cezası: -{penalty} (max_zscore: {max_zscore:.2f})")
            elif max_zscore >= 2.0:
                penalty = 15  # Şişmiş
                logger.debug(f"⚠️ Hafif Z-Score cezası: -{penalty} (max_zscore: {max_zscore:.2f})")
            else:
                penalty = 0  # Normal
            
            # Ek detaylı cezalar
            volume_zscore = abs(metrics.get('volume_zscore', 0.0))
            price_zscore = abs(metrics.get('price_change_zscore', 0.0))
            deviso_zscore = abs(metrics.get('deviso_zscore', 0.0))
            
            # Hacim aşırı şişmişse ekstra ceza
            if volume_zscore >= 3.0:
                penalty += 10
                logger.debug(f"📊 Ekstra hacim cezası: +10 (volume_zscore: {volume_zscore:.2f})")
            
            # Fiyat değişimi aşırı şişmişse ekstra ceza
            if price_zscore >= 3.0:
                penalty += 5
                logger.debug(f"💰 Ekstra fiyat cezası: +5 (price_zscore: {price_zscore:.2f})")
            
            final_score = base_score - penalty
            
            if penalty > 0:
                logger.debug(f"🎯 Z-Score ceza uygulandı: {base_score:.1f} → {final_score:.1f} (toplam ceza: -{penalty})")
    ...
    def get_model_info(self) -> Dict:
        ...
        return {
            ...
            'model_type': 'Balanced Regression + Z-Score Penalty'
        }
