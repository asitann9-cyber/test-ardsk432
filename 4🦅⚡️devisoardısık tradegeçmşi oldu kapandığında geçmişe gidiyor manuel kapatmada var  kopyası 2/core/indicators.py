...
🔥 YENİ: Z-Score hesaplama eklendi
"""
...
def calculate_z_score_metrics(df: pd.DataFrame, window: int = 20) -> Dict:
    """
    🔥 YENİ: Z-Score hesaplama fonksiyonu
    Volume, price change ve deviso ratio için z-score hesaplar
    
    Args:
        df (pd.DataFrame): OHLCV verileri
        window (int): Rolling window boyutu
        
    Returns:
        Dict: Z-Score metrikleri
    """
    try:
        ...
        volume_zscore = ((vol - vol_mean) / vol_std).iloc[-1]
        volume_zscore = float(volume_zscore) if not pd.isna(volume_zscore) else 0.0
        
        # Price Change Z-Score (son 5 mumun ortalama değişimi)
        price_changes = df['close'].pct_change().rolling(window=5).mean()
        pc_mean = price_changes.rolling(window=window).mean()
        pc_std = price_changes.rolling(window=window).std()
        price_change_zscore = ((price_changes - pc_mean) / pc_std).iloc[-1]
        price_change_zscore = float(price_change_zscore) if not pd.isna(price_change_zscore) else 0.0
        
        # Deviso Ratio Z-Score (basit deviso hesaplama)
        deviso_values = []
        for i in range(max(0, len(df) - window), len(df)):
            try:
                mini_df = df.iloc[max(0, i-10):i+1]  # Son 10 mum ile deviso hesapla
                if len(mini_df) >= 10:
                    deviso_val = calculate_deviso_ratio(mini_df)
                    deviso_values.append(deviso_val)
            except:
                deviso_values.append(0.0)
        
        if len(deviso_values) >= 5:
            deviso_series = pd.Series(deviso_values)
            deviso_mean = deviso_series.mean()
            deviso_std = deviso_series.std()
            if deviso_std > 0:
                deviso_zscore = (deviso_values[-1] - deviso_mean) / deviso_std
            else:
                deviso_zscore = 0.0
        else:
            deviso_zscore = 0.0
        
        deviso_zscore = float(deviso_zscore) if not pd.isna(deviso_zscore) else 0.0
        
        # Maximum Z-Score (en yüksek mutlak değer)
        max_zscore = max(abs(volume_zscore), abs(price_change_zscore), abs(deviso_zscore))
        
        return {
            'volume_zscore': volume_zscore,
            'price_change_zscore': price_change_zscore,
            'deviso_zscore': deviso_zscore,
...
def compute_consecutive_metrics(df: pd.DataFrame) -> Dict:
    ...
    # 🔥 YENİ: Z-Score hesaplama
    try:
        zscore_metrics = calculate_z_score_metrics(df)
        logger.debug(f"🎯 Z-Score hesaplandı - Max: {zscore_metrics['max_zscore']:.2f}")
    except Exception as e:
        logger.debug(f"Z-Score hesaplama hatası: {e}")
        zscore_metrics = {
            'volume_zscore': 0.0,
    ...
    return {
        ...
        'max_zscore': zscore_metrics['max_zscore']
    }
