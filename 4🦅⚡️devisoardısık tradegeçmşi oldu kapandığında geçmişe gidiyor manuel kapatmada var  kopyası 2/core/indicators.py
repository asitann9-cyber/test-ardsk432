...
🔥 DÜZELTME: Pine Script uyumlu TEK Z-Score hesaplama
"""
...
def calculate_z_score_metrics(df: pd.DataFrame, window: int = 20) -> Dict:
    ...
    try:
        ...
        # 🔥 DÜZELTME: Artık sadece tek Z-Score var
        # Backward compatibility için eski field'ları da döndür ama hepsi aynı değer
        return {
            ...
            'max_zscore': abs(zscore)  # Pine Script Z-Score (mutlak değer)
        }
...
def compute_consecutive_metrics(df: pd.DataFrame) -> Dict:
    ...
    🔥 DÜZELTME: Pine Script uyumlu TEK Z-Score eklendi
    ...
    return {
        ...
        # 🔥 DÜZELTME: Pine Script uyumlu TEK Z-Score
