...
def register_callbacks(app, auto_scan_control):
    ...
    def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct, max_zscore):  
        """🔥 YENİ: Z-Score filtresi dahil sinyal güncelleme"""
        ...
        for _, row in df.iterrows():
            ...
            # 🔥 DÜZELTME: Orjinal Z-Score değerlerini gönder (artı/eksi işaretiyle)
            # max_zscore yerine individual z-score'ları kullan
            volume_zscore = row.get('volume_zscore', 0.0)
            price_zscore = row.get('price_change_zscore', 0.0) 
            deviso_zscore = row.get('deviso_zscore', 0.0)
            
            # En yüksek mutlak değerli olanı seç ama işaretiyle birlikte
            abs_scores = [abs(volume_zscore), abs(price_zscore), abs(deviso_zscore)]
            max_abs_index = abs_scores.index(max(abs_scores))
            
            if max_abs_index == 0:
                primary_zscore = volume_zscore
            elif max_abs_index == 1:
                primary_zscore = price_zscore
            else:
                primary_zscore = deviso_zscore
            
            zscore_indicator = create_zscore_indicator(primary_zscore)
            ...
            table_data.append({
                ...
                'max_zscore': zscore_indicator,  # 🔥 YENİ SÜTUN
