"""
🎨 UI Bileşenleri - TAMAMEN GÜNCELLENMİŞ
Dash arayüz bileşenleri ve layout fonksiyonları
🔥 YENİ: Z-Score işaretli gösterim + Pine Script uyumlu renkler
"""
...
def create_zscore_indicator(zscore):
    """
    🔥 GÜNCELLENMIŞ: Z-Score görsel göstergesi - Pine Script uyumlu
    Z-Score değerine göre işaretli (+/-) renk ve emoji döndürür
    
    Pine Script mantığı:
    - Pozitif Z-Score: Turuncu → Kırmızı (0 ile 2.8 arası)
    - Negatif Z-Score: Açık mavi → Sarı (-2 ile 0 arası)
    """
    try:
        zscore = float(zscore)
        
        # Pozitif Z-Score renklendirme (Pine Script: turuncu → kırmızı)
        if zscore >= 2.0:
            return f"🔴 {zscore:+.1f}"      # Kırmızı pozitif (çok yüksek)
        elif zscore >= 1.0:
            return f"🟠 {zscore:+.1f}"      # Turuncu pozitif (yüksek)
        
        # Negatif Z-Score renklendirme (Pine Script: açık mavi → sarı)
        elif zscore <= -2.0:
            return f"🔵 {zscore:+.1f}"      # Mavi negatif (çok düşük)
        elif zscore <= -1.0:
            return f"🟡 {zscore:+.1f}"      # Sarı negatif (düşük)
        
        # Nötr bölge (-1.0 ile +1.0 arası)
        else:
            return f"⚪ {zscore:+.1f}"      # Beyaz nötr
    except:
        return "❓ N/A"
...
def create_control_panel():
    ...
    return html.Div([
        ...
        # 🔥 YENİ: Z-Score filtresi
        html.Div([
            html.Label("Max Z-Score"),
            dcc.Input(
                id='inp-max-zscore',
                type='number',
                value=5.0,
    ...
    return html.Div([
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "SL/TP Mode", "id": "sltp_mode"}  # 🔥 YENİ SÜTUN
            ],
...
def create_signals_table():
    """🔥 Sinyaller tablosunu oluştur - Z-Score işaretli gösterim"""
    return html.Div([
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "Z-Score", "id": "max_zscore"},  # 🔥 İŞARETLİ Z-SCORE SÜTUNU
            ...
            style_data_conditional=[
                ...
                # 🔥 YENİ: Z-Score renklendirme - işaretli değerler için
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} contains 🔴'},
                    'backgroundColor': '#dc2626',  # Kırmızı - Pozitif yüksek
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} contains 🟠'},
                    'backgroundColor': '#f59e0b',  # Turuncu - Pozitif orta
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} contains 🔵'},
                    'backgroundColor': '#0ea5e9',  # Mavi - Negatif yüksek
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} contains 🟡'},
                    'backgroundColor': '#eab308',  # Sarı - Negatif orta
                    'color': '#000000',
                    'fontWeight': 'bold'
                },
