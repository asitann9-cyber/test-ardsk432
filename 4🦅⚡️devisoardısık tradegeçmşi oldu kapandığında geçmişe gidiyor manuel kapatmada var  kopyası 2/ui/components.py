...
def create_zscore_indicator(zscore):
    """
    🔥 YENİ: Z-Score görsel göstergesi
    Z-Score değerine göre renk ve emoji döndürür
    """
    try:
        zscore = float(zscore)
        
        if zscore >= 3.0:
            return f"🚨 {zscore:.1f}"  # Çok tehlikeli (kırmızı)
        elif zscore >= 2.5:
            return f"⚠️ {zscore:.1f}"   # Tehlikeli (turuncu)
        elif zscore >= 2.0:
            return f"⚡ {zscore:.1f}"   # Uyarı (sarı)
        else:
            return f"✅ {zscore:.1f}"   # Normal (yeşil)
    except:
        return "❓ N/A"
...
def create_signals_table():
    """🔥 Sinyaller tablosunu oluştur - Z-Score sütunu eklendi"""
    return html.Div([
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "Z-Score", "id": "max_zscore"},  # 🔥 YENİ SÜTUN
            ...
            style_data_conditional=[
                ...
                # 🔥 YENİ: Z-Score renklendirme
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} >= 3.0'},
                    'backgroundColor': '#dc2626',  # Kırmızı - Çok tehlikeli
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} >= 2.5'},
                    'backgroundColor': '#f59e0b',  # Turuncu - Tehlikeli
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} >= 2.0'},
                    'backgroundColor': '#eab308',  # Sarı - Uyarı
                    'color': '#000000',
                    'fontWeight': 'bold'
                },
