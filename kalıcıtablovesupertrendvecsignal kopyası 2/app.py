...
🆕 YENİ: C-Signal ±20 L/S Sinyal Tespiti ve TELEGRAM BİLDİRİMİ
✅ FIX: Kalıcı tabloda güncel ratio gösterimi
✅ FIX: C-Signal Telegram bildirimleri çalışıyor!
"""
import pandas as pd
import logging
    ...
    def analyze_supertrend():
        ...
        try:
            ...
            if results:
                ...
                logger.info(f"🔍 {len(permanent_symbols)} kalıcı sembol için C-Signal kontrolü başlatılıyor...")
                ...
                for symbol_data in permanent_symbols:
                    ...
                    if c_signal_value is not None:
                        ...
                        # ✅ FIX: Sinyal tetiklendiyse listeye ekle
                        if signal_result['signal_triggered']:
                            c_signal_alerts.append({
                                ...
                                'ratio_percent': updated_symbol.get('ratio_percent', 0),
                                'supertrend_type': updated_symbol.get('supertrend_type', 'None')
                            })
                            logger.info(f"🔔 {updated_symbol['symbol']} C-Signal ALERT tetiklendi: {signal_result['signal_type']} - C={c_signal_value:.2f}")
                
                # 🆕 C-Signal alertleri logla ve TELEGRAM'A GÖNDER
                ...
                if c_signal_alerts:
                    ...
                    # 🔔 TELEGRAM BİLDİRİMLERİ GÖNDER
                    if telegram_service.is_configured():
                        logger.info(f"📱 Telegram konfigürasyonu OK - {len(c_signal_alerts)} alert gönderiliyor...")
                        
                        # ✅ FIX: Alert sayısına göre tek veya toplu gönder
                        if len(c_signal_alerts) == 1:
                            # Tek alert - detaylı mesaj
                            alert = c_signal_alerts[0]
                            
                            # ✅ FIX: Güncel sembol verisini al (spam kontrolü için)
                            symbol_data = memory_storage.get_permanent_symbol(alert['symbol'])
                            
                            if symbol_data:
                                # ✅ FIX: Spam kontrolü - 30 dakika interval
                                can_send = telegram_service.should_send_c_signal_alert(symbol_data, min_interval_minutes=30)
                                
                                if can_send:
                                    logger.info(f"📤 {alert['symbol']} için C-Signal alert gönderiliyor (spam kontrolü geçildi)...")
                                    ...
                                    success = telegram_service.send_c_signal_alert(
                                        ...
                                        ratio_percent=alert['ratio_percent'],
                                        supertrend_type=alert['supertrend_type']
                                    )
                                    
                                    if success:
                                        # ✅ FIX: Alert zamanını güncelle (spam önleme için)
                                        ...
                                        logger.info(f"✅ {alert['symbol']} C-Signal alert Telegram'a başarıyla gönderildi!")
                                    else:
                                        logger.error(f"❌ {alert['symbol']} C-Signal alert gönderilemedi!")
                                else:
                                    logger.info(f"⏳ {alert['symbol']} spam koruması aktif - alert atlandı (30 dakika bekleme)")
                            else:
                                logger.warning(f"⚠️ {alert['symbol']} kalıcı listede bulunamadı!")
                        
                        elif len(c_signal_alerts) > 1:
                            # ✅ FIX: Birden fazla alert - toplu mesaj (spam kontrolsüz)
                            logger.info(f"📤 {len(c_signal_alerts)} alert toplu mesaj olarak gönderiliyor...")
                            
                            sent_count = telegram_service.send_batch_c_signal_alerts(c_signal_alerts)
                            
                            if sent_count > 0:
                                # ✅ FIX: Tüm alertler için zaman güncellemesi
                                for alert in c_signal_alerts:
                                    memory_storage.update_permanent_symbol(alert['symbol'], {
                                        'last_c_signal_alert_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                    })
                                ...
                                logger.info(f"✅ {sent_count} C-Signal alert toplu olarak Telegram'a başarıyla gönderildi!")
                            else:
                                logger.error(f"❌ Toplu C-Signal alert gönderilemedi!")
                    else:
                        logger.warning("⚠️ Telegram konfigürasyonu eksik, C-Signal alertleri gönderilemedi")
                        logger.warning(f"   Bot Token: {'✅' if telegram_service.bot_token else '❌'}")
                        logger.warning(f"   Chat ID: {'✅' if telegram_service.chat_id else '❌'}")
                else:
                    logger.info("ℹ️ Yeni C-Signal alert yok")
                ...
                # ✅ FIX: Telegram gönderim durumu mesajı
                telegram_status_msg = ""
                if c_signal_alerts:
                    if telegram_sent_count > 0:
                        telegram_status_msg = f" - ✅ {telegram_sent_count} alert Telegram'a gönderildi"
                    else:
                        telegram_status_msg = f" - ⏳ {len(c_signal_alerts)} alert spam koruması nedeniyle atlandı"
                ...
                return jsonify({
                    ...
                    "message": f"{len(formatted_results)} sembol analiz edildi - {high_priority_count} sembol kalıcı listeye eklendi - {len(c_signal_alerts)} C-Signal alert{telegram_status_msg}"
                })
            else:
...
def main():
    ...
    try:
        ...
        print("   ✅ C-Signal TELEGRAM BİLDİRİMLERİ ÇALIŞIYOR!")
        ...
        print("   ✅ TELEGRAM'A OTOMATIK BİLDİRİM ÇALIŞIYOR!")
        print("="*70)
        print("📱 TELEGRAM BİLDİRİMLERİ:")
        print("   ✅ Manuel trend değiştirme bildirimleri")
        print("   ✅ C-Signal L/S bildirimleri (ÇALIŞIYOR!)")
        print("   ✅ Tek veya toplu alert mesajları")
        print("   ✅ Spam önleme (C-Signal: 30 dakika interval)")
        print("   ✅ Detaylı loglama sistemi")
        print("="*70)
