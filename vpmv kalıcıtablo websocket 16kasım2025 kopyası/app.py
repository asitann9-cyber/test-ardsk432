...
def handle_kline_closed(symbol, close_price, is_kline_closed):
    """WebSocket'ten gelen HER VERİYİ işle (açık + kapalı mumlar)"""
    try:
        # ✅ YENİ: Açık mumları da işle, sadece logla
        if not is_kline_closed:
            logger.debug(f"⏳ Açık mum: {symbol} | close={close_price}")
        else:
        ...
        if permanent_symbol:
            ...
            # 🔥 PANEL ANLIK GÜNCELLEME (HER 1-2 SANİYE)
            WebSocketManager.broadcast({
                "event": "permanent_update",
                "data": memory_storage.get_permanent_symbols()
            })
