...
def batch_analyze_with_ai(interval: str) -> pd.DataFrame:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                if processed_count % 100 == 0:
                    ...
                    logger.info(f"🤖 İşlenen: {processed_count}/{len(symbols)} - Hız: {rate:.1f} s/sn - Deviso Başarı: {success_rate:.1f}% ({deviso_success_count})")
    ...
    for symbol, saved_info in list(saved_signals.items()):
        ...
        if symbol not in fresh_symbols:
            ...
            # Güçlü trend varsa daha az ceza
            ...
            logger.debug(f"📉 {symbol}: {original_score:.0f} → {new_score:.0f} (yaş: {minutes_old:.1f}dk, deviso: {deviso_ratio:.2f})")
    ...
    logger.info(f"✅ YENİ DEVISO AI Analiz tamamlandı:")
    ...
    # 🔥 YENİ: Deviso ratio'ya göre de sıralama
    df = df.sort_values(
    ...
    if len(df) > 0:
        ...
        logger.info(f"🏆 En yüksek AI skoru: {top_signal['ai_score']:.0f}% - {top_signal['symbol']} (Deviso: {top_signal['deviso_ratio']:.2f})")
        ...
        for i, row in df.head(5).iterrows():
            logger.debug(f"   {i}: {row['symbol']} | AI={row['ai_score']} | Trend={row.get('trend_strength',0)}")
...
def get_deviso_correlation_analysis(df: pd.DataFrame) -> Dict:
    ...
    correlations = {}
    try:
        if all(col in df.columns for col in ['deviso_ratio', 'ai_score', 'run_perc', 'vol_ratio']):
            ...
            if df['vol_ratio'].notna().sum() > 2:
                correlations['deviso_vol_corr'] = df['deviso_ratio'].corr(df['vol_ratio'])
