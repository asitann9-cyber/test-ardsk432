"""
🚀 Ana Uygulama - Kripto AI Trading Sistemi (Deviso Entegrasyonu)
Modüler yapıda organize edilmiş Dash web uygulaması + Live Trading Bot + Deviso
"""
...
from config import (
    ...
    DEVISO_AVAILABLE, deviso_manager  # YENİ!
)
...
if DEVISO_AVAILABLE:
    try:
        from trading.deviso_manager import DevisoManager
        from trading.deviso_live_test import DevisoLiveTest
        from trading.deviso_live_trading import DevisoLiveTrading
        DEVISO_MODULES_AVAILABLE = True
    except ImportError as e:
        print(f"⚠️ Deviso modülleri import edilemedi: {e}")
        DEVISO_MODULES_AVAILABLE = False
        DEVISO_AVAILABLE = False
else:
    DEVISO_MODULES_AVAILABLE = False
...
# 🆕 DEVISO CALLBACK'LERİ
if DEVISO_AVAILABLE and DEVISO_MODULES_AVAILABLE:
    @app.callback(
        Output('deviso-status', 'children'),
        [Input('btn-start-deviso-test', 'n_clicks'),
         Input('btn-start-deviso-live', 'n_clicks'),
         Input('btn-stop-deviso', 'n_clicks')]
    )
    def control_deviso_bots(test_clicks, live_clicks, stop_clicks):
        """🎯 Deviso bot kontrolü"""
        if not deviso_manager:
            return "❌ Deviso manager bulunamadı"
        
        ctx = dash.callback_context
        
        if not ctx.triggered:
            return deviso_manager.get_status_text()
        
        trigger_id = ctx.triggered[0]['prop_id'].split('.')[0]
        
        if trigger_id == 'btn-start-deviso-test' and test_clicks > 0:
            success = deviso_manager.start_live_test()
            if success:
                return "🧪 Deviso Live Test Başlatıldı (Demo)"
            else:
                return "❌ Deviso Live Test Başlatılamadı"
        
        elif trigger_id == 'btn-start-deviso-live' and live_clicks > 0:
            success = deviso_manager.start_live_trading()
            if success:
                return "💰 Deviso Live Trading Başlatıldı (Gerçek Para)"
            else:
                return "❌ Deviso Live Trading Başlatılamadı"
        
        elif trigger_id == 'btn-stop-deviso' and stop_clicks > 0:
            deviso_manager.stop_all_bots()
            return "⛔ Tüm Deviso botları durduruldu"
        
        return deviso_manager.get_status_text()

    @app.callback(
        Output('deviso-timeframe-status', 'children'),
        [Input('dd-timeframe', 'value'),
         Input('inp-min-ai', 'value')]
    )
    def update_deviso_timeframe(selected_timeframe, min_ai_score):
        """🎯 UI'den timeframe değişikliği geldiğinde Deviso botlarını güncelle"""
        if not deviso_manager:
            return "Deviso sistem yok"
        
        # Deviso manager'a timeframe güncellemesi gönder
        deviso_manager.update_timeframe_settings(selected_timeframe, min_ai_score)
        
        # Strateji bilgisi döndür
        strategy_info = deviso_manager.get_current_strategy_info()
        
        return html.Div([
            html.P(f"🎯 Deviso Timeframe: {selected_timeframe}"),
            html.P(f"📊 Strateji: {strategy_info['strategy'].upper()}"),
            html.P(f"🤖 Önerilen AI Eşiği: {strategy_info['recommended_ai']}%"),
            html.P(f"🔍 Analiz TF'leri: {', '.join(strategy_info['analysis_timeframes'])}")
        ])

    @app.callback(
        Output('deviso-top3-analysis', 'children'),
        [Input('interval-component', 'n_intervals')]
    )
    def update_deviso_top3_display(n_intervals):
        """En iyi 3 seçimini göster"""
        if not deviso_manager:
            return "Deviso yok"
        
        top3_data = deviso_manager.get_top3_analysis()
        
        # Live Test Top 3
        live_test_top3 = top3_data.get('live_test_top3', [])
        # Live Trading Top 3  
        live_trading_top3 = top3_data.get('live_trading_top3', [])
        
        return html.Div([
            html.H4("🧪 Live Test - En İyi 3 Seçimi"),
            html.Div([
                html.P(f"{i+1}. {signal['symbol']} - AI: {signal['ai_score']:.0f}%")
                for i, signal in enumerate(live_test_top3)
            ]) if live_test_top3 else html.P("Henüz seçim yapılmadı"),
            
            html.H4("💰 Live Trading - En İyi 3 Seçimi"),
            html.Div([
                html.P(f"{i+1}. {signal['symbol']} - AI: {signal['ai_score']:.0f}%")
                for i, signal in enumerate(live_trading_top3)
            ]) if live_trading_top3 else html.P("Henüz seçim yapılmadı")
        ])
    ...
    try:
        ...
        if not trades_df.empty:
            ...
            for _, row in trades_df.sort_values('timestamp', ascending=False).head(50).iterrows():
                ...
                if DEVISO_AVAILABLE:
                    trade_data['bot_type'] = row.get('bot_type', 'Ana Bot')
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):  
    ...
    for _, row in df.iterrows():
        ...
        signal_data = {
            ...
            'live_status': "⭐",  # Placeholder: ⭐ = Beklemede, ✅ = Açık pozisyon
            'timestamp': datetime.now(LOCAL_TZ).strftime('%H:%M')
        }
        
        # 🆕 Deviso status sütunu ekle (sadece DEVISO_AVAILABLE=True ise)
        if DEVISO_AVAILABLE:
            signal_data['deviso_status'] = "🎯"  # Placeholder: 🎯 = Deviso tarafından izleniyor
        
        table_data.append(signal_data)
    ...
    # 🆕 Deviso durumu
    if DEVISO_AVAILABLE and deviso_manager:
        deviso_status = deviso_manager.get_brief_status()
        status += f" | 🎯 Deviso: {deviso_status}"
...
if __name__ == "__main__":
    setup_csv_files()
    logger.info("🚀 AI Crypto Analytics + Live Trading + Deviso başlatılıyor...")
    ...
    # Deviso kontrolü
    if DEVISO_AVAILABLE and DEVISO_MODULES_AVAILABLE:
        logger.info("🎯 Deviso bot sistemi hazır")
        if deviso_manager:
            logger.info("✅ Deviso Manager başlatıldı")
    elif DEVISO_AVAILABLE:
        logger.warning("⚠️ Deviso sistemi aktif ama modüller yüklenemedi")
    else:
        logger.info("ℹ️ Deviso sistemi deaktif")
