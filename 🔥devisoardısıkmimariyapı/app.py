    ...
    try:
        ...
        if status['api_connected']:
            ...
            return html.Div([
                ...
                html.Br(),
                html.Small("Binance API anahtarlarını .env dosyasında kontrol edin", style={'color': '#9ca3af'})
            ])
...
def update_performance_metrics(n_intervals):
    ...
    try:
        ...
        for symbol, position in open_positions.items():
            ...
            total_invested += position.get('invested_amount', 0)
        ...
        return html.Div([
            ...
            html.Div([
                ...
                html.H2(f"${effective_capital:.2f}", style={'color': '#3b82f6', 'margin': '5px 0', 'fontSize': '18px'})
            ], style={
                'width': '16%', 'display': 'inline-block', 'textAlign': 'center', 
            ...
            html.Div([
                html.H4("📋 Açık Pozisyon", style={'margin': '0', 'fontSize': '14px', 'color': '#6b7280'}),
                html.H2(f"{len(open_positions)}/{MAX_OPEN_POSITIONS}", style={'color': '#6b7280', 'margin': '5px 0', 'fontSize': '18px'})
            ], style={
                'width': '16%', 'display': 'inline-block', 'textAlign': 'center', 
            ...
            # 🔥 YENİ: Otomatik SL/TP durumu
            html.Div([
                html.H4("🤖 Auto SL/TP", style={'margin': '0', 'fontSize': '14px', 'color': '#f59e0b'}),
                html.H2(f"{auto_sltp_count}/{len(open_positions)}", style={'color': '#f59e0b', 'margin': '5px 0', 'fontSize': '18px'})
            ], style={
                'width': '16%', 'display': 'inline-block', 'textAlign': 'center', 
...
def update_trading_tables(n_intervals):
    """🔥 YENİ: Config'den güncel pozisyonları alan trading tabloları"""
    positions_data = []
    ...
    if config.is_live_mode():
        ...
        current_positions = config.paper_positions
        mode_info = "Paper"
    ...
    try:
        for symbol, position in current_positions.items():
            ...
            positions_data.append({
                ...
                'sltp_mode': auto_sltp_status  # 🔥 YENİ SÜTUN
            })
    except Exception as e:
