...
🔥 LIVE TRADER ENTEGRASYONu TAMAMLANDI
"""
...
try:
    from trading.live_trader import (
        ...
        get_live_trading_status, live_bot
    )
...
def update_performance_metrics(n_intervals):
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.H2(f"{get_auto_sltp_count()}/{len(open_positions)}", style={'color': '#f59e0b', 'margin': '5px 0', 'fontSize': '18px'})
        ], style={
            'width': '16%', 'display': 'inline-block', 'textAlign': 'center', 
...
def get_auto_sltp_count():
    """🔥 YENİ: Otomatik SL/TP olan pozisyon sayısını al"""
    try:
        if LIVE_TRADER_AVAILABLE:
            status = get_live_trading_status()
            return status.get('auto_sltp_positions', 0)
        return 0
    except:
        return 0
...
def update_trading_tables(n_intervals):
    ...
    try:
        for symbol, position in open_positions.items():
            ...
            auto_sltp_status = "🤖 Auto" if position.get('auto_sltp', False) else "📱 Manual"
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):  
    # 🔥 YENİ: config.current_data kullan
    ...
    if LIVE_TRADER_AVAILABLE and is_live_trading_active():
        live_positions_count = len(open_positions)
        auto_sltp_count = get_auto_sltp_count()
        status += f" | 🤖 Live Bot aktif ({live_positions_count}/{MAX_OPEN_POSITIONS}) [Auto SL/TP: {auto_sltp_count}]"
...
def get_live_bot_status_for_symbol(symbol):
    """🔥 YENİ: Sembol için live bot durumunu al"""
    try:
