...
class LiveTradingBot:
    ...
    def __init__(self):
        ...
        self.symbol_info_cache: Dict[str, Dict] = {}  # 🔥 YENİ: Sembol bilgi cache
            ...
            for s in exchange_info["symbols"]:
                if s["symbol"] == symbol:
                    info = {
                        "symbol": symbol,
                        "status": s["status"],
                        "quantity_precision": s["quantityPrecision"],  # 🔥 YENİ
                    ...
                    self.symbol_info_cache[symbol] = info  # Cache'e kaydet
                    return info
            return {}
        except Exception as e:
    ...
    def calculate_position_size(self, symbol: str, price: float) -> float:
        ...
        try:
            ...
            quantity_precision = int(info.get("quantity_precision", 8))  # 🔥 YENİ

            # 🔥 DÜZELTME: Quantity precision'a göre yuvarla
            ...
            if lot_size > 0:
                ...
                qty = round(qty, quantity_precision)  # Tekrar precision'a yuvarla
    ...
    def open_position(self, signal: Dict) -> bool:
        ...
        try:
            ...
            if order_status in ["FILLED", "PARTIALLY_FILLED"] and executed_qty > 0:
                ...
                # 2️⃣ Stop Loss emri (otomatik)
                sl_order_id = None
                try:
                    sl_order = self.client.futures_create_order(
                        symbol=symbol,
                        side=close_side,
                        type="STOP_MARKET",  # 🔥 DÜZELTME: String kullan
                        quantity=executed_qty,
                        stopPrice=stop_loss,
                        timeInForce="GTC",  # 🔥 DÜZELTME: String kullan
                        recvWindow=60000,
                    )
                ...
                # 3️⃣ Take Profit emri (otomatik)
                tp_order_id = None
                try:
                    tp_order = self.client.futures_create_order(
                        symbol=symbol,
                        side=close_side,
                        type="TAKE_PROFIT_MARKET",  # 🔥 DÜZELTME: String kullan
                        quantity=executed_qty,
                ...
                position_data = {
                    ...
                    "sl_order_id": sl_order_id,  # 🔥 YENİ
                    "tp_order_id": tp_order_id,  # 🔥 YENİ
                    "auto_sltp": True,  # 🔥 YENİ: Otomatik SL/TP aktif
                }
    ...
    def close_position(self, symbol: str, close_reason: str) -> bool:
        ...
        try:
            ...
            # 1️⃣ Bekleyen SL/TP emirlerini iptal et
    ...
    def monitor_positions(self) -> None:
        ...
        try:
            ...
            for symbol, position in list(open_positions.items()):
                # Otomatik SL/TP olan pozisyonları atlayabilir veya daha az sıklıkta kontrol edebiliriz
                if position.get("auto_sltp", False):
                    # Sadece pozisyon durumunu logla
                    current_price = get_current_price(symbol)
                    if current_price:
                ...
                # İleride manuel pozisyonlar için kontrol ekleyebilirsiniz
...
def live_trading_loop() -> None:
    ...
    while live_trading_active:
        try:
            ...
            # 🔥 YENİ: Bekleyen emirleri temizle (her döngüde)
...
def get_live_trading_status() -> Dict:
    ...
    return {
        ...
        "auto_sltp_positions": auto_sltp_count,  # 🔥 YENİ
        "auto_sltp_enabled": True,  # 🔥 YENİ
    }