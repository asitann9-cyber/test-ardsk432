        ...
        try:
            ...
            if results:
                ...
                return jsonify({
                    ...
                    "timeframe": timeframe,
                    "message": f"{len(formatted_results)} sembol analiz edildi - Supertrend + Telegram bildirimleri"
                })
            else:
...
def update_permanent_c_signals(memory_storage, analysis_service, telegram_service):
    ...
    AMA MANUEL DEĞİŞTİRİLEN TÜRLERİ KORUMA ALTINDA TUT (Supertrend için)
    """
    current_time = datetime.now().strftime('%H:%M')
    permanent_symbols = memory_storage.get_permanent_symbols()
    ...
    for symbol_data in permanent_symbols:
        try:
            ...
            if is_manual_override:
                logger.debug(f"🔒 {symbol} manuel tür koruması aktif: {symbol_data.get('manual_type_value', 'Unknown')} - TÜR KORUNUYOR")
...
def main():
    ...
    try:
        ...
        print("   • ATR bazlı Supertrend hesaplama")
        print("   • Pullback seviyesi ve Ratio % hesaplama")
        ...
        logger.info("Clean Architecture ile Supertrend sistem bellekte veri tutacak, CSV kullanılmayacak")
