...
🆕 YENİ: MTF (Multi-TimeFrame) TIME sistemi - Pine Script uyumlu
...
📊 YENİ: get_mtf_directions() - 5 timeframe mum yönü tespiti (1h, 2h, 4h, 1d, 1w)
"""
...
class AnalysisService:
    """Supertrend, C-Signal, VPMV ve MTF TIME analizi için service sınıfı"""
    ...
    # =====================================================
    # 📊 YENİ: MTF (MULTI-TIMEFRAME) TIME SİSTEMİ
    # =====================================================
    
    @staticmethod
    def get_mtf_directions(symbol: str) -> Dict[str, int]:
        """
        📊 Pine Script TIME mantığı - 5 timeframe'in mum yönlerini hesapla
        
        Pine Script:
        c1H = request.security(syminfo.tickerid, "60", close)
        o1H = request.security(syminfo.tickerid, "60", open)
        dir1H = c1H > o1H ? 1 : c1H < o1H ? -1 : 0
        ...
        try:
            ...
            for tf in timeframes:
                try:
                    # Her timeframe için son 2 mumu çek (güvenlik için)
                    df = BinanceService.fetch_klines_data(symbol, tf, limit=2)
                    
                    if df is not None and len(df) >= 1:
                        # Son mumun open ve close değerleri
                        close_price = float(df['close'].iloc[-1])
                        open_price = float(df['open'].iloc[-1])
                        
                        # Pine Script mantığı: dir = close > open ? 1 : close < open ? -1 : 0
                        if close_price > open_price:
                            directions[tf] = 1  # Bullish 🟢
                        elif close_price < open_price:
                            directions[tf] = -1  # Bearish 🔴
                        else:
                            directions[tf] = 0  # Nötr ⚪ (eşit - nadir)
                        
                        logger.debug(f"📊 {symbol} {tf.upper()}: close={close_price:.4f}, open={open_price:.4f}, dir={directions[tf]}")
                    else:
                        # Veri alınamazsa nötr kabul et
                        directions[tf] = 0
                        logger.warning(f"⚠️ {symbol} {tf.upper()}: Veri alınamadı, nötr kabul edildi")
                        
                except Exception as tf_error:
                    logger.debug(f"❌ {symbol} {tf.upper()} yön hesaplama hatası: {tf_error}")
                    directions[tf] = 0  # Hata durumunda nötr
            
            logger.info(f"📊 {symbol} MTF yönleri: 1H={directions.get('1h', 0)} 2H={directions.get('2h', 0)} 4H={directions.get('4h', 0)} 1D={directions.get('1d', 0)} 1W={directions.get('1w', 0)}")
            return directions
            
        except Exception as e:
            logger.error(f"❌ MTF yön hesaplama hatası ({symbol}): {e}")
            # Hata durumunda tüm timeframe'leri nötr döndür
            return {
                '1h': 0,
                '2h': 0,
                '4h': 0,
    ...
    @staticmethod
    def calculate_mtf_match_count(mtf_directions: Dict[str, int], vpmv_direction: int) -> int:
        """
        MTF yönleri ile VPMV yönünün kaç tanesinin uyuştuğunu hesapla
        
        Pine Script:
        vpmv_dir = vpmv_score >= 0 ? 1 : -1
        match1H = vpmv_dir == dir1H and dir1H != 0
        matchCount = (match1H ? 1 : 0) + (match2H ? 1 : 0) + ...
        
        Args:
            mtf_directions (Dict[str, int]): MTF yönleri
            vpmv_direction (int): VPMV yönü (1=Long, -1=Short)
            
        Returns:
            int: Uyuşan timeframe sayısı (0-5)
        """
        try:
            ...
            for tf in timeframes:
                tf_dir = mtf_directions.get(tf, 0)
                
                # Pine mantığı: vpmv_dir == tf_dir VE tf_dir != 0
                if vpmv_direction == tf_dir and tf_dir != 0:
                    match_count += 1
            
            return match_count
            
        except Exception as e:
            logger.debug(f"MTF match count hesaplama hatası: {e}")
            return 0
    ...
    def analyze_single_symbol(symbol: str, timeframe: str = '4h') -> Optional[Dict[str, Any]]:
        """
        Tek sembol için TAM analiz (Supertrend + C-Signal + VPMV + TETİKLEYİCİ + MTF TIME)
        🆕 Pine Script uyumlu 4 bileşenli VPMV sistemi + Tetikleyici + MTF TIME
        ...
        Returns:
            Optional[Dict[str, Any]]: Analiz sonuçları (VPMV 4 bileşen + Tetikleyici + MTF TIME dahil)
        """
        try:
            ...
            # 📊 YENİ: MTF TIME verileri
            mtf_directions = AnalysisService.get_mtf_directions(symbol)
            vpmv_direction = 1 if vpmv_net_power >= 0 else -1
            mtf_match_count = AnalysisService.calculate_mtf_match_count(mtf_directions, vpmv_direction)
            ...
            return {
                ...
                # 📊 YENİ: MTF TIME bilgileri
                'mtf_directions': mtf_directions,  # {'1h': 1, '2h': -1, ...}
                'mtf_match_count': mtf_match_count,  # 0-5 arası
    ...
    def analyze_multiple_symbols(symbols: List[str], timeframe: str = '4h', max_workers: int = 5) -> List[Dict[str, Any]]:
        """
        Çoklu sembol analizi - Paralel işleme (VPMV + Tetikleyici + MTF TIME dahil)
        ...
        Returns:
            List[Dict[str, Any]]: Analiz sonuçları listesi (VPMV + Tetikleyici + MTF TIME dahil)
        """
        try:
            logger.info(f"{len(symbols)} sembol için {timeframe} TAM analizi (Supertrend+C-Signal+VPMV+Tetikleyici+MTF) başlatılıyor...")
            ...
            logger.info(f"✅ {len(results)} sembol TAM analizi edildi (VPMV + Tetikleyici + MTF TIME dahil)")
            return results
    ...
    def get_analysis_summary(results: List[Dict[str, Any]], timeframe: str) -> Dict[str, Any]:
        """
        Analiz özetini hazırla (VPMV + Tetikleyici + MTF TIME dahil)
        ...
        try:
            ...
            # 📊 YENİ: MTF TIME istatistikleri
            mtf_high_match_count = sum(1 for r in results if r.get('mtf_match_count', 0) >= 4)
            ...
            return {
                ...
                'mtf_high_match_count': mtf_high_match_count,
                'timeframe': timeframe,
                'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            }
    ...
    def update_symbol_with_analysis(symbol_data: Dict[str, Any], timeframe: str, preserve_manual_type: bool = False) -> Dict[str, Any]:
        """
        Manuel tür korunarak sembol verisini güncelle (VPMV + C-Signal + Tetikleyici + MTF TIME dahil)
        """
        try:
            ...
            if df is not None and len(df) >= 50:
                ...
                # 📊 MTF TIME verilerini güncelle (Her durumda!)
                mtf_directions = AnalysisService.get_mtf_directions(symbol)
                symbol_data['mtf_directions'] = mtf_directions
                
                # Manuel tür koruma kontrolü
                if preserve_manual_type:
                    # ✅ Sadece VPMV + Tetikleyici güncelle
                    ...
                    # MTF match count güncelle
                    vpmv_direction = 1 if vpmv_result['net_power'] >= 0 else -1
                    symbol_data['mtf_match_count'] = AnalysisService.calculate_mtf_match_count(mtf_directions, vpmv_direction)
                    
                    logger.info(f"🔒 {symbol} MANUEL TÜR KORUNDU - VPMV + C-Signal + Tetikleyici + MTF güncellendi")
                    
                else:
                    # ✅ TAM ANALİZ (Supertrend + VPMV + C-Signal + Tetikleyici + MTF TIME)
                    ...
                    if full_analysis:
                        ...
                        symbol_data['mtf_match_count'] = full_analysis.get('mtf_match_count', 0)
                        
                        logger.info(f"🔄 {symbol} TAM GÜNCELLEME YAPILDI")
                
                # 🔥 LOG - C-Signal değeri
                logger.info(f"💡 C-Signal güncellendi: {symbol} = {c_signal}")
                logger.info(f"📊 MTF Match Count: {symbol} = {symbol_data.get('mtf_match_count', 0)}/5")
                ...
                symbol_data['mtf_directions'] = {'1h': 0, '2h': 0, '4h': 0, '1d': 0, '1w': 0}
                symbol_data['mtf_match_count'] = 0
