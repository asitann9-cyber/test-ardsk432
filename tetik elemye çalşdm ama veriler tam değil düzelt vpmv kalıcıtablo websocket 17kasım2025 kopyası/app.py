...
def handle_kline_closed(symbol, close_price, is_kline_closed):
    """
    ⚡ CRITICAL FIX: WebSocket'ten gelen HER VERİYİ işle
    
    🔥 YENİ MANTIK:
    - AÇIK MUM: Sadece fiyat güncelle (hızlı)
    - KAPALI MUM: Tam analiz yap (Supertrend + VPMV + Tetikleyici)
    """
    
    # 🔥🔥🔥 EN ÜST SATIRA EKLE - HER CALLBACK'TE ÇALIŞIR 🔥🔥🔥
    logger.info(f"╔{'=' * 78}╗")
    logger.info(f"║ 🔥 CALLBACK ÇAĞRILDI!{' ' * 51}║")
    logger.info(f"║   Symbol: {symbol:<64}║")
    logger.info(f"║   Price: {close_price:<65}║")
    logger.info(f"║   Is Closed: {is_kline_closed:<59}║")
    logger.info(f"╚{'=' * 78}╝")
    
    try:
        # ✅ ÖNCE kontrol et - kalıcı listede mi?
        permanent_symbol = memory_storage.get_permanent_symbol(symbol)
        if not permanent_symbol:
            logger.warning(f"⚠️ {symbol} kalıcı listede YOK, işlem yapılmıyor")
            return  # Kalıcı listede değilse işlem yapma
        
        if not is_kline_closed:
            # ⚡⚡⚡ AÇIK MUM - HIZLI GÜNCELLEME ⚡⚡⚡
            current_time = datetime.now().strftime('%H:%M:%S.%f')[:-3]  # Milisaniye ile
            
            # 🔥 HER AÇIK MUM İÇİN DETAYLI LOG
            logger.info(f"⚡⚡⚡ AÇIK MUM GELDI! ⚡⚡⚡")
            logger.info(f"   📍 Sembol: {symbol}")
            logger.info(f"   💰 Fiyat: {close_price}")
            logger.info(f"   ⏰ Zaman: {current_time}")
            logger.info(f"   🔥 is_kline_closed: {is_kline_closed}")
            
            # ✅ Sadece fiyatı güncelle - analiz yapma
            memory_storage.update_permanent_symbol(symbol, {
                'current_price': close_price,
                'last_live_update': current_time
            })
            
            # ✅ Frontend'e anlık broadcast (FARKLI EVENT TİPİ)
            WebSocketManager.broadcast({
                "event": "live_price_update",  # 🆕 Yeni event tipi
                "symbol": symbol,
                "price": close_price,
                "time": current_time,
            ...
            logger.info(f"   ✅ Frontend'e live_price_update gönderildi")
            
        else:
            # 🟦 KAPANAN MUM - TAM ANALİZ
            logger.info(f"🟦 KAPANAN MUM: {symbol} | close={close_price}")
            
            # ✅ Tam analiz (Supertrend + VPMV + Tetikleyici)
            updated = AnalysisService.update_symbol_with_analysis(
                permanent_symbol,
                permanent_symbol.get("timeframe", "4h"),
            ...
            # ✅ Frontend'e tam güncelleme
            WebSocketManager.broadcast({
                "event": "permanent_update",
                "data": memory_storage.get_permanent_symbols()
            })
            
            logger.info(f"✅ {symbol} tam analiz: Ratio={updated.get('ratio_percent')}%, VPMV={updated.get('vpmv_net_power')}")
