...
🚨 SORUN GİDERİLDİ: Erken kapatma + Restart sorunları çözüldü
"""
...
# 🔧 FIX: Thread ve connection cleanup için
_cleanup_lock = threading.Lock()


def _sync_server_time(client: "Client", retries: int = 3, sleep_s: float = 0.2) -> bool:
    """🔧 FIX: Binance Futures sunucu saatine göre timestamp offset ayarla."""
    import time as _t
    for i in range(retries):
        try:
            ...
            logger.info(f"⏱️ Time sync: offset={offset}ms (try {i+1})")
            _t.sleep(sleep_s)
            return True
        except Exception as e:
            logger.warning(f"⚠️ Time sync attempt {i+1} failed: {e}")
            _t.sleep(sleep_s)
    logger.error("❌ Time sync başarısız - timestamp hataları olabilir")
    return False
...
def handle_websocket_message(msg):
    """🔧 FIX: WebSocket mesaj handler - SL/TP tetiklemelerini yakala + yanlış algılamayı önle"""
    try:
        msg_type = msg.get('e', 'unknown')
        
        # 🔧 FIX: Sadece ORDER_TRADE_UPDATE'e odaklan
        if msg_type == 'ORDER_TRADE_UPDATE':
            ...
            logger.debug(f"📨 WebSocket: {symbol} {order_type} {order_status} (ID:{order_id})")
            
            # 🔧 FIX: Sadece gerçek SL/TP tetiklemelerini işle
            if (order_status == 'FILLED' and 
                order_type in ['STOP_MARKET', 'TAKE_PROFIT_MARKET'] and
                symbol in websocket_symbols and
                symbol in config.live_positions):  # 🔧 FIX: Pozisyon kontrolü ekle
                
                logger.info(f"🎯 GERÇEK SL/TP TETİKLENDİ: {symbol} {order_type}")
                
                # 🔧 FIX: Testnet gecikme kompansasyonu
            ...
            # 🔧 FIX: MARKET emirlerini yanlış algılamayı önle
            elif order_type == 'MARKET' and order_status == 'FILLED':
                logger.debug(f"📝 Market emir tamamlandı: {symbol} (pozisyon açma/kapama)")
...
def setup_websocket_for_top_symbols():
    """🔧 FIX: İlk 50 sembol için WebSocket kurulumu - safer reconnection"""
    global websocket_manager, websocket_symbols
    
    try:
        if not live_bot.is_connected or config.current_data is None or config.current_data.empty:
            return False
        ...
        if top_50_symbols == websocket_symbols and websocket_manager is not None:
            logger.debug("🔄 WebSocket sembolleri değişmedi")
            return True
            
        # 🔧 FIX: Eski WebSocket'i güvenli şekilde durdur
        if websocket_manager:
            try:
                websocket_manager.stop()
                time.sleep(1)  # Cleanup için bekle
            except Exception as e:
                logger.debug(f"WebSocket stop hatası (normal): {e}")
            finally:
                websocket_manager = None
                
        # 🔧 FIX: Yeni WebSocket bağlantısı
        ...
        logger.info(f"✅ WebSocket yenilendi - İlk 50 sembol izleniyor: {len(websocket_symbols)} adet")
        logger.debug(f"🎯 WebSocket sembolleri: {', '.join(list(websocket_symbols)[:10])}...")
...
def stop_websocket():
    """🔧 FIX: WebSocket'i güvenli şekilde durdur"""
    global websocket_manager, websocket_symbols
    ...
    with _cleanup_lock:
        try:
            ...
            logger.info("🛑 WebSocket güvenli şekilde durduruldu")
        except Exception as e:
...
class LiveTradingBot:
    ...
    def _refresh_tradable_cache(self) -> None:
        ...
        try:
            ...
            if cache:
                logger.debug("🧭 Örnek: " + ", ".join(list(cache)[:10]))
        except Exception as e:
            logger.warning(f"⚠️ Tradable sembol keşfi başarısız: {e}")

    def connect_to_binance(self) -> bool:
        """🔧 FIX: Binance API'ye bağlan ve bağlantıyı doğrula."""
        ...
        try:
            ...
            # 🔧 FIX: Timestamp sync - zorunlu
            if not _sync_server_time(self.client):
                logger.warning("⚠️ Timestamp sync başarısız - devam ediliyor")
    ...
    def open_position(self, signal: Dict) -> bool:
        ...
        try:
            ...
            # 🔧 FIX: Emir durumu beklemeli kontrol - daha güvenli
    ...
    def close_position(self, symbol: str, close_reason: str) -> bool:
        """🔧 FIX: Pozisyon kapat + Beklemeli kontrol eklendi - Sadece CLOSED kaydı CSV'ye"""
        try:
    ...
    def check_filled_orders(self) -> None:
        """🔧 FIX: Otomatik SL/TP emirlerinin dolup dolmadığını kontrol et - Sadece REST API coinler için + yanlış algılamayı önle"""
        try:
            current_positions = config.live_positions
            symbols_to_remove = []
            ...
            for symbol, position in list(current_positions.items()):
                ...
                # 🔧 FIX: Order ID varlığını kontrol et
                if not sl_order_id and not tp_order_id:
                    logger.debug(f"⚠️ {symbol} için SL/TP order ID bulunamadı")
                    continue
    ...
    def _handle_auto_close(self, symbol: str, close_reason: str, filled_order: Dict) -> None:
        ...
        try:
            current_positions = config.live_positions
            if symbol not in current_positions:
                logger.warning(f"⚠️ {symbol} pozisyonu config'te bulunamadı")
                return
    ...
    def fill_empty_positions(self) -> None:
        ...
        try:
            logger.debug("🔄 fill_empty_positions başlatıldı")
        
            if not live_trading_active:
                logger.debug("❌ Live trading aktif değil - çıkılıyor")
                return

            current_positions = config.live_positions
            current_position_count = len(current_positions)
            logger.debug(f"📊 Mevcut pozisyon: {current_position_count}/{MAX_OPEN_POSITIONS}")
        
            if current_position_count >= MAX_OPEN_POSITIONS:
                logger.debug("✅ Tüm pozisyon slotları dolu")
                return

            needed_slots = MAX_OPEN_POSITIONS - current_position_count
            logger.debug(f"🎯 Gereken slot sayısı: {needed_slots}")

            if config.current_data is None or config.current_data.empty:
                logger.debug("❌ config.current_data boş - UI'den veri bekleniyor")
                return

            logger.debug(f"✅ UI'den veri alındı: {len(config.current_data)} satır")
            ...
            if exclude_symbols:
                ...
                logger.debug(f"🚫 Açık pozisyonlar hariç: {len(df)}/{before_exclude} sinyal kaldı")

            if df.empty:
                logger.debug("ℹ️ Uygun yeni sembol yok (hepsi açık pozisyonlarda)")
                return

            logger.debug("📊 UI'deki sıralama korunuyor - ilk N sinyali alınıyor")
            ...
            if opened > 0:
                ...
                logger.debug("😔 Hiçbir pozisyon açılamadı")
...
def live_trading_loop() -> None:
    """🔧 FIX: Ana live trading döngüsü - WebSocket + REST hibrit + safer execution"""
    ...
    while live_trading_active:
        try:
            ...
            balance = live_bot.get_account_balance()
            logger.debug(f"💰 Mevcut bakiye: ${balance:.2f}")
            ...
            logger.debug(f"⏱️ Döngü #{loop_count}: {loop_time:.2f}s tamamlandı")
            ...
            if current_positions:
                ...
                logger.debug(f"📊 WebSocket: {ws_count}, REST: {len(current_positions)-ws_count}, Auto SL/TP: {auto_count}")
...
def start_live_trading() -> bool:
    """🔧 FIX: Live trading'i başlat - Cleanup ve restart sorunları çözüldü"""
    global live_trading_active, live_trading_thread

    with _cleanup_lock:
        # 🔧 FIX: Önceki thread'i temizle
...
def stop_live_trading() -> None:
    """🔧 FIX: Live trading'i durdur - Kapsamlı cleanup"""
    global live_trading_active, live_trading_thread
    ...
    with _cleanup_lock:
        ...
        # 🔧 FIX: Thread'i güvenli şekilde durdur
        if live_trading_thread is not None:
            try:
                live_trading_thread.join(timeout=5)
                if live_trading_thread.is_alive():
                    logger.warning("⚠️ Thread 5 saniyede durmadı")
            except Exception as e:
        ...
        # 🔧 FIX: Client bağlantısını temizle
        try:
            if live_bot.client:
...
def get_live_bot_status_for_symbol(symbol: str) -> str:
    """🔧 FIX: App.py callback'i için sembol durumu al - WebSocket/REST ayrımı"""
    try:
        current_positions = config.live_positions
