...
🤖 YENİ: WebSocket + REST hibrit sistem entegrasyonu
🔧 DÜZELTME: WebSocket/REST hesaplama sorunu çözüldü
"""
...
try:
    from trading.live_trader import (
        ...
        websocket_active_symbols  # 🔧 DÜZELTME: Gerçek WebSocket durumu için
    )
    LIVE_TRADER_AVAILABLE = True
except ImportError:
    LIVE_TRADER_AVAILABLE = False
    websocket_active_symbols = set()  # Fallback boş set
...
def get_websocket_rest_counts(df_length: int) -> tuple:
    """🔧 DÜZELTME: Gerçek WebSocket/REST sayılarını hesapla - dinamik kontrol"""
    try:
        # WebSocket gerçekten aktif mi ve sembol var mı?
        if (LIVE_TRADER_AVAILABLE and 
            ...
            # WebSocket yok veya boş - tümü REST
            websocket_count = 0
        ...
        # Fallback: Live pozisyon varsa WebSocket varsay
        try:
...
def control_live_trading(start_clicks, stop_clicks):
    """🔧 DEBUG: Live Trading kontrolü"""
    print(f"🔥 DEBUG: CALLBACK TETİKLENDİ! start={start_clicks}, stop={stop_clicks}")
    logger.info(f"🔥 DEBUG: CALLBACK TETİKLENDİ! start={start_clicks}, stop={stop_clicks}")
    
    if not LIVE_TRADER_AVAILABLE:
        return "❌ Live trader modülü bulunamadı - .env dosyasını kontrol edin"
    
    ctx = dash.callback_context
    print(f"🔥 DEBUG: CONTEXT = {ctx.triggered}")
    ...
    if trigger_id == 'btn-start-live-trading' and start_clicks > 0:
        ...
        print("🔥 DEBUG: DURDURMA KOMUTU - stop_live_trading() çağrılıyor...")
        logger.info("🔥 DEBUG: DURDURMA KOMUTU - stop_live_trading() çağrılıyor...")
        
        try:
            stop_live_trading()  # Bu satır çalışıyor mu?
            print("🔥 DEBUG: stop_live_trading() TAMAMLANDI")
            logger.info("🔥 DEBUG: stop_live_trading() TAMAMLANDI")
            return "🔴 Live Trading Durduruldu - Tüm pozisyonlar kapatıldı"
        except Exception as e:
            print(f"🔥 DEBUG: stop_live_trading() HATASI: {e}")
            logger.error(f"🔥 DEBUG: stop_live_trading() HATASI: {e}")
            return f"❌ Durdurma hatası: {e}"
...
def update_api_connection_status(n_intervals):
    """🔥 YENİ: API bağlantı durumu göstergesi + WebSocket bilgisi - DÜZELTME"""
    if not LIVE_TRADER_AVAILABLE:
        return html.Div([
    ...
    try:
        ...
        if status['api_connected']:
            # 🔧 DÜZELTME: Gerçek WebSocket sayısını al
...
def update_trading_tables(n_intervals):
    """🔥 DÜZELTME: Dictionary iteration problemi çözüldü + WebSocket/REST durumu"""
    positions_data = []
    ...
    try:
        for symbol, position in list(current_positions.items()):
            ...
            # 🔥 YENİ: WebSocket/REST + Otomatik SL/TP durumu göster
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):  
    """🔥 YENİ: Config entegre sinyal güncelleme + DÜZELTME: WebSocket/REST hesaplama"""
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        # 🔥 YENİ: Live bot durumu + WebSocket/REST ayrımı
        live_status = "⭐"
        if LIVE_TRADER_AVAILABLE:
            live_status = get_live_bot_status_for_symbol(row['symbol'])
            
            # 🔧 DÜZELTME: Gerçek WebSocket durumuna göre simge seçimi
    ...
    if LIVE_TRADER_AVAILABLE and is_live_trading_active():
        ...
        # 🔧 DÜZELTME: Gerçek WebSocket/REST sayıları
        try:
...
if __name__ == "__main__":
    ...
    if LIVE_TRADER_AVAILABLE:
        ...
        logger.info("🔧 DÜZELTME: WebSocket/REST sayı hesaplama sorunu çözüldü")
    else:
