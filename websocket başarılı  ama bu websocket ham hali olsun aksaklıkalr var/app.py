...
def get_websocket_rest_counts(df_length: int) -> tuple:
    """🔧 YENİ: Gerçek WebSocket/REST sayılarını hesapla"""
    try:
        if LIVE_TRADER_AVAILABLE and websocket_active_symbols:
            websocket_count = len(websocket_active_symbols)
            rest_count = max(0, df_length - websocket_count)
        else:
            # Live trading aktif değilse tüm coinler REST ile kontrol
            websocket_count = 0
        ...
        # Fallback: Açık pozisyon varsa WebSocket, yoksa REST
