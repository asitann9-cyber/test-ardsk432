...
def update_api_connection_status(n_intervals):
    """🔥 YENİ: API bağlantı durumu göstergesi + WebSocket bilgisi"""
    if not LIVE_TRADER_AVAILABLE:
        return html.Div([
    ...
    try:
        ...
        if status['api_connected']:
            websocket_count = status.get('websocket_symbols', 0)
            return html.Div([
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):  
    """🔥 YENİ: Config entegre sinyal güncelleme (log_volume uyumlu) + WebSocket/REST gösterimi"""
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        if LIVE_TRADER_AVAILABLE:
            ...
            # İlk 50'ye göre simge seçimi
            if i < 50:  # İlk 50 coin
                if "⭐" in live_status:
                    live_status = "⭐🤖"  # WebSocket hazır
                # Açık pozisyon varsa zaten doğru simgeyi dönüyor
            else:  # 51+ coinler
                if "⭐" in live_status:
    ...
    # 🔥 YENİ: WebSocket/REST istatistik paneli
    websocket_count = len([i for i in range(min(50, len(df)))])  # İlk 50
    rest_count = max(0, len(df) - 50)  # 51+
    ...
    if LIVE_TRADER_AVAILABLE and is_live_trading_active():
        ...
        # 🔥 YENİ: WebSocket/REST dağılım bilgisi
        try:
            live_status = get_live_trading_status()
            websocket_symbol_count = live_status.get('websocket_symbols', 0)
            status += f" | 🤖 {current_mode} Bot aktif ({len(current_positions)}/{MAX_OPEN_POSITIONS})"
            status += f" [🤖WS:{websocket_symbol_count}, 📱REST:{max(0, len(df)-websocket_symbol_count)}, SL/TP:{auto_sltp_count}]"
        except:
...
if __name__ == "__main__":
    ...
    if LIVE_TRADER_AVAILABLE:
        ...
        logger.info("🤖 Hibrit sistem: İlk 50 coin WebSocket, geri kalan REST API")
