...
class AnalysisService:
    ...
    def calculate_pullback_levels_pine_script(df: pd.DataFrame, supertrend: pd.Series, trend: pd.Series) -> pd.Series:
        ...
        try:
            ...
            # DEBUGGING: İlk trend değerini kontrol et
            print(f"İlk trend değeri: {trend.iloc[0]}")
            ...
            for i in range(1, len(df)):
                ...
                # DEBUGGING: Trend değişimlerini yazdır
                if current_trend != previous_trend:
                    print(f"Index {i}: Trend değişimi {previous_trend} -> {current_trend}")
                    
                    if current_trend == 1:  # Bullish'e geçiş
                        bullish_pullback = low.iloc[i]  # TREND DEĞİŞİM ANINDA SABİTLE
                        print(f"  Bullish pullback SABİTLENDİ: {bullish_pullback} (Index {i} low)")
                    elif current_trend == -1:  # Bearish'e geçiş
                        bearish_pullback = high.iloc[i]  # TREND DEĞİŞİM ANINDA SABİTLE
                        print(f"  Bearish pullback SABİTLENDİ: {bearish_pullback} (Index {i} high)")
            ...
            # DEBUGGING: Son değerleri yazdır
            print(f"Son pullback değeri: {pullback_level.iloc[-1]}")
            print(f"Son close değeri: {close.iloc[-1]}")
            print(f"Son trend: {trend.iloc[-1]}")
            
            return pullback_level.ffill()
            
        except Exception as e:
            print(f"Pullback hesaplama hatası: {e}")
            return pd.Series(index=df.index if df is not None else [], dtype=float)
        ...
        try:
            ...
            for i in range(len(df)):
                ...
                if pd.isna(pullback) or pullback <= 0:
                    ...
                    # DEBUG: Son değeri yazdır
                    if i == len(df) - 1:
                        print(f"RATIO HESAPLAMA:")
                        print(f"  Trend: {current_trend}")
                        print(f"  Close: {close.iloc[i]}")
                        print(f"  Pullback: {pullback}")
                        print(f"  Ratio: {ratio_percent.iloc[i]}")
            
            return ratio_percent
            
        except Exception as e:
            print(f"Ratio yüzde hesaplama hatası: {e}")
            return pd.Series(index=df.index if df is not None else [], dtype=float)
