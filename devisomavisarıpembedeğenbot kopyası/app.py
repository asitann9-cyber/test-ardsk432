...
def register_routes(app, analysis_service, memory_storage, binance_service, telegram_service):
    ...
    def analyze_supertrend():
        """Supertrend analizi gerçekleştir + Devis'So entegrasyonu"""
        try:
            data = request.get_json()
            ...
            logger.info(f"{len(selected_symbols)} sembol için {timeframe} Supertrend + Devis'So analizi başlatılıyor...")
            ...
            if results:
                ...
                # Ratio >= 100% olan emtiaları kalıcı listeye ekle (Devis'So çizgi değerleri ile)
                ...
                # Kalıcı listeki Devis'So analizi güncelle + Telegram bildirimi  
                ...
                for symbol_data in permanent_symbols:
                    ...
                    # Devis'So çizgi teması varsa Telegram bildirimi gönder
                ...
                return jsonify({
                    ...
                    "message": f"{len(formatted_results)} sembol analiz edildi - Supertrend + Devis'So Trend + Telegram bildirimleri ({deviso_contact_count} çizgi teması)"
                })
            else:
    ...
    def get_permanent_high_ratio():
        """Kalıcı %100+ ratio emtialar listesi - Devis'So Trend + çizgi değerleri ile"""
        try:
            permanent_symbols = memory_storage.get_permanent_symbols()
            ...
            return jsonify({
                ...
                "message": f"Kalıcı listede {len(formatted_permanent)} emtia (%100+ Ratio + Devis'So Trend: {deviso_contact_count} çizgi teması) + Telegram bildirimleri"
            })
    ...
    # =====================================================
    # MANUEL EMTİA EKLEME ENDPOINT'İ - DEVİS'SO TREND İÇİN
    # =====================================================
    @app.route('/api/consecutive/add-to-permanent', methods=['POST'])
    def add_symbol_to_permanent():
        """Manuel olarak emtia kalıcı listeye ekleme - Devis'So Trend ile"""
        try:
            data = request.get_json()
            ...
            logger.info(f"✅ {symbol} manuel olarak kalıcı listeye eklendi (Devis'So Trend)")
            
            return jsonify({
                "success": True,
                "message": f"✅ {symbol} kalıcı listeye eklendi! (Devis'So Trend entegrasyonu + çizgi takibi)",
                "symbol_data": {
                    'symbol': symbol,
    ...
    # =====================================================
    # MANUEL TÜR DEĞİŞTİRME - DEVİS'SO TREND İÇİN UYARLANMIŞ
    # =====================================================
    @app.route('/api/consecutive/update-symbol-type', methods=['POST'])
    def update_symbol_type():
        """Kalıcı listedeki sembolün trend türünü manuel güncelle + Devis'So sıfırlama"""
        try:
            data = request.get_json()
            symbol = data.get('symbol')
            ...
            return jsonify({
                "success": True,
                "message": f"🔒 {symbol} trend türü {old_type} → {new_type} değiştirildi ve KİLİTLENDİ! Devis'So sistemi sıfırlandı!",
                "symbol": symbol,
                "old_type": old_type,
                "new_type": new_type,
                "is_locked": True,
...
def update_permanent_deviso_analysis(memory_storage, analysis_service, telegram_service):
    """
    Kalıcı listeki sembollerin Devis'So analizini güncelle
    """
    current_time = datetime.now().strftime('%H:%M')
    permanent_symbols = memory_storage.get_permanent_symbols()
    ...
    for symbol_data in permanent_symbols:
        try:
            ...
            # Devis'So çizgi teması varsa Telegram bildirimi
            deviso_status = updated_symbol.get('deviso_status', {})
            if deviso_status.get('has_line_contact', False):
                if telegram_service.should_send_deviso_alert(updated_symbol, deviso_status):
                    success = telegram_service.send_deviso_line_contact_alert(updated_symbol)
...
def main():
    """Ana uygulama başlatma fonksiyonu - Devis'So Trend entegrasyonu ile"""
    try:
        print("\n" + "="*70)
        ...
        print("   🎯 DEVİS'SO ÇIZGI TEMAS SİSTEMİ")
        print("="*70)
        ...
        print("="*70)
        print("🎯 DEVİS'SO TREND ÇİZGİ TAKİBİ (YENİ!):")
        ...
        print("="*70)
        print("🔧 GÜNCELLENDİ - DEVİS'SO ENTEGRASYONU:")
        print("   • TERS MOMENTUM sistemi → DEVİS'SO TREND çizgi takibi")
        print("   • analysis_service.py: Devis'So hesaplama eklendi")
        print("   • memory_storage.py: Çizgi değerleri ve temas durumu")
        print("   • telegram_service.py: Çizgi temas bildirimleri")
        print("   • app.py: Devis'So endpoint entegrasyonu")
        ...
        print("   • Sadece LONG trend için bildirim aktif")
        print("="*70)
        ...
        logger.info("Devis'So Trend entegrasyonu ile sistem başlatılıyor")
