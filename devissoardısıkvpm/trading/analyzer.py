...
def analyze_symbol_with_ai(symbol: str, interval: str) -> Dict:
    """
    🔥 YENİ: VPMV bazlı sembol analizi + TIME Alignment
    ...
    try:
        ...
        # 🔥 YENİ: VPMV metriklerini hesapla (symbol parametresi ile TIME alignment dahil)
...
def batch_analyze_with_ai(interval: str) -> pd.DataFrame:
    """
    🔥 YENİ: VPMV bazlı toplu analiz + TIME Alignment
    ...
    logger.info(f"🤖 {len(symbols)} sembol için VPMV + TIME Alignment analiz başlatılıyor...")
            ...
            try:
                ...
                if processed_count % 100 == 0:
                    ...
                    logger.info(
                        ...
                        f"VPMV Başarı: {success_rate:.1f}% ({vpmv_success_count}) - Yüksek TIME: {high_time_alignment_count}"
                    )
    ...
    logger.info("✅ VPMV + TIME Alignment Analiz tamamlandı:")
    ...
    # DataFrame oluştur ve sırala - 🔥 YENİ SIRALAMA
    df = pd.DataFrame(fresh_results)
    df = df.sort_values(
        by=['ai_score', 'vpmv_score', 'time_match_count', 'volatility_component'],
    ...
    if len(df) > 0:
        ...
        for i, row in df.head(5).iterrows():
            logger.debug(
                ...
                f"Trigger={row.get('trigger_type', 'Yok')}"
            )
...
def filter_signals(df: pd.DataFrame, filters: Dict) -> pd.DataFrame:
    """
    🔥 YENİ: VPMV bazlı sinyal filtreleme
...
def get_top_signals(df: pd.DataFrame, count: int = 10) -> pd.DataFrame:
    """
    🔥 YENİ: En iyi VPMV sinyalleri al
...
def analyze_signal_quality(df: pd.DataFrame) -> Dict:
    """
    🔥 YENİ: VPMV bazlı sinyal kalitesi analizi
    ...
    # 🔥 YENİ: TIME alignment kalite analizi
    time_quality = {}
...
def update_signal_scores():
    """
    🔥 YENİ: Kayıtlı sinyallerin skorlarını güncelle (VPMV bazlı yaş cezası)
    """
    global saved_signals
...
# 🔥 YENİ: VPMV spesifik analiz fonksiyonları
