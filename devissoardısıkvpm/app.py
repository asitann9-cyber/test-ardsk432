...
🔥 YENİ: VPMV (Volume-Price-Momentum-Volatility) + TIME Alignment Sistemi
...
from config import (
    ...
    DEFAULT_MIN_TIME_MATCH, LOCAL_TZ, MAX_OPEN_POSITIONS
)
...
current_settings = {
    ...
    'min_vpmv_score': DEFAULT_MIN_VPMV_SCORE,  # 🔥 YENİ
    'min_time_match': DEFAULT_MIN_TIME_MATCH,  # 🔥 YENİ
    'min_ai': DEFAULT_MIN_AI_SCORE * 100
}


def auto_scan_worker():
    """🔥 Otomatik tarama işleyicisi - VPMV sistemi"""
    global auto_scan_active
    
    while auto_scan_active:
        try:
            logger.info("🔄 VPMV otomatik tarama başlatılıyor...")
            # VPMV bazlı analiz
...
def start_auto_scan():
    ...
    if not auto_scan_active:
        ...
        thread.start()
        logger.info(f"🚀 VPMV otomatik tarama başlatıldı")
...
@app.callback(
    ...
    [State('dd-timeframe', 'value'),
     State('inp-min-vpmv', 'value'),  # 🔥 YENİ
     State('inp-min-time', 'value'),  # 🔥 YENİ
     State('inp-min-ai', 'value')]
)
def update_signals(n_intervals, tf, min_vpmv, min_time, min_ai_pct):
    """🔥 YENİ: VPMV + TIME Alignment bazlı sinyal güncelleme"""
    global current_settings
    
    tf = tf or DEFAULT_TIMEFRAME
    min_vpmv = float(min_vpmv or DEFAULT_MIN_VPMV_SCORE)
    min_time = int(min_time or DEFAULT_MIN_TIME_MATCH)
    min_ai_pct = float(min_ai_pct or 30)
    
    current_settings.update({
        'timeframe': tf,
        'min_vpmv_score': min_vpmv,
        'min_time_match': min_time,
        'min_ai': min_ai_pct
    })
    ...
    # 🔥 YENİ: VPMV Filtreleme
    if min_vpmv > 0:
        df = df[abs(df['vpmv_score']) >= min_vpmv]
    
    # 🔥 YENİ: TIME Filtreleme
    if min_time > 0:
        df = df[df['time_match_count'] >= min_time]
    
    # AI skoru filtresi
    if min_ai_pct > 30:
        df = df[df['ai_score'] >= min_ai_pct]
    
    # 🔥 YENİ: Sıralama (VPMV öncelikli)
    df = df.sort_values(
        by=['ai_score', 'vpmv_score', 'time_match_count'],
    ...
    for _, row in df.iterrows():
        ...
        # 🔥 YENİ: TIME gösterimi
        time_match = row.get('time_match_count', 0)
        time_display = f"{time_match}/5"
        ...
        table_data.append({
            ...
            'volume': vol_comp,  # 🔥 YENİ
            'price': price_comp,  # 🔥 YENİ
            'momentum': mom_comp,  # 🔥 YENİ
            'volatility': volatility_comp,  # 🔥 YENİ
            'vpmv_score': vpmv_display,  # 🔥 YENİ
            'time_match': time_display,  # 🔥 YENİ
            'trigger': trigger,  # 🔥 YENİ
    ...
    # 🔥 YENİ: TIME alignment istatistikleri
    ...
    stats_panel = html.Div([
        ...
        # 🔥 YENİ: TIME alignment kartı
        html.Div([
            html.Div(f"{high_time}", className="stat-value", style={'color': '#3b82f6'}),  
            html.Div("Yüksek TIME", className="stat-label")
        ], className="stat-card"),
    ...
    # 🔥 YENİ: TIME bilgisi
    if high_time > 0:
        status += f" | ⏰ Yüksek TIME: {high_time}"
...
if __name__ == "__main__":
    setup_csv_files()
    logger.info("🚀 AI Crypto Analytics - VPMV System + Live Trading başlatılıyor...")
