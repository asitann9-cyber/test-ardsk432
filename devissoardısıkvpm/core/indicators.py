...
🔥 MTF (Multi-Timeframe) desteği: 1H, 2H, 4H
...
def calculate_supertrend(df: pd.DataFrame, atr_period: int = 10, multiplier: float = 3.0) -> Tuple[pd.Series, pd.Series]:
    ...
    Returns:
        Tuple[pd.Series, pd.Series]: (trend, buy_signal, sell_signal)
...
def compute_vpmv_metrics(df: pd.DataFrame, symbol: str = None) -> Dict:
    """
    🔥 YENİ: VPMV metriklerini hesapla (ESKİ compute_consecutive_metrics yerine)
    
    Args:
        df (pd.DataFrame): OHLCV verileri
        symbol (str): Sembol adı (TIME alignment için)
        
    Returns:
        Dict: Tüm VPMV metrikleri
    """
