"""
🤖 AI Model Sistemi - VPMV Tabanlı
🔥 YENİ: VPMV (Volume-Price-Momentum-Volatility) bazlı skorlama
🔥 TIME Alignment entegrasyonu (0-5 uyum sayısı)
...
class CryptoMLModel:
    """🔥 YENİ: VPMV bazlı AI model sistemi"""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("🤖 VPMV AI modeli oluşturuluyor...")
            self._train_initial_model()
    
    def _create_balanced_training_data(self):
        """
        🔥 YENİ: VPMV bazlı dengeli eğitim verisi
        ...
        for i in range(15):
            ...
            time_align = random.randint(4, 5)  # 4-5 uyum
        ...
        for i in range(25):
            ...
            time_align = random.randint(3, 4)  # 3-4 uyum
        ...
        for i in range(35):
            ...
            time_align = random.randint(2, 3)  # 2-3 uyum
        ...
        for i in range(50):
            ...
            time_align = random.randint(0, 2)  # 0-2 uyum
        ...
        logger.info(f"📊 {len(demo_data)} VPMV eğitim verisi oluşturuldu")
    
    def _train_initial_model(self):
        """🔥 YENİ: RandomForestRegressor ile VPMV model eğitimi"""
        try:
            ...
            logger.info(f"🎯 VPMV AI modeli eğitildi: R²={r2_score:.2%}")
            self.save_model()
    ...
    def create_features(self, metrics: Dict) -> np.array:
        """
        🔥 YENİ: VPMV metriklerinden ML features oluştur
        
        Args:
            metrics (Dict): compute_vpmv_metrics() çıktısı
            
        Returns:
            np.array: Normalize edilmiş feature array
        """
        features = {}
        ...
        # 5. TIME Alignment (0-5) → (0, 1)
        time_align = metrics.get('time_match_count', 0)
        features['time_alignment'] = time_align / 5.0
        
        # 6. VPMV Score (-50, +50) → (0, 1)
    ...
    def _calculate_manual_score(self, metrics: Dict) -> float:
        """
        🔥 YENİ: Manuel VPMV skorlama sistemi
        
        Puan Dağılımı (Toplam 100 puan):
        - Volume: 15 puan
        - Price: 35 puan (en yüksek)
        - Momentum: 15 puan
        - Volatility: 15 puan
        - TIME Alignment: 20 puan
        """
        score = 0.0
        
        # =====================================
        # 1. VOLUME BİLEŞENİ (15 puan)
        # =====================================
        vol_comp = abs(metrics.get('volume_component', 0.0))
        if vol_comp >= 40.0:
            score += 15
        elif vol_comp >= 25.0:
            score += 10 + (vol_comp - 25.0) / 15.0 * 5
        elif vol_comp >= 15.0:
            score += 5 + (vol_comp - 15.0) / 10.0 * 5
        else:
            score += min(vol_comp / 15.0 * 5, 5)
        
        # =====================================
        # 2. PRICE BİLEŞENİ (35 puan) ⭐
        # =====================================
        price_comp = abs(metrics.get('price_component', 0.0))
        if price_comp >= 40.0:
            score += 35
        elif price_comp >= 25.0:
            score += 25 + (price_comp - 25.0) / 15.0 * 10
        elif price_comp >= 15.0:
            score += 15 + (price_comp - 15.0) / 10.0 * 10
        else:
            score += min(price_comp / 15.0 * 15, 15)
        
        # =====================================
        # 3. MOMENTUM BİLEŞENİ (15 puan)
        # =====================================
        mom_comp = abs(metrics.get('momentum_component', 0.0))
        if mom_comp >= 40.0:
            score += 15
        elif mom_comp >= 25.0:
            score += 10 + (mom_comp - 25.0) / 15.0 * 5
        elif mom_comp >= 15.0:
            score += 5 + (mom_comp - 15.0) / 10.0 * 5
        else:
            score += min(mom_comp / 15.0 * 5, 5)
        
        # =====================================
        # 4. VOLATILITY BİLEŞENİ (15 puan)
        # =====================================
        vol_comp_val = abs(metrics.get('volatility_component', 0.0))
        if vol_comp_val >= 30.0:
            score += 15
        elif vol_comp_val >= 20.0:
            score += 10 + (vol_comp_val - 20.0) / 10.0 * 5
        elif vol_comp_val >= 10.0:
            score += 5 + (vol_comp_val - 10.0) / 10.0 * 5
        else:
            score += min(vol_comp_val / 10.0 * 5, 5)
        
        # =====================================
        # 5. TIME ALIGNMENT (20 puan)
        # =====================================
        time_match = metrics.get('time_match_count', 0)
        if time_match == 5:
            score += 20  # Mükemmel uyum
        elif time_match == 4:
            score += 16
        elif time_match == 3:
            score += 12
    ...
    def save_model(self):
        ...
        try:
            ...
            logger.info(f"💾 VPMV AI modeli kaydedildi: {AI_MODEL_FILE}")
        except Exception as e:
    ...
    def load_model(self):
        ...
        try:
            ...
            logger.info(f"✅ VPMV AI modeli yüklendi: R²={self.model_stats['accuracy']:.2%}")
        except:
    ...
    def get_model_info(self) -> Dict:
        ...
        return {
            ...
            'model_type': 'VPMV-Based RandomForest Regressor + TIME Alignment'
        }
