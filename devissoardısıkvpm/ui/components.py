...
🔥 YENİ: Z-Score sadece artı/eksi değer gösterimi - Pine Script gibi
...
def create_zscore_indicator(zscore):
    """Sadece artı/eksi değer göster - Pine Script gibi"""
    try:
        zscore = float(zscore)
        return f"{zscore:+.2f}"  # +1.25 veya -2.10 gibi
    except:
        return "N/A"
...
def create_signals_table():
    """🔥 Sinyaller tablosunu oluştur - Z-Score + VPM eklendi"""
    return html.Div([
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                {"name": "VPM", "id": "vpm_score"},  # 🔥 YENİ SÜTUN
                {"name": "C-Signal", "id": "c_signal_momentum", 'type': 'numeric', 'format': {"specifier": ".2f"}},
                {"name": "Z-Score", "id": "max_zscore"},  # 🔥 SADECE SAYI GÖSTERİMİ
            ...
            style_data_conditional=[
                ...
                # 🔥 YENİ: VPM renklendirme (String değerler için)
                {
                    'if': {'column_id': 'vpm_score', 'filter_query': '{vpm_score} contains +'},
                    'backgroundColor': '#10b981',  # Yeşil - Pozitif VPM
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'vpm_score', 'filter_query': '{vpm_score} contains -'},
                    'backgroundColor': '#dc2626',  # Kırmızı - Negatif VPM
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                ...
                # 🔥 Z-Score renklendirme - SADECE SAYISAL DEĞERLER İÇİN
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} > +2.00'},
                    'backgroundColor': '#dc2626',  # Kırmızı - Pozitif yüksek
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'max_zscore', 'filter_query': '{max_zscore} > +1.00'},
...
def create_layout():
    """🔥 Ana layout'u oluştur - Live trading + VPM güncellemeleri"""
    return html.Div([
        # Header
        create_header(),
