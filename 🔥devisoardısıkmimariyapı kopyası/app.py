...
✅ Live Trading callback'leri eklendi
...
# 🔥 DÜZELTME: Live Trading import'u güvenli hale getir
...
try:
    ...
    print(f"⚠️ Live trader modülü bulunamadı: {e}")
    print("💡 Çözüm: pip install python-binance python-dotenv")
...
# 🔥 DÜZELTME: Live Trading callback'leri eklendi
@app.callback(
...
def control_live_trading(start_clicks, stop_clicks):
    ...
    if not LIVE_TRADER_AVAILABLE:
        return html.Div([
            "❌ Live trader modülü bulunamadı",
            html.Br(),
            html.Small("💡 Çözüm: pip install python-binance python-dotenv", 
                      style={'color': '#fbbf24'})
        ])
    ...
    if not ctx.triggered:
        try:
            ...
            return f"⚠️ Live Trading durumu kontrol edilemiyor: {e}"
    ...
    try:
        ...
        return f"❌ Live Trading kontrol hatası: {e}"
...
def update_api_status(n_intervals):
    ...
    if not LIVE_TRADER_AVAILABLE:
        return html.Div([
            "❌ Live Trader Yok",
            html.Br(),
            html.Small("pip install python-binance python-dotenv", style={'color': '#fbbf24'})
        ], style={'color': '#ef4444'})
    ...
    try:
        ...
        return html.Div(f"⚠️ API Durumu: {e}", style={'color': '#f59e0b'})


# 🔥 DÜZELTME: API connection indicator callback eklendi
@app.callback(
...
def update_api_connection_status(n_intervals):
    ...
    if not LIVE_TRADER_AVAILABLE:
        return html.Div([
            html.Span("❌ Modül Yok", style={'color': '#ef4444', 'fontSize': '18px'}),
            html.Br(),
            html.Small("python-binance ve python-dotenv gerekli", style={'color': '#fbbf24'})
        ])
    ...
    try:
        ...
        if status['api_connected']:
            return html.Div([
                html.Span("✅ Bağlı", style={'color': '#22c55e', 'fontSize': '18px'}),
                html.Br(),
                html.Small(f"{status['environment'].upper()} | ${status['balance']:.2f}", 
                          style={'color': '#cccccc'})
            ])
        else:
            return html.Div([
                html.Span("❌ Bağlantı Yok", style={'color': '#ef4444', 'fontSize': '18px'}),
                html.Br(),
                html.Small("API anahtarlarını kontrol edin", style={'color': '#fbbf24'})
            ])
    except Exception as e:
        return html.Div([
            html.Span("⚠️ Hata", style={'color': '#f59e0b', 'fontSize': '18px'}),
            html.Br(),
            html.Small(str(e)[:50], style={'color': '#cccccc'})
        ])
...
if __name__ == "__main__":
    ...
    if LIVE_TRADER_AVAILABLE:
        ...
        logger.warning("⚠️ Live trading modülü bulunamadı")
        logger.warning("💡 Çözüm: pip install python-binance python-dotenv")
