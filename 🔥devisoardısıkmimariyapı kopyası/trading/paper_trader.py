...
def fill_empty_positions():
    ...
    try:
        ...
        # Trading için minimum AI skoru yükseltildi
        if min_ai_pct < 90:
            min_ai_pct = 90.0
        ...
        if df.empty:
            logger.warning("⚠️ Filtre sonrası uygun sinyal yok - seçim yapılmadı")
            return
        ...
        # Sırala (AI, hareket, Gauss, hacim)
        df = df.sort_values(by=['ai_score', 'run_perc', 'gauss_run', 'vol_ratio'],
                            ascending=[False, False, False, False])

        # --- En iyi 3 kuralı ---
        top_n = min(3, needed_positions)
        top_signals = df.head(top_n)

        logger.info(f"🎯 TÜM SİNYALLER (AI≥{min_ai_pct:.0f}) → seçilen {len(top_signals)} sinyal (en fazla 3):")
        for _, s in top_signals.iterrows():
            logger.info(f"   🚀 {s['symbol']}: AI={s['ai_score']:.0f}%, run={s['run_count']}, move={s['run_perc']:.2f}%")
        ...
        logger.info(f"🏆 SONUÇ: {opened_count} yeni pozisyon | Toplam: {len(open_positions)}/{MAX_OPEN_POSITIONS}")
...
def start_paper_trading() -> bool:
    ...
    logger.info(f"📊 Maksimum pozisyon: {MAX_OPEN_POSITIONS} (TÜM SİNYALLER tablosundan EN İYİLER)")
