...
class LiveTradingBot:
    ...
    def connect_to_binance(self) -> bool:
        """🔑 DÜZELTME: Logic hatası düzeltildi"""
        global binance_client

        if not BINANCE_AVAILABLE:
            logger.error("❌ python-binance kütüphanesi yüklü değil")
            print("❌ python-binance kütüphanesi yüklü değil")
            return False

        # 🔥 KRITIK DÜZELTME: Logic operator hatası düzeltildi
        if not BINANCE_API_KEY or not BINANCE_SECRET_KEY:  # ✅ Her iki not operatörü
            logger.error("❌ API anahtarları config.py'de bulunamadı")
            print("❌ API anahtarları config.py'de bulunamadı")
            print(f"🔍 Debug - API Key var: {BINANCE_API_KEY is not None}")
            print(f"🔍 Debug - Secret Key var: {BINANCE_SECRET_KEY is not None}")
            return False

        # ✅ API anahtarları bulundu
        print("✅ API anahtarları config.py'den başarıyla alındı")
        logger.info("✅ API anahtarları config.py'den başarıyla alındı")
        ...
        try:
            ...
            if ENVIRONMENT == "testnet":
                ...
                print(f"🧪 Testnet bağlantısı kuruldu: {BASE}")
            else:
                self.client = Client(
                    api_key=BINANCE_API_KEY,
                    api_secret=BINANCE_SECRET_KEY,
                ...
                print(f"🚀 Mainnet bağlantısı kuruldu: {BASE}")
            ...
            print("✅ Binance API ping başarılı")
            ...
            print(f"✅ Hesap bilgisi alındı - Bakiye: ${self.account_balance:.2f}")
            ...
            print("✅ Live Trading Bot başarıyla bağlandı!")
            return True
