...
class LiveTradingBot:
    ...
    def fill_empty_positions(self) -> None:
        """🎯 Boş pozisyon slotlarını doldur - 🚨 SORUN GİDERİLDİ: En yüksek AI skorlu sinyaller seçiliyor"""
        try:
            ...
            # 🚨 SORUN GİDERİLDİ: Sınyal kopyası oluştur ve normalize et
            df = config.current_data.copy()
            logger.info(f"📋 DataFrame kopyası oluşturuldu: {len(df)} satır")

            # 🚨 SORUN GİDERİLDİ: AI skor normalizasyonu - tutarlı format
            if "ai_score" in df.columns:
                ai_max = df["ai_score"].max()
                ai_min = df["ai_score"].min()
                logger.info(f"🎯 AI skor aralığı: {ai_min:.2f} - {ai_max:.2f}")
                
                if ai_max <= 1.0:
                    # 0-1 aralığındaysa 0-100'e çevir
                    df["ai_score"] = (df["ai_score"] * 100.0).clip(0, 100)
                    logger.info("🔄 AI skorları 0-100 aralığına çevrildi")
                
                # Güncel aralığı tekrar logla
                new_max = df["ai_score"].max()
                new_min = df["ai_score"].min()
                logger.info(f"📊 Normalize edilmiş AI skor aralığı: {new_min:.2f} - {new_max:.2f}")
            ...
            # Live trading için minimum AI skorunu biraz yükselt
            live_min_ai = max(min_ai, 25.0)  # Minimum %25
            
            logger.info(f"🎯 Live Trading filtreleri:")
            logger.info(f"   AI Skoru ≥ {live_min_ai:.0f}%")
            logger.info(f"   Run Count ≥ {min_streak}")
            logger.info(f"   Run Percentage ≥ {min_move}%")
            logger.info(f"   Volume Ratio ≥ {min_volr}")

            # 🚨 SORUN GİDERİLDİ: Adım adım filtreleme ve her adımda log
            original_count = len(df)
            logger.info(f"🔢 Filtreleme öncesi: {original_count} sinyal")

            # AI skor filtresi
            df = df[df["ai_score"] >= live_min_ai]
            logger.info(f"🎯 AI skor filtresi sonrası: {len(df)}/{original_count} sinyal kaldı")

            # Run count filtresi
            df = df[df["run_count"] >= min_streak]
            logger.info(f"📈 Run count filtresi sonrası: {len(df)}/{original_count} sinyal kaldı")

            # Run percentage filtresi
            df = df[df["run_perc"] >= min_move]
            logger.info(f"📊 Run percentage filtresi sonrası: {len(df)}/{original_count} sinyal kaldı")

            # Volume ratio filtresi
            if "vol_ratio" in df.columns:
                vol_before = len(df)
                df = df[df["vol_ratio"].fillna(0) >= min_volr]
                logger.info(f"🔊 Volume ratio filtresi sonrası: {len(df)}/{vol_before} sinyal kaldı")
            ...
            if exclude:
                ...
                logger.info(f"🔒 Hariç tutulan pozisyonlar: {list(exclude)}")

            # 🚨 SORUN GİDERİLDİ: Tradable cache kontrolü - ama sadece bilgi amaçlı
            tradable_before = len(df)
            if self.tradable_cache:
                df_tradable = df[df["symbol"].isin(self.tradable_cache)]
                logger.info(f"🧭 Tradable cache kontrolü: {len(df_tradable)}/{tradable_before} sembol tradable")
                
                # Tradable olmayan sinyalleri logla
                non_tradable = df[~df["symbol"].isin(self.tradable_cache)]
                if len(non_tradable) > 0:
                    logger.warning(f"⚠️ Tradable olmayan yüksek skorlu sinyaller:")
                    for _, row in non_tradable.head(5).iterrows():
                        logger.warning(f"   {row['symbol']}: AI={row['ai_score']:.0f}%")
                
                df = df_tradable
            else:
                logger.warning("⚠️ Tradable cache boş - tüm sinyaller değerlendiriliyor")

            if df.empty:
                logger.warning(f"🔍 Filtrelemeler sonrası hiç uygun sinyal kalmadı!")
                logger.warning(f"💡 Öneriler:")
                logger.warning(f"   - AI skor limitini düşürün (şu an: {live_min_ai:.0f}%)")
                logger.warning(f"   - Diğer filtreleri gevşetin")
                logger.warning(f"   - Otomatik tarama çalıştığından emin olun")
                return

            # 🚨 GÜNCELLEME: UI, Analyzer ve Paper Trader ile aynı sıralama
            logger.info("🔄 Sinyaller sıralanıyor (AI + TrendStrength + RunPerc + GaussRun + VolRatio)...")
            logger.info(f"📋 Trader filtre sonrası ilk 5: {df.head(5)['symbol'].tolist()}")
            ...
            logger.info("✅ Sıralama tamamlandı (referans kriterlerle)")
            # 🔍 Trader log: seçilen ilk 3
            logger.info(f"📊 Trader’ın kullandığı ilk 3: {df.head(3)['symbol'].tolist()}")
            


            # 🚨 SORUN GİDERİLDİ: En iyi N sinyali seç
            top_n = min(needed_slots, len(df))
            top_signals = df.head(top_n)
            
            logger.info(f"🏆 EN YÜKSEK SKORLU {top_n} SİNYAL SEÇİLDİ:")
            for i, (_, signal) in enumerate(top_signals.iterrows(), 1):
                logger.info(f"   🥇 #{i}: {signal['symbol']} | AI={signal['ai_score']:.0f}% | Deviso={signal.get('deviso_ratio', 0):.2f} | Run={signal['run_count']}×{signal['run_perc']:.2f}%")

            # 🚨 SORUN GİDERİLDİ: Tradable kontrolü ve pozisyon açma
            opened = 0
            for idx, (_, signal) in enumerate(top_signals.iterrows(), 1):
                ...
                logger.info(f"🔍 #{idx}: {symbol} tradable kontrolü yapılıyor...")
                ...
                logger.info(f"✅ {symbol} tradable onaylandı - pozisyon açılıyor...")
                
                # Pozisyon aç
                success = self.open_position(signal.to_dict())
                if success:
                    opened += 1
                    logger.info(f"🚀 {symbol} pozisyonu başarıyla açıldı! (AI: {ai_score:.0f}%)")
            ...
            # Sonuç özeti
            if opened > 0:
                logger.info(f"🎊 BAŞARIYLA TAMAMLANDI: {opened} yeni live pozisyon açıldı!")
                logger.info(f"📊 Yeni durum: {len(current_positions) + opened}/{MAX_OPEN_POSITIONS} pozisyon")
            else:
                logger.warning(f"😔 Hiçbir pozisyon açılamadı!")
                logger.warning(f"🔍 Sorun giderme önerileri:")
                logger.warning(f"   - Binance API bağlantısını kontrol edin")
                logger.warning(f"   - Bakiye yeterliliğini kontrol edin")
                logger.warning(f"   - Tradable sembol listesini güncelleyin")
