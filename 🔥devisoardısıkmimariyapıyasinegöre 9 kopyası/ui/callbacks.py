...
def register_callbacks(app, auto_scan_control):
    ...
    def update_trading_tables(n_intervals):
        ...
        logger.debug(f"📊 UI Pozisyon Tablosu: {len(positions_data)} satır, Trade Tablosu: {len(trades_data)} satır")
        for i, row in enumerate(positions_data[:5]):
            logger.debug(f"   Poz {i}: {row['symbol']} | Side={row['side']} | AI={row['ai_score']}")

        for i, row in enumerate(trades_data[:5]):
            logger.debug(
                f"   Trade {i}: {row['timestamp']} | {row['symbol']} | Side={row['side']} | "
                f"Entry={row['entry_price']} | Exit={row['exit_price']} | PnL={row['pnl']}"
            )
    ...
    def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct):
        ...
        # 🚨 Trader ile uyumlu olsun diye aynı filtreleri uygula
        df = df[df['ai_score'] >= 30]         # AI Skoru ≥ 30
        df = df[df['run_count'] >= 3]         # Run Count ≥ 3
        df = df[df['run_perc'] >= 0.5]        # Run % ≥ 0.5
        df = df[df['vol_ratio'].fillna(0) >= 1.5]  # Volume Ratio ≥ 1.5

        logger.info(f"📋 UI filtre sonrası ilk 5: {df.head(5)['symbol'].tolist()}")

        df = df.sort_values(
            by=['ai_score', 'trend_strength', 'run_perc', 'gauss_run', 'vol_ratio'], 
            ascending=[False, False, False, False, False]
        )
        # 🔍 UI log: gösterilen ilk 3
        ...
        logger.debug("📊 UI’ye gönderilen tablo ilk 5 satır:")
        for i, row in df.head(5).iterrows():
           logger.debug(f"   {i}: {row['symbol']} | AI={row['ai_score']} | run={row['run_count']} | move={row['run_perc']}")
