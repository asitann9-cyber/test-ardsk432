...
def handle_kline_closed(symbol, close_price, is_kline_closed):
    """WebSocket'ten gelen kapanış verisiyle kalıcı liste güncellemesi"""
    try:
        if not is_kline_closed:
            return
        
        logger.info(f"🟦 Yeni kapanış geldi: {symbol} | close={close_price}")
        ...
        if permanent_symbol:
            ...
            logger.info(f"🔄 {symbol} WebSocket ile güncellendi")

            # 🔥 PANEL ANLIK GÜNCELLEME
            WebSocketManager.broadcast({
                "event": "permanent_update",
                "data": memory_storage.get_permanent_symbols()
            })
...
def main():
    ...
    try:
        ...
        if persistent_symbols:
            logger.info(f"🔌 WebSocket başlatılıyor → {len(persistent_symbols)} sembol izleniyor")
            ...
            logger.info("⚠️ Kalıcı listede sembol yok → WebSocket başlatılmadı")
