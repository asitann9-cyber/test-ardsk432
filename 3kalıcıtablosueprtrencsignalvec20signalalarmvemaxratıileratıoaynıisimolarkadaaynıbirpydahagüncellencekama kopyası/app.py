...
def register_routes(app, analysis_service, memory_storage, binance_service, telegram_service):
    ...
    def get_permanent_high_ratio():
        """✅ Kalıcı %100+ ratio emtialar listesi - GÜNCEL RATIO + VPMV GÖSTERİMİ"""
        try:
            permanent_symbols = memory_storage.get_permanent_symbols()
            
            # Her kalıcı sembol için ek bilgiler ekle (VPMV dahil)
            formatted_permanent = []
            ...
            return jsonify({
                ...
                "message": f"Kalıcı listede {len(formatted_permanent)} emtia ({active_c_signal_count} aktif C-Signal ±{Config.C_SIGNAL_ALERT_THRESHOLD})"
            })
    ...
    def add_symbol_to_permanent():
        """Manuel olarak emtia kalıcı listeye ekleme (VPMV dahil)"""
        try:
            data = request.get_json()
            ...
            logger.info(f"✅ {symbol} manuel olarak kalıcı listeye eklendi (VPMV: {manual_symbol_data['vpmv_net_power']})")
...
def main():
    ...
    try:
        print("\n" + "="*70)
        print("🎯 Supertrend + C-Signal + VPMV Analiz Sistemi")
        print("="*70)
        print("📊 ÖZELLİKLER:")
        ...
        print("   🆕 VPMV (Volume-Price-Momentum-Volatility) NET POWER Sistemi")
        ...
        print("   ✅ Kalıcı Tabloda Güncel Ratio + VPMV Gösterimi")
        print("="*70)
        ...
        print("="*70)
        print("🎯 VPMV NET POWER SİSTEMİ:")
        print("   • 6 Bileşenli Analiz: Price, Momentum, Acceleration,")
        print("     Volume Pressure, Volatility, Time Analysis")
        print("   • SuperTrend Reset Mekanizması")
        print("   • -100 ile +100 arası NET POWER değeri")
        print("   • STRONG LONG (>10) | LONG (>0) | SHORT (<0) | STRONG SHORT (<-10)")
        print("   • Kalıcı tabloda görüntüleme")
        print("   • API endpoint'leri: /api/vpmv/statistics, /api/vpmv/filter/<signal>")
        print("="*70)
        ...
        print("   • VPMV NET POWER hesaplama ve gösterim")
        print("   • Manuel tür değiştirme ve kilitleme")
        print("   • Her iki tabloda da güncel ratio + VPMV gösterimi")
        print("="*70)
        print("🔔 DİNAMİK C-SIGNAL ±X SİSTEMİ:")
        print("   • ⚙️ Panel'den threshold ayarlanabilir! (Varsayılan: ±20)")
        print("   • C-Signal >= +X = LONG (L) sinyali")
        print("   • C-Signal <= -X = SHORT (S) sinyali")
        print("   • Örnek: ±5, ±10, ±15, ±20, ±25, ±30 vb.")
