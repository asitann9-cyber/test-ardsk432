...
class MemoryStorage:
    ...
    # =====================================================
    # PERMANENT HIGH RATIO MANAGEMENT - ✅ VPMV ENTEGRASYONU
    # =====================================================
    
    def add_permanent_symbol(self, symbol_data: Dict[str, Any]) -> None:
        """Kalıcı listeye sembol ekle (VPMV dahil)"""
        ...
        if not existing_symbol:
            ...
            logger.info(f"🎯 {symbol} kalıcı listeye eklendi ({permanent_entry['add_reason']}) - VPMV: {symbol_data.get('vpmv_net_power', 0)}")
        else:
            # ✅ MEVCUT SEMBOL - Güncel değerleri güncelle (VPMV dahil)
            ...
            logger.info(f"🔄 {symbol} güncel değerlerle güncellendi: Ratio {ratio_percent:.2f}%, VPMV: {symbol_data.get('vpmv_net_power', 0)}")
    ...
    def get_permanent_symbol(self, symbol: str) -> Optional[Dict[str, Any]]:
        """Belirli kalıcı sembolü getir (VPMV alanları dahil)"""
    ...
    def update_permanent_symbol(self, symbol: str, update_data: Dict[str, Any]) -> bool:
        """Kalıcı sembol verisini güncelle (VPMV dahil)"""
        existing_symbol = self.get_permanent_symbol(symbol)
        if existing_symbol:
            existing_symbol.update(update_data)
            
            # 🆕 VPMV güncellemesi varsa zamanı işaretle
            if 'vpmv_net_power' in update_data or 'vpmv_signal' in update_data:
                existing_symbol['vpmv_update_time'] = datetime.now().strftime('%H:%M')
    ...
    def get_vpmv_statistics(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: VPMV özet istatistikleri
        """
    ...
    def get_top_vpmv_symbols(self, limit: int = 10, sort_by: str = 'highest') -> List[Dict[str, Any]]:
        ...
        Args:
            limit (int): Kaç sembol döndürülecek
            sort_by (str): 'highest' veya 'lowest'
            
        Returns:
            List[Dict[str, Any]]: Sıralanmış sembol listesi
        """
    ...
    def _check_c_signal_threshold(self, symbol: str, current_value: float, 
        ...
        """
        🆕 DİNAMİK C-Signal ±X threshold kontrolü - Config'den threshold alır
        ...
        """
        # 🆕 Config'den dinamik threshold'u al
    ...
    def get_supertrend_statistics(self) -> Dict[str, Any]:
        """Supertrend + VPMV sistemi istatistikleri"""
        if not self.permanent_high_ratio:
            return {
                'total_symbols': 0,
                'bullish_count': 0,
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        """Sistem istatistiklerini getir (VPMV dahil)"""
        base_stats = {
            **self.system_stats,
        ...
        # Supertrend + VPMV istatistikleri ekle
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        """Veri bütünlüğünü kontrol et (VPMV dahil)"""
        issues = []
    ...
    def export_data(self) -> Dict[str, Any]:
        """Tüm veriyi export et (VPMV dahil)"""
        return {
    ...
    def import_data(self, data: Dict[str, Any]) -> bool:
        """Veriyi import et - Backward compatibility (VPMV alanları dahil)"""
        try:
            ...
            logger.info("✅ Veri başarıyla import edildi (VPMV alanları dahil)")
            return True
