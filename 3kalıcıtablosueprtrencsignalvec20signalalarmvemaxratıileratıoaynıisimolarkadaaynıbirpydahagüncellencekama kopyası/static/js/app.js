...
connectWebSocket() {
    ...
    ws.onmessage = (msg) => {
        try {
            ...
            if (data.event === "permanent_update") {
    ...
    ws.onclose = () => {
        console.log("🔴 WS kapandı, 3 saniye sonra yeniden bağlanıyor...");
  ...
  // =====================================================
  // 🆕 PERMANENT LIST FUNCTIONS - VPMV RENDER EKLENDİ
  // =====================================================
  ...
  /**
   * 🆕 Kalıcı tabloyu güncelle - VPMV SÜTUNLARI DAHİL
   */
  updatePermanentTable(permanentSymbols) {
    ...
    if (permanentSymbols.length === 0) {
      tbody.innerHTML = '<tr><td colspan="10" style="padding: 20px; text-align: center; color: var(--text-muted);">🏆 Henüz kalıcı listede emtia yok</td></tr>';
      return;
    }
  ...
  async startSupertrendAnalysis(isAutoUpdate = false) {
    ...
    try {
      if (!isAutoUpdate) {
        this.showStatus('analysis-status', `🎯 ${this.selectedSymbols.length} emtia için ${timeframe} Supertrend+VPMV analizi başlatılıyor...`, 'info', true);
      } else {
        this.updateCounter++;
      ...
      const data = await response.json();
      if (data.success) {
        console.log(`📊 API'den ${data.results.length} sonuç alındı (VPMV dahil)`);
        ...
        if (!isAutoUpdate) {
          ...
          this.showStatus('analysis-status', 
            `✅ Analiz başlatıldı (VPMV dahil) - ${data.high_priority_count || 0} emtia kalıcı listeye eklendi${cSignalInfo} - Normal: 2dk, Kalıcı: 30sn güncellenecek (${now})`, 
            'success');
        } else {
          const now = new Date().toLocaleTimeString('tr-TR');
      ...
      console.error('Supertrend+VPMV analiz hatası:', error);
      if (!isAutoUpdate) {
  ...
  async init() {
    console.log('🎯 Supertrend + C-Signal + VPMV System initializing...');
    ...
    try {
      ...
      console.log('✅ Supertrend + C-Signal + VPMV System initialized');
