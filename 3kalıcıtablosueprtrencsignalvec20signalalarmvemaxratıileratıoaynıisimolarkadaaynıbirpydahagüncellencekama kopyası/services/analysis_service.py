...
class AnalysisService:
    ...
    def calculate_vpmv_net_power(df: pd.DataFrame, trend: pd.Series, reset_signals: np.ndarray) -> Dict[str, Any]:
        """
        🎯 Pine Script VPMV - 4 Bileşenli Sistem
        ...
        try:
            ...
            atr = np.mean(atr_values[-10:]) if len(atr_values) >= 10 else np.mean(atr_values) if atr_values else 0.01

            # Reset sonrası kümülatif hesaplamalar
    ...
    def analyze_single_symbol(symbol: str, timeframe: str = '4h') -> Optional[Dict[str, Any]]:
        ...
        🆕 Pine Script uyumlu 4 bileşenli VPMV sistemi
        ...
        Returns:
            Optional[Dict[str, Any]]: Analiz sonuçları (VPMV 4 bileşen dahil)
        """
        try:
            ...
            # 🆕 VPMV NET POWER hesapla (4 bileşenli - Pine Script)
