...
class AnalysisService:
    ...
    def calculate_vpmv_net_power(df: pd.DataFrame, trend: pd.Series, reset_signals: np.ndarray) -> Dict[str, Any]:
        ...
        3. Reset bar'ında hesaplama yapılmıyor (not just_reset)
        4. Tetikleyici sistemi eklendi (Pine Script mantığı)
        ...
        try:
            ...
            signal_price = None
            avg_volume = None  # ✅ DÜZELTME: Reset anında hesaplanacak
            ...
            for i in range(len(reset_signals) - 1, -1, -1):
                if reset_signals[i]:
                    ...
                    # ✅ DÜZELTME: Reset anında avg_volume hesapla (Pine: ta.sma(vol, 20))
            ...
            # ✅ DÜZELTME: Wilder's Smoothing ATR (Pine Script mantığı)
            atr_period = 10
            atr_wilder = None
            ...
            # ✅ DÜZELTME: Reset bar'ından sonra hesaplama (Pine: not just_reset)
            ...
            # ✅ YENİ: TETİKLEYİCİ SİSTEMİ (Pine Script mantığı)
            trigger_name = "Yok"
            trigger_active = False
            ...
            # Tetikleyici belirleme (Pine Script mantığı)
            if price_trig:
                trigger_active = True
                max_val = 0.0
