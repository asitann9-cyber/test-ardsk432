...
🆕 YENİ: VPMV (Volume-Price-Momentum-Volatility) NET POWER sistemi eklendi
...
class AnalysisService:
    ...
    @staticmethod
    def calculate_vpmv_net_power(df: pd.DataFrame, trend: pd.Series, reset_signals: np.ndarray) -> float:
        """
        🎯 VPMV NET POWER hesaplama - Pine Script'in birebir Python karşılığı
        
        6 Bileşen:
        1. Price Change (Fiyat Değişimi)
        2. Momentum (Fiyat × Hacim)
        3. Acceleration (ROC 14)
        4. Volume Pressure (Hacim Baskısı)
        5. Volatility (Yukarı/Aşağı Volatilite)
        6. Time Analysis (Zaman Analizi)
        
        Args:
            df (pd.DataFrame): OHLCV verileri
            trend (pd.Series): SuperTrend trend değerleri
            reset_signals (np.ndarray): Reset sinyalleri
            
        Returns:
            float: NET POWER değeri (-100 ile +100 arası)
        """
        try:
            ...
            # Bar-by-bar hesaplama (Pine Script mantığı)
            for i in range(1, len(close)):
                ...
                price_vol_momentum = delta_x * volume[i]
                if price_vol_momentum > 0:
                    pos_mom += price_vol_momentum
                elif price_vol_momentum < 0:
                    neg_mom += abs(price_vol_momentum)
                
                # 3. ACCELERATION (ROC 14)
                if i >= 14:
                    roc_value = ((close[i] - close[i-14]) / close[i-14]) * 100
                    if roc_value > 0:
                        pos_acc += roc_value
                    elif roc_value < 0:
                        neg_acc += abs(roc_value)
                
                # 4. VOLUME PRESSURE
                volume_pressure = volume[i] if delta_x > 0 else (-volume[i] if delta_x < 0 else 0)
                if volume_pressure > 0:
                    pos_vp += volume_pressure
                elif volume_pressure < 0:
                    neg_vp += abs(volume_pressure)
                ...
                volatility_up = tr if close[i] > close[i-1] else 0
                volatility_down = tr if close[i] < close[i-1] else 0
                
                cum_vol_up += volatility_up
                cum_vol_down += volatility_down
                
                # 6. TIME ANALYSIS
                range_tf = high[i] - low[i]
                momentum_tf = close[i] - close[i-1]
            ...
            # Yüzde hesaplamaları
            total_x = pos_x + neg_x
            pos_x_percent = (pos_x / total_x * 100) if total_x > 0 else 0
            neg_x_percent = (neg_x / total_x * 100) if total_x > 0 else 0
            
            total_mom = pos_mom + neg_mom
            pos_mom_percent = (pos_mom / total_mom * 100) if total_mom > 0 else 0
            neg_mom_percent = (neg_mom / total_mom * 100) if total_mom > 0 else 0
            
            total_acc = pos_acc + neg_acc
            pos_acc_percent = (pos_acc / total_acc * 100) if total_acc > 0 else 0
            neg_acc_percent = (neg_acc / total_acc * 100) if total_acc > 0 else 0
            
            total_vp = pos_vp + neg_vp
            pos_vp_percent = (pos_vp / total_vp * 100) if total_vp > 0 else 0
            neg_vp_percent = (neg_vp / total_vp * 100) if total_vp > 0 else 0
            
            total_vol = cum_vol_up + cum_vol_down
            vol_up_pct = (cum_vol_up / total_vol * 100) if total_vol > 0 else 0
            vol_down_pct = (cum_vol_down / total_vol * 100) if total_vol > 0 else 0
            
            total_time = pos_time + neg_time
            time_pct = (pos_time / total_time * 100) if total_time > 0 else 50
            
            # Unified NET POWER
            total_pos = pos_x_percent + pos_mom_percent + pos_acc_percent + pos_vp_percent + vol_up_pct + time_pct
            total_neg = neg_x_percent + neg_mom_percent + neg_acc_percent + neg_vp_percent + vol_down_pct + (100 - time_pct)
            
            grand_total = total_pos + total_neg
            bullish_pct = (total_pos / grand_total * 100) if grand_total > 0 else 50
            
            # NET POWER = Bullish% - Bearish%
            net_power = bullish_pct - (100 - bullish_pct)
            
            return round(net_power, 2)
            
        except Exception as e:
            logger.error(f"VPMV NET POWER hesaplama hatası: {e}")
            return 0.0
    ...
    def analyze_single_symbol(symbol: str, timeframe: str = '4h') -> Optional[Dict[str, Any]]:
        """
        Tek sembol için TAM analiz (Supertrend + C-Signal + VPMV)
        ...
        Returns:
            Optional[Dict[str, Any]]: Analiz sonuçları (VPMV dahil)
        """
        try:
            ...
            # 🆕 VPMV NET POWER hesapla
            reset_signals = AnalysisService.detect_supertrend_reset(trend)
            vpmv_net_power = AnalysisService.calculate_vpmv_net_power(df, trend, reset_signals)
    ...
    def analyze_multiple_symbols(symbols: List[str], timeframe: str = '4h', max_workers: int = 5) -> List[Dict[str, Any]]:
        """
        Çoklu sembol analizi - Paralel işleme (VPMV dahil)
        ...
        Returns:
            List[Dict[str, Any]]: Analiz sonuçları listesi (VPMV dahil)
        """
        try:
            logger.info(f"{len(symbols)} sembol için {timeframe} TAM analizi (Supertrend+C-Signal+VPMV) başlatılıyor...")
            ...
            logger.info(f"✅ {len(results)} sembol TAM analizi edildi (VPMV dahil)")
            return results
    ...
    def get_analysis_summary(results: List[Dict[str, Any]], timeframe: str) -> Dict[str, Any]:
        """
        Analiz özetini hazırla (VPMV dahil)
    ...
    def update_symbol_with_analysis(symbol_data: Dict[str, Any], timeframe: str, preserve_manual_type: bool = False) -> Dict[str, Any]:
        """
        Manuel tür korunarak sembol verisini güncelle (VPMV dahil)
        ...
        Returns:
            Dict[str, Any]: Güncellenmiş sembol verileri (VPMV dahil)
        """
        try:
            ...
            if df is not None and len(df) >= 50:
                ...
                if preserve_manual_type:
                    # Sadece VPMV güncelle, Supertrend verilerini koruma
                    ...
                    symbol_data['vpmv_net_power'] = vpmv_net_power
                    symbol_data['vpmv_signal'] = vpmv_signal
                    
                    logger.debug(f"🔒 {symbol} manuel tür koruması: {symbol_data.get('supertrend_type', 'Unknown')} - VPMV güncellendi: {vpmv_net_power}")
                else:
                    # Normal güncelleme - TAM analiz (VPMV dahil)
                    ...
                    if full_analysis:
                        ...
                        logger.debug(f"🔄 {symbol} TAM güncelleme: {full_analysis.get('trend_direction')} - Ratio: {full_analysis.get('ratio_percent')}% - VPMV: {full_analysis.get('vpmv_net_power')}")
                
                logger.debug(f"Analiz güncellendi: {symbol} = C-Signal: {c_signal} | VPMV: {symbol_data.get('vpmv_net_power', 0)} (Preserve: {preserve_manual_type})")
            else:
                # Veri yetersiz
