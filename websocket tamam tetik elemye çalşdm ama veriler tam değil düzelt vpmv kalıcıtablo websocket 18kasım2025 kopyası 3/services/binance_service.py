...
🔥🔥 FIX: None-safe logging - Format string crash önleme
🔥🔥 FIX: Ping/pong timeout artırıldı (20s → 30s, 10s → 20s)
🔥🔥 FIX: Otomatik reconnect mekanizması eklendi
"""
...
class BinanceService:
    ...
    def start_websocket(cls, symbols: List[str], interval: str = "1m", on_message_callback=None):
        ...
        🔥🔥 FIX: None-safe logging - Format string crash önleme
        🔥🔥 FIX: Ping/pong timeout artırıldı
        🔥🔥 FIX: Otomatik reconnect mekanizması
        ...
        def on_close(ws, close_status_code, close_msg):
            """
            🔥🔥 FIX: None-safe close logging
            """
            try:
                # 🔥🔥 KRİTİK: None-safe formatting
                status_code = close_status_code if close_status_code is not None else "Unknown"
                message = str(close_msg) if close_msg is not None else "No message"
                ...
                logger.warning(f"║ Code: {status_code_str:<69}║")
                logger.warning(f"║ Message: {message_str:<66}║")
                logger.warning(f"╚{'=' * 78}╝")
                
                stop_flag.set()
            except Exception as log_err:
                logger.error(f"❌ Close logging failed: {log_err}")
                stop_flag.set()
        ...
        def on_ping(ws, message):
            ...
            try:
                ...
                logger.error(f"❌ Ping logging failed: {log_err}")
        ...
        def on_pong(ws, message):
            ...
            try:
                ...
                logger.error(f"❌ Pong logging failed: {log_err}")
        ...
        def on_open(ws):
            ...
            try:
                ...
                interval_str = str(interval) if interval is not None else "unknown"
                url_str = str(ws_url)[:60] if ws_url is not None else "unknown"
                
                logger.info(f"╔{'=' * 78}╗")
                logger.info(f"║ 🟢 WEBSOCKET BAĞLANTISI AÇILDI!{' ' * 41}║")
                logger.info(f"║ Sembol Sayısı: {symbol_count:<59}║")
                logger.info(f"║ Interval     : {interval_str:<59}║")
                logger.info(f"║ URL          : {url_str:<59}║")
                logger.info(f"╚{'=' * 78}╝")
            except Exception as log_err:
                logger.error(f"❌ Open logging failed: {log_err}")

        def run_ws():
            """WebSocket ana çalıştırma fonksiyonu"""
            nonlocal ws_app
            ...
            try:
                ...
                # 🔥🔥 FIX: Ping/pong timeout artırıldı + reconnect eklendi
                ws_app.run_forever(
                    ping_interval=30,  # 🔥 20s → 30s (timeout riskini azaltır)
                    ping_timeout=20,   # 🔥 10s → 20s (daha toleranslı)
                    reconnect=5        # 🔥 Otomatik reconnect (5 saniye ara ile)
                )
