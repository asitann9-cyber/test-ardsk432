...
class AnalysisService:
    ...
    def update_symbol_with_analysis(symbol_data: Dict[str, Any], timeframe: str, preserve_manual_type: bool = False) -> Dict[str, Any]:
        """
        Manuel tür korunarak sembol verisini güncelle (VPMV + C-Signal + Tetikleyici dahil)
        """
        try:
            ...
            if df is not None and len(df) >= 50:
                ...
                if preserve_manual_type:
                    ...
                    logger.info(f"🔒 {symbol} MANUEL TÜR KORUNDU - VPMV + C-Signal + Tetikleyici güncellendi")
                    
                else:
                    # ✅ TAM ANALİZ (Supertrend + VPMV + C-Signal + Tetikleyici)
