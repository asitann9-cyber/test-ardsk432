...
class AnalysisService:
    ...
    def calculate_mtf_time_signals(symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for period_minutes, label in AnalysisService.TIME_PERIOD_LABELS.items():
                try:
                    # Binance'den o periyotun verisini çek
                    df = BinanceService.fetch_klines_data(symbol, f"{int(period_minutes)}m", limit=2)
                    ...
                    logger.debug(f"TIME signal hesaplama hatası ({symbol} - {label}): {e}")
                    time_signals[label] = {
                        'direction': 0,
                        'signal': '⚪ Nötr',
                        'close': None,
    ...
    def analyze_single_symbol(symbol: str, timeframe: str = '4h') -> Optional[Dict[str, Any]]:
        """
        Tek sembol için TAM analiz (Supertrend + C-Signal + VPMV + TETİKLEYİCİ + TIME)
        🆕 Pine Script uyumlu 4 bileşenli VPMV sistemi + Tetikleyici + Multi-Timeframe TIME
        ...
        Returns:
            Optional[Dict[str, Any]]: Analiz sonuçları (VPMV 4 bileşen + Tetikleyici + TIME dahil)
        """
        try:
            ...
            # VPMV yönünü belirle (TIME match count için)
    ...
    def analyze_multiple_symbols(symbols: List[str], timeframe: str = '4h', max_workers: int = 5) -> List[Dict[str, Any]]:
        """
        Çoklu sembol analizi - Paralel işleme (VPMV + Tetikleyici + TIME dahil)
        ...
        Returns:
            List[Dict[str, Any]]: Analiz sonuçları listesi (VPMV + Tetikleyici + TIME dahil)
        """
        try:
            logger.info(f"{len(symbols)} sembol için {timeframe} TAM analizi (Supertrend+C-Signal+VPMV+Tetikleyici+TIME) başlatılıyor...")
            ...
            logger.info(f"✅ {len(results)} sembol TAM analizi edildi (VPMV + Tetikleyici + TIME dahil)")
            return results
        ...
        try:
            ...
            if df is not None and len(df) >= 50:
                ...
                if preserve_manual_type:
                    ...
                    # ✅ TAM ANALİZ (Supertrend + VPMV + C-Signal + Tetikleyici + TIME)
