...
🔥🔥 FIX: WebSocket Timestamp Serialization - JSON crash tamamen önlendi!
🔥🔥 FIX: None-safe logging - Format string hataları önlendi!
🔥🔥🔥 FINAL FIX: numpy.bool_ serialization - JSON crash %100 önlendi!
"""
...
def convert_timestamps_to_string(data):
    """
    🔥🔥🔥 FINAL FIX: Timestamp, datetime, numpy.bool_, numpy.ndarray → JSON serialization
    ...
    Args:
        data: Dict, list veya herhangi bir veri yapısı
        
    Returns:
        JSON-serializable veri
    """
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, (np.bool_, np.generic)):
                result[key] = bool(value)  # numpy.bool_ → Python bool
            elif isinstance(value, np.ndarray):
                # 🔥 numpy array kontrolü
                if value.size == 0:
                    result[key] = None
                else:
                    result[key] = value.tolist()  # array → list
            elif isinstance(value, pd.Timestamp):
                result[key] = value.strftime('%Y-%m-%d %H:%M:%S')
            elif isinstance(value, datetime):
                result[key] = value.strftime('%Y-%m-%d %H:%M:%S')
            elif isinstance(value, (dict, list)):
                result[key] = convert_timestamps_to_string(value)
            else:
                # 🔥 pd.isna kontrolü EN SONDA (numpy.bool_ ile çakışmayı önle)
                try:
                    ...
                    # pd.isna bazı tiplerde hata verebilir, o zaman olduğu gibi bırak
                    result[key] = value
        return result
    elif isinstance(data, list):
        return [convert_timestamps_to_string(item) for item in data]
    else:
        return data
...
def format_symbols_for_frontend(symbols_list):
    ...
    🔥🔥🔥 FINAL FIX: numpy.bool_, numpy.ndarray, Timestamp, datetime → JSON serialization
    ...
    for sym in symbols_list:
        try:
            ...
            # 🔥🔥🔥 KRİTİK: TÜM ÖZEL TİPLERİ STANDART TİPLERE ÇEVİR
            for key, value in list(formatted_sym.items()):
                # 1. numpy.bool_ kontrolü ÖNCE
                if isinstance(value, (np.bool_, np.generic)):
                    formatted_sym[key] = bool(value)
                # 2. numpy.ndarray kontrolü
                elif isinstance(value, np.ndarray):
                    if value.size == 0:
                        formatted_sym[key] = None
                    else:
                        formatted_sym[key] = value.tolist()
                # 3. Timestamp kontrolü
                elif isinstance(value, pd.Timestamp):
                    formatted_sym[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                # 4. datetime kontrolü
                elif isinstance(value, datetime):
                    formatted_sym[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                # 5. NaN/NaT kontrolü (EN SONDA - numpy.bool_ ile çakışmasın)
                else:
                    try:
                        # 🔥🔥 FIX: numpy array'leri pd.isna()'dan önce tekrar kontrol et
                        # (DeprecationWarning: "truth value of empty array is ambiguous" fix)
                        if isinstance(value, np.ndarray):
                            # Bu satıra hiç gelmemeli ama yine de koruma
                            if value.size == 0:
                                formatted_sym[key] = None
                            else:
                                formatted_sym[key] = value.tolist()
                        ...
                        # pd.isna bazı tiplerde hata verir, o zaman olduğu gibi bırak
                        pass
...
def handle_kline_closed(symbol, close_price, is_kline_closed):
    ...
    🔥🔥🔥 FINAL FIX: numpy.bool_ serialization - JSON crash %100 önlendi!
    🔥🔥 FIX: Timestamp serialization - JSON crash önleme
    🔥🔥 FIX: None-safe logging - Format string hataları önleme
    🔥🔥 FIX: DeprecationWarning - numpy array pd.isna() sorunu çözüldü
    ...
    # 🔥🔥 HER CALLBACK'TE LOG (None-safe + Safe formatting)
    try:
        logger.info(f"╔{'=' * 78}╗")
        logger.info(f"║ 🔥 CALLBACK: {str(safe_symbol):<64}║")
        logger.info(f"║   Price: {str(safe_price):<65}║")
    ...
    try:
        ...
        if should_analyze:
            ...
            # 🔥🔥🔥 Broadcast 1: TAM ANALİZ SONRASI (FINAL FIX: numpy.bool_ + Timestamp serialization)
            try:
                # Tüm permanent symbols'ları al
                all_permanent = memory_storage.get_permanent_symbols()
                
                # 🔥🔥🔥 KRİTİK: numpy.bool_, numpy.ndarray, Timestamp → JSON-safe
                safe_permanent = []
                for perm_sym in all_permanent:
                    safe_sym = perm_sym.copy()
                    
                    # Tüm özel tipleri çevir
                    for key, value in list(safe_sym.items()):
                        # 1. numpy.bool_ kontrolü ÖNCE
                        if isinstance(value, (np.bool_, np.generic)):
                            safe_sym[key] = bool(value)
                        # 2. numpy.ndarray kontrolü
                        elif isinstance(value, np.ndarray):
                            if value.size == 0:
                                safe_sym[key] = None
                            else:
                                safe_sym[key] = value.tolist()
                        # 3. Timestamp kontrolü
                        elif isinstance(value, pd.Timestamp):
                            safe_sym[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                        # 4. datetime kontrolü
                        elif isinstance(value, datetime):
                            safe_sym[key] = value.strftime('%Y-%m-%d %H:%M:%S')
                        # 5. NaN/NaT kontrolü (EN SONDA)
                        else:
                            try:
                                # 🔥🔥 FIX: numpy array'leri pd.isna()'dan önce tekrar kontrol et
                                # (DeprecationWarning: "truth value of empty array is ambiguous" fix)
                    ...
                    safe_permanent.append(safe_sym)
                
                # Frontend formatına çevir
                formatted_data = format_symbols_for_frontend(safe_permanent)
            ...
            try:
                ratio = updated.get('ratio_percent', 0)
                vpmv_net = updated.get('vpmv_net_power', 0)
                vpmv_signal = updated.get('vpmv_signal', 'N/A')
                c_signal = updated.get('c_signal', 'N/A')
                trigger_name = updated.get('vpmv_trigger_name', 'Yok')
                trigger_active = updated.get('vpmv_trigger_active', False)
                time_match = updated.get('time_match_count', 0)
                
                # None-safe string conversion
                ratio_str = str(ratio) if ratio is not None else "0"
                vpmv_net_str = str(vpmv_net) if vpmv_net is not None else "0"
                vpmv_signal_str = str(vpmv_signal) if vpmv_signal is not None else "N/A"
                c_signal_str = str(c_signal) if c_signal is not None else "N/A"
                trigger_name_str = str(trigger_name) if trigger_name is not None else "Yok"
                trigger_active_str = str(trigger_active) if trigger_active is not None else "False"
                time_match_str = str(time_match) if time_match is not None else "0"
                
                logger.info(f"╔{'=' * 78}╗")
                logger.info(f"║ ✅ TAM ANALİZ TAMAMLANDI: {str(safe_symbol):<50}║")
                logger.info(f"║   💰 Price: {str(safe_price):<62}║")
                logger.info(f"║   📊 Ratio: {ratio_str:<62}%║")
                logger.info(f"║   🎯 VPMV NET: {vpmv_net_str:<57}║")
                logger.info(f"║   📊 VPMV Signal: {vpmv_signal_str:<55}║")
                logger.info(f"║   💡 C-Signal: {c_signal_str:<58}║")
                logger.info(f"║   🔥 Tetikleyici: {trigger_name_str:<55}║")
                logger.info(f"║   ✅ Aktif: {trigger_active_str:<61}║")
                logger.info(f"║   🕐 TIME Match: {time_match_str}/6{' ' * 54}║")
                logger.info(f"║   ⏰ Zaman: {str(time_str):<63}║")
                logger.info(f"╚{'=' * 78}╝")
            except Exception as log_err:
            ...
            # 🔥🔥🔥 Broadcast 2: SADECE FİYAT GÜNCELLEMESİ (FINAL FIX: numpy.bool_ + Timestamp serialization)
            try:
                # Tüm permanent symbols'ları al
                all_permanent = memory_storage.get_permanent_symbols()
...
def main():
    ...
    try:
        ...
        print("🔥🔥 FIX: WebSocket Timestamp Serialization - JSON crash tamamen önlendi!")
        print("🔥🔥 FIX: None-safe logging - Format string hataları önlendi!")
        print("🔥🔥🔥 FINAL FIX: numpy.bool_ serialization - JSON crash %100 önlendi!")
        print("="*70 + "\n")
