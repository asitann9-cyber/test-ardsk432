...
🚀 PERFORMANCE FIX: None-safe istatistik hesaplamaları
"""
...
class MemoryStorage:
    ...
    def get_time_statistics(self) -> Dict[str, Any]:
        """TIME sistemi istatistiklerini getir (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            ...
            # 🔥 FIX: None-safe hesaplama
            match_counts = [s.get('time_match_count', 0) or 0 for s in self.permanent_high_ratio]
    ...
    def get_symbols_by_time_match(self, min_match: int = 4) -> List[Dict[str, Any]]:
        """Belirli TIME eşleşme sayısına sahip sembolleri getir (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            return [
                {
                    'symbol': s['symbol'],
                    'time_match_count': s.get('time_match_count', 0) or 0,  # 🔥 FIX: None-safe
                ...
                if (s.get('time_match_count', 0) or 0) >= min_match  # 🔥 FIX: None-safe
            ]
    ...
    def get_vpmv_statistics(self) -> Dict[str, Any]:
        """VPMV istatistiklerini getir (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            if not self.permanent_high_ratio:
                return {
                    'total_symbols': 0,
            ...
            # 🔥 FIX: None-safe VPMV değerleri
            vpmv_values = [s.get('vpmv_net_power', 0) or 0 for s in self.permanent_high_ratio]
    ...
    def get_top_vpmv_symbols(self, limit: int = 10, sort_by: str = 'highest') -> List[Dict[str, Any]]:
        """En yüksek/düşük VPMV değerine sahip sembolleri getir (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            sorted_symbols = sorted(
                self.permanent_high_ratio,
                key=lambda x: x.get('vpmv_net_power', 0) or 0,  # 🔥 FIX: None-safe
    ...
    def get_active_triggers(self) -> List[Dict[str, Any]]:
        """Aktif tetikleyicileri olan sembolleri getir (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            return [
                {
                    'symbol': s['symbol'],
                    'trigger_name': s.get('vpmv_trigger_name', 'Yok'),
                    'vpmv_net_power': s.get('vpmv_net_power', 0) or 0,  # 🔥 FIX: None-safe
                    'vpmv_signal': s.get('vpmv_signal', 'NEUTRAL'),
                    'tradingview_link': s.get('tradingview_link', '#')
                }
                for s in self.permanent_high_ratio 
    ...
    def get_supertrend_statistics(self) -> Dict[str, Any]:
        """Supertrend + VPMV + Tetikleyici + TIME sistemi istatistikleri (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            if not self.permanent_high_ratio:
                return {
                    'total_symbols': 0,
            ...
            # 🔥 FIX: None-safe ratio hesaplamaları
            ratios = [s.get('abs_ratio_percent', 0) or 0 for s in self.permanent_high_ratio]
            avg_ratio = sum(ratios) / len(ratios) if ratios else 0
            max_ratio = max(ratios) if ratios else 0
            
            high_z_score_count = sum(1 for s in self.permanent_high_ratio 
                                   if abs(s.get('z_score', 0) or 0) >= 2.0)  # 🔥 FIX: None-safe
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        """Veri bütünlüğünü kontrol et (Thread-safe) - 🔥 None-safe"""
        with self._permanent_lock:
            with self._selected_symbols_lock:
                issues = []
                ...
                for symbol_data in self.permanent_high_ratio:
                    ...
                    # 🔥 FIX: None-safe validasyon
                    if (symbol_data.get('abs_ratio_percent', 0) or 0) < 0:
                    ...
                    # 🔥 FIX: None-safe VPMV validasyonu
                    vpmv_value = symbol_data.get('vpmv_net_power', 0) or 0
                    ...
                    # 🔥 FIX: None-safe TIME validasyonu
                    time_match = symbol_data.get('time_match_count', 0) or 0
            ...
            with self._permanent_lock:
                with self._stats_lock:
                    try:
                        ...
                        logger.info("✅ Veri başarıyla import edildi (VPMV + Tetikleyici + TIME + Thread-safe + None-safe)")
                        return True
