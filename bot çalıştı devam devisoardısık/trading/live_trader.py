...
def get_current_live_positions() -> Dict:
    """🔥 DÜZELTME: Mevcut live pozisyonları Binance'den al - AI skorunu config'den koru"""
    try:
        ...
        for pos in binance_positions:
            ...
            if abs(position_amt) > 0:
                ...
                # 🔥 DÜZELTME: Config'deki mevcut pozisyon verisinden AI skorunu koru
                existing_position = config.live_positions.get(symbol)
                if existing_position and 'signal_data' in existing_position:
                    # Mevcut pozisyondan gerçek signal verisini al
                    signal_data = existing_position['signal_data']
                else:
                    # Mevcut pozisyon yoksa varsayılan değerler
                    signal_data = {
                        'ai_score': 50,  # Varsayılan
                        'run_type': side.lower(),
                        'run_count': 3,
                        'run_perc': 2.0,
                        'gauss_run': 6.0,
                ...
                position_data = {
                    ...
                    'signal_data': signal_data  # 🔥 GERÇEK AI SKORU KORUNDU
                }
...
class LiveTradingBot:
    ...
    def close_position(self, symbol: str, close_reason: str) -> bool:
        """🔒 DÜZELTME: Pozisyon kapat + Beklemeli kontrol eklendi"""
        try:
            ...
            # 🔥 DÜZELTME: Beklemeli kontrol eklendi (open_position gibi)
            ...
            if order_status == "FILLED" and executed_qty > 0:
                ...
                # CSV'ye CLOSED status ile kaydet
    ...
    def _handle_auto_close(self, symbol: str, close_reason: str, filled_order: Dict) -> None:
        ...
        try:
            ...
            # Trade kaydı - CSV'ye CLOSED status ile
            trade_data = position.copy()
            trade_data.update({
                "exit_price": exit_price,
                "current_value": position["quantity"] * exit_price,
    ...
    def fill_empty_positions(self) -> None:
        ...
        try:
            ...
            # UI'deki hazır veriyi doğrudan kullan
            ...
            # UI'deki filtrelenmiş ve sıralanmış veriyi doğrudan kullan
            df = config.current_data.copy()
        
            # Sadece açık pozisyonları hariç tut
            ...
            # UI'deki sıralama korunuyor - ilk N sinyali alınıyor
            logger.info("📊 UI'deki sıralama korunuyor - ilk N sinyali alınıyor")
        
            # İlk N sinyali al (UI'deki sıralama ile)
            ...
            for i, (_, signal) in enumerate(top_signals.iterrows(), 1):
                logger.info(f"   🥇 #{i}: {signal['symbol']} | AI={signal['ai_score']:.0f}% | Run={signal['run_count']}×{signal['run_perc']:.2f}%")

            # Tradable kontrolü ve pozisyon açma
            opened = 0
            ...
            # Sonuç özeti
            if opened > 0:
                logger.info(f"🎊 BAŞARILI: {opened} yeni pozisyon açıldı (UI'nin ilk {len(top_signals)} sinyalinden)")
                logger.info(f"📊 Yeni durum: {len(current_positions) + opened}/{MAX_OPEN_POSITIONS} pozisyon")
            else:
                logger.warning("😔 Hiçbir pozisyon açılamadı - tradable sembol kontrolü gerekebilir")
