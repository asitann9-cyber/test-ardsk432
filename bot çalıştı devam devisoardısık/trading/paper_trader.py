...
def fill_empty_positions():
    ...
    try:
        ...
        # 🔥 Kullanıcının belirlediği filtreleri uygula
        ...
        trading_min_ai = max(min_ai_pct, 15.0)  # Minimum güvenlik eşiği
        ...
        if df.empty:
            logger.warning("⚠️ Filtre sonrası uygun sinyal yok")
            return
        ...
        # ✅ REFERANS SIRALAMA: Analyzer ile aynı
        df = df.sort_values(
        ...
        logger.debug("📊 PaperTrader sıralama sonrası ilk 5 sinyal:")
        for i, row in df.head(5).iterrows():
            logger.debug(f"   {i}: {row['symbol']} | AI={row['ai_score']} | Trend={row.get('trend_strength',0)}")


        logger.info(f"🎯 GERÇEK EN İYİ {top_n} SİNYAL → seçilen:")
