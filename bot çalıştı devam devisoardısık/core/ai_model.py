...
class CryptoMLModel:
    ...
    def _calculate_balanced_score(self, metrics: Dict) -> float:
        """🆕 GÜNCELLENMIŞ: RSI momentum + log volume + çift yön uyumu skorlama (Toplam 100 puan)"""
        score = 0.0
        
        # 1. Deviso Ratio (25 puan) - Azaltıldı çünkü çift yön kontrolü ayrı kategori oldu
        deviso_ratio = abs(metrics.get('deviso_ratio', 0.0))
        
        if deviso_ratio >= 10.0:
            deviso_score = 25
        elif deviso_ratio >= 5.0:
            deviso_score = 18 + (deviso_ratio - 5.0) / 5.0 * 7  # 18-25 arası
        elif deviso_ratio >= 2.0:
            deviso_score = 12 + (deviso_ratio - 2.0) / 3.0 * 6  # 12-18 arası
        elif deviso_ratio >= 0.5:
            deviso_score = 4 + (deviso_ratio - 0.5) / 1.5 * 8   # 4-12 arası
        else:
            deviso_score = deviso_ratio / 0.5 * 4                # 0-4 arası
            
        score += deviso_score
        
        # 2. RSI Momentum (25 puan) - YENİ ANA KRİTER (aynı kaldı)
        ...
        # 3. Logaritmik Hacim (20 puan) - YENİ ÖNEMLI KRİTER (aynı kaldı)
        log_vol_strength = metrics.get('log_volume_strength', 0.0)
        ...
        # 4. Ardışık Metrikler (15 puan) - Azaltıldı (aynı kaldı)
        ...
        # 🆕 5. ÇİFT YÖN UYUMU (15 puan) - YENİ: RSI momentum + Deviso çift kontrolü
        run_type = metrics.get('run_type', 'none')
        original_deviso = metrics.get('deviso_ratio', 0.0)
        
        # Mükemmel uyum: Her iki gösterge de doğru yönde
        if ((run_type == 'long' and rsi_momentum > 0 and original_deviso > 0) or 
            (run_type == 'short' and rsi_momentum < 0 and original_deviso < 0)):
            trend_score = 15  # Mükemmel çift yön uyumu - maximum puan
            logger.debug(f"✅ Çift yön uyumu: {run_type.upper()} - RSI:{rsi_momentum:.2f}, Deviso:{original_deviso:.2f}")
        
        # RSI uyumlu ama deviso değil
        elif ((run_type == 'long' and rsi_momentum > 0) or 
              (run_type == 'short' and rsi_momentum < 0)):
            trend_score = 10  # RSI yönü doğru, deviso uyumsuz
            logger.debug(f"⚠️ Kısmi uyum (RSI): {run_type.upper()} - RSI:{rsi_momentum:.2f} uyumlu, Deviso:{original_deviso:.2f} uyumsuz")
        
        # Deviso uyumlu ama RSI değil  
        elif ((run_type == 'long' and original_deviso > 0) or 
              (run_type == 'short' and original_deviso < 0)):
            trend_score = 8   # Deviso yönü doğru, RSI uyumsuz
            logger.debug(f"⚠️ Kısmi uyum (Deviso): {run_type.upper()} - Deviso:{original_deviso:.2f} uyumlu, RSI:{rsi_momentum:.2f} uyumsuz")
        
        # Hiçbiri uyumlu değil
        else:
            trend_score = 2   # Çifte uyumsuzluk - düşük puan
            logger.debug(f"❌ Çifte uyumsuzluk: {run_type.upper()} - RSI:{rsi_momentum:.2f}, Deviso:{original_deviso:.2f}")
            
        score += trend_score
        
        # 6. Kalite Cezaları (maksimum -10 puan) - aynı kaldı
        penalty = 0
