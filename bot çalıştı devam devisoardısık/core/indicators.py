...
def compute_consecutive_metrics(df: pd.DataFrame) -> Dict:
    """
    🔄 HİBRİT: Eski deviso + Yeni RSI momentum + log volume sistemi
    ...
    try:
        ...
        logger.debug(f"RSI momentum hesaplama hatası: {e}")
        rsi_momentum = 0.0
        momentum_eval = {'strength': 'very_weak', 'direction': 'neutral', 'score': 20, 'abs_change': 0.0}
    
    # 🆕 LOGARİTMİK HACİM HESAPLAMA  
    try:
        log_volume_metrics = calculate_log_volume_metrics(df)
    except Exception as e:
        logger.debug(f"Log volume hesaplama hatası: {e}")
        log_volume_metrics = {
            'log_volume_ratio': 0.0,
            'log_volume_change': 0.0,
            'log_volume_trend': 0.0,
    ...
    # 🔄 ESKİ DEVISO RATIO HESAPLAMA (senin kodundaki gibi)
    try:
    ...
    return {
        ...
        # 🆕 YENİ LOGARİTMİK HACİM METRİKLERİ
        ...
        # Deviso (senin kodundaki tam hesaplama ile)
        'deviso_ratio': float(deviso_ratio)
        
        # ❌ ESKİ VOLUME SİSTEMİ KALDIRILDI: 'vol_ratio', 'hh_vol_streak'
    }
