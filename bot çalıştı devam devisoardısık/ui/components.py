...
🆕 RSI MOMENTUM + LOGARİTMİK HACİM SÜTUNLARI EKLENDİ
❌ ESKİ VOLUME SÜTUNLARI KALDIRILDI
"""
...
def create_control_panel():
    """🆕 GÜNCELLENMIŞ: RSI momentum filtreleri eklendi"""
    return html.Div([
        html.Div([
        ...
        # 🆕 RSI momentum filtresi (eski vol ratio yerine)
        html.Div([
            html.Label("Min RSI Δ"),
            dcc.Input(
                id='inp-min-rsi-momentum',
                type='number',
                value=2.0,  # Varsayılan minimum RSI momentum
                step=0.5,
                min=0,
                style={'width': '80px', 'backgroundColor': '#2a2a2a', 'border': '1px solid #404040', 'color': '#ffffff', 'padding': '8px'}
            )
...
def create_status_section():
    """Durum gösterge bölümünü oluştur"""
    return html.Div([
        html.Div([
...
def create_signals_table():
    """🆕 GÜNCELLENMIŞ: RSI momentum + log volume sütunları eklendi"""
    return html.Div([
        html.H3("📊 TÜM SİNYALLER Paneli - RSI Momentum + Log Volume", style={
            'textAlign': 'center',
            'margin': '0 0 1rem 0',
            'fontSize': '1.3rem',
            'fontWeight': '600'
        ...
        dash_table.DataTable(
            ...
            columns=[
                ...
                # 🆕 YENİ SÜTUNLAR
                {"name": "RSI Δ", "id": "rsi_momentum", 'type': 'numeric', 'format': {"specifier": ".2f"}},
                {"name": "Log Vol", "id": "log_volume_strength", 'type': 'numeric', 'format': {"specifier": ".2f"}},
                {"name": "Mom Score", "id": "momentum_score", 'type': 'numeric'},
                
                # ❌ KALDIRILDI: Vol/SMA, HH Vol
            ...
            style_data_conditional=[
                ...
                # 🆕 RSI MOMENTUM renklendirme
                {
                    'if': {'column_id': 'rsi_momentum', 'filter_query': '{rsi_momentum} > 0'},
                    'backgroundColor': '#059669',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                ...
                # 🆕 LOG VOLUME renklendirme
                {
                    'if': {'column_id': 'log_volume_strength', 'filter_query': '{log_volume_strength} >= 3'},
                    'backgroundColor': '#3b82f6',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'log_volume_strength', 'filter_query': '{log_volume_strength} >= 1.5'},
                    'backgroundColor': '#6366f1',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                
                # 🆕 MOMENTUM SCORE renklendirme
                {
                    'if': {'column_id': 'momentum_score', 'filter_query': '{momentum_score} >= 80'},
                    'backgroundColor': '#10b981',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                {
                    'if': {'column_id': 'momentum_score', 'filter_query': '{momentum_score} >= 60'},
                    'backgroundColor': '#059669',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                
                # Deviso ratio renklendirme (aynı)
                {
                ...
                # Live bot status renklendirme (aynı)
                {
