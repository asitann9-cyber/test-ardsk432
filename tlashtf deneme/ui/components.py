...
📊 Basitleştirilmiş layout: Symbol, ULTRA Signal, Power, Bars Ago, AI Score
"""
...
def create_header():
    ...
    return html.Div([
        html.H1("🔥 ULTRA PANEL v5 - Pine Script Etiket Sistemi", style={
            'textAlign': 'center',
            'margin': '0 0 0.5rem 0',
            'fontSize': '2rem',
            'fontWeight': '700'
        }),
        html.P("📊 Sadece ULTRA 3/4 ve 4/4 sinyalleri | Bars Ago tracking aktif", style={
            'textAlign': 'center',
            'margin': '0 0 1rem 0',
            'color': '#06b6d4',
            'fontWeight': '500',
...
def create_control_panel():
    """🔥 BASİTLEŞTİRİLMİŞ Kontrol paneli - Sadece 3 filtre"""
    return html.Div([
        html.Div([
            html.Label("⏰ Timeframe", style={'fontWeight': '600', 'marginBottom': '5px'}),
            dcc.Dropdown(
                id='dd-timeframe',
                options=[
        ...
        html.Div([
            html.Label("🎯 Min AI Score %", style={'fontWeight': '600', 'marginBottom': '5px'}),
            dcc.Input(
                id='inp-min-ai',
                type='number',
        ...
        html.Div([
            html.Label("📉 Max Bars Ago", style={'fontWeight': '600', 'marginBottom': '5px'}),
            dcc.Input(
                id='inp-max-bars-ago',
                type='number',
                value=15,
            ...
            html.Small("(0 = sadece yeni)", style={'color': '#9ca3af', 'marginTop': '3px', 'display': 'block'})
...
def create_signals_table():
    """🔥 ULTRA ETİKET SİSTEMİ - BASİTLEŞTİRİLMİŞ TABLO (Pine Script mantığı)"""
    return html.Div([
        html.H3("🔥 ULTRA SİNYAL TARAYICI", style={
            'textAlign': 'center',
            'margin': '0 0 1rem 0',
            'fontSize': '1.5rem',
            'fontWeight': '700',
        ...
        html.P("📊 Pine Script 'ULTRA 3/4' ve 'ULTRA 4/4' etiketleri | Bars Ago tracking", style={
            'textAlign': 'center',
            'margin': '0 0 1rem 0',
            'color': '#9ca3af',
            'fontSize': '0.85rem'
        ...
        dash_table.DataTable(
            ...
            columns=[
                {"name": "💰 Coin", "id": "symbol"},
                {"name": "🔥 ULTRA Signal", "id": "ultra_signal"},  # 🔥 "🔥 BUY 3/4" formatı
                {"name": "⚡ Power", "id": "power"},  # 🔥 "5.2" formatı
                {"name": "📉 Bars Ago", "id": "bars_ago"},  # 🔥 YENİ SÜTUN "🆕 Şimdi" veya "📉 3 mum"
                {"name": "🎯 AI Score", "id": "ai_score"},
                {"name": "🤖", "id": "live_status"},
                {"name": "⏰ Time", "id": "timestamp"}
            ],
            data=[],
            page_size=30,  # Daha fazla satır göster
            sort_action='native',
            ...
            style_data_conditional=[
                # 🔥 ULTRA SIGNAL renklendirme (BUY içeren)
                {
                    'if': {'column_id': 'ultra_signal', 'filter_query': '{ultra_signal} contains BUY'},
                    'backgroundColor': '#059669',
                    'color': '#ffffff',
                    'fontWeight': 'bold',
                    'fontSize': '0.95rem'
                },
                # 🔥 ULTRA SIGNAL renklendirme (SELL içeren)
                {
                    'if': {'column_id': 'ultra_signal', 'filter_query': '{ultra_signal} contains SELL'},
                    'backgroundColor': '#dc2626',
                    'color': '#ffffff',
                    'fontWeight': 'bold',
                    'fontSize': '0.95rem'
                ...
                # 🔥 4/4 HTF extra vurgu
                {
                    'if': {'column_id': 'ultra_signal', 'filter_query': '{ultra_signal} contains 4/4'},
                ...
                # 🔥 POWER renklendirme (>10)
                {
                    'if': {'column_id': 'power', 'filter_query': '{power} > 10'},
                    'backgroundColor': '#f59e0b',
                    'color': '#ffffff',
                    'fontWeight': 'bold'
                },
                # 🔥 POWER renklendirme (>5)
                {
                    'if': {'column_id': 'power', 'filter_query': '{power} > 5'},
                    'backgroundColor': '#eab308',
                    'color': '#000000',
                    'fontWeight': 'bold'
                },
                # 🔥 BARS AGO renklendirme (Yeni sinyaller)
                {
                    'if': {'column_id': 'bars_ago', 'filter_query': '{bars_ago} contains Şimdi'},
                    'backgroundColor': '#22c55e',
                    'color': '#000000',
                    'fontWeight': 'bold',
                    'fontSize': '0.9rem'
                },
                # 🔥 BARS AGO renklendirme (1-5 mum)
                {
                    'if': {'column_id': 'bars_ago', 'filter_query': '{bars_ago} contains mum'},
                    'backgroundColor': '#6b7280',
                    'color': '#ffffff'
                },
                # 🔥 Live bot status (Açık pozisyon)
                {
...
def create_layout():
    """🔥 ULTRA ETİKET SİSTEMİ - BASİTLEŞTİRİLMİŞ LAYOUT"""
    return html.Div([
        # Header
        create_header(),
        
        # Kontrol paneli (sadece 3 filtre)
        ...
        # 🔥 ULTRA SİNYAL TARAYICI (basitleştirilmiş tablo)
