...
🔥 ÖZELLIKLER: HTF Count, Power, Whale detection, Memory sistemi
🔥 YENİ: Bars Ago tracking + Pine Script uyumlu crossover detection
"""
...
def analyze_symbol_with_ai(symbol: str, interval: str) -> Dict:
    ...
    Pine Script mantığına uygun:
    - HTF crossover detection (4H, 6H, 8H, 12H)
    - Memory sistemi (sinyal kaybolsa bile hatırlar)
    - Bars ago tracking (kaç mum önce)
    ...
    try:
        ...
        # Veri çek (son 200 bar - HTF hesabı için)
        ...
        # 🔥 ULTRA PANEL v5 METRİKLERİNİ HESAPLA (compute_consecutive_metrics içinde yapılıyor)
        metrics = compute_consecutive_metrics(df, symbol=symbol)
        ...
        # 🔥 BARS AGO (kaç bar önce sinyal oluştu)
        ...
        result = {
            ...
            'bars_ago': bars_ago,  # Kaç bar önce
            
            'ai_score': ai_score,
            'last_update': last_update.strftime('%Y-%m-%d %H:%M:%S %Z'),
            
            # 🔥 YENİ: Memory için timestamp
            'signal_timestamp': datetime.now(LOCAL_TZ),
            'memory_valid': True,
        }
        ...
        # Bars ago uyarısı
        if bars_ago > 0:
            logger.debug(f"⏰ {symbol}: Sinyal {bars_ago} bar önce oluştu")
        
        logger.debug(
            f"✅ {symbol}: Ultra sinyal onaylandı - "
            f"{ultra_signal} | HTF:{htf_count}/4 | Power:{total_power:.1f} | "
            f"AI:{ai_score:.0f}% | Bars ago:{bars_ago}"
        )
        return result
...
def batch_analyze_with_ai(interval: str) -> pd.DataFrame:
    ...
    Memory sistemi ile çalışır:
    - Yeni sinyalleri tespit eder
    - Eski sinyalleri korur (15 dakika)
    - Yaşlanma cezası uygular
    - HTF ve Power bazlı adaptif ceza
            ...
            try:
                ...
                if res:  # Geçerli ultra sinyal
                    ...
                    saved_signals[symbol] = {
                        ...
                        'first_seen': saved_signals.get(symbol, {}).get('first_seen', datetime.now(LOCAL_TZ))
                    }
    ...
    # 🔥 ESKİ SİNYALLERİ KORUMA VE SKOR DÜŞÜRME (Memory sistemi)
    protected_count = 0
    for symbol, saved_info in list(saved_signals.items()):
        ...
        if minutes_old > 15:
            ...
            logger.debug(f"🗑️ {symbol}: 15 dakika aşıldı, memory'den silindi")
            continue
        
        # Yeni taramada bulunamayan eski sinyaller
        if symbol not in fresh_symbols:
            old_data = saved_info['data'].copy()
            original_score = old_data['ai_score']
            
            # 🔥 ULTRA PANEL v5 BAZLI ADAPTIF CEZA SİSTEMİ
            htf_count = old_data.get('htf_count', 0)
            total_power = old_data.get('total_power', 0.0)
            bars_ago_original = old_data.get('bars_ago', 0)
            
            # HTF count ve power'a göre base ceza
            if htf_count == 4 and total_power > 20:
                base_penalty = 5  # 🔥 Çok güçlü sinyal, minimal ceza
            elif htf_count == 4 and total_power > 10:
                base_penalty = 10  # 🔥 Güçlü sinyal
            elif htf_count == 4:
                base_penalty = 15
            elif htf_count == 3 and total_power > 15:
                base_penalty = 20
            elif htf_count == 3 and total_power > 10:
                base_penalty = 25
            else:
                base_penalty = 35  # Zayıf sinyal, ağır ceza
            ...
            if minutes_old <= 3:
                ...
                penalty = base_penalty + 30
            
            # Bars ago arttır (her scan'de +1)
            bars_ago_new = bars_ago_original + 1
            
            new_score = max(5, original_score - penalty)
            old_data['ai_score'] = new_score
            old_data['score_status'] = f"📉-{penalty}"
            old_data['bars_ago'] = bars_ago_new
            old_data['memory_protected'] = True
            ...
            logger.debug(
                ...
                f"Bars ago: {bars_ago_original}→{bars_ago_new})"
            )
    ...
    # 5. Bars Ago (en yeni önce)
    df = df.sort_values(
        by=['ai_score', 'htf_count', 'total_power', 'whale_active', 'bars_ago'],
        ascending=[False, False, False, False, True]  # bars_ago küçük olanı önce
    )
    
    if len(df) > 0:
        top_signal = df.iloc[0]
        whale_emoji = "🐋" if top_signal['whale_active'] else ""
        memory_emoji = "📉" if top_signal.get('memory_protected', False) else "🆕"
        
        logger.info(
            f"🏆 En yüksek AI skoru: {top_signal['ai_score']:.0f}% - {top_signal['symbol']} "
            f"| {top_signal['ultra_signal']} | HTF:{top_signal['htf_count']}/4 | "
            f"Power:{top_signal['total_power']:.1f} | Bars ago:{top_signal.get('bars_ago', 0)} "
            f"{whale_emoji} {memory_emoji}"
        )
        ...
        if protected_count > 0:
            logger.info(f"📉 {protected_count} korunan sinyal skor düşüşü ile aşağı kaydı")
        ...
        for idx, (i, row) in enumerate(df.head(5).iterrows(), 1):
            ...
            memory_emoji = "📉" if row.get('memory_protected', False) else "🆕"
            logger.debug(
                f"   #{idx}: {row['symbol']} | AI={row['ai_score']:.0f}% | "
                f"{row['ultra_signal']} | HTF={row['htf_count']}/4 | "
                f"Power={row['total_power']:.1f} | Bars ago={row.get('bars_ago', 0)} "
                f"{whale_emoji} {memory_emoji}"
            )
...
def filter_signals(df: pd.DataFrame, filters: Dict) -> pd.DataFrame:
    ...
    # 🔥 YENİ: Bars ago filtresi (maksimum kaç bar önce)
    max_bars_ago = filters.get('max_bars_ago', 50)
    if 'bars_ago' in filtered_df.columns:
        filtered_df = filtered_df[filtered_df['bars_ago'] <= max_bars_ago]
...
def analyze_signal_quality(df: pd.DataFrame) -> Dict:
    ...
    if 'total_power' in df.columns:
        power_stats = {
            ...
            'high_power': len(df[df['total_power'] >= 20]),
            'medium_power': len(df[(df['total_power'] >= 10) & (df['total_power'] < 20)]),
            'low_power': len(df[df['total_power'] < 10])
        }
    
    # 🔥 YENİ: Memory protected count
    memory_protected_count = len(df[df.get('memory_protected', False) == True])
    
    # 🔥 YENİ: Bars ago stats
    avg_bars_ago = df['bars_ago'].mean() if 'bars_ago' in df.columns else 0
    max_bars_ago = df['bars_ago'].max() if 'bars_ago' in df.columns else 0
    ...
    return {
        ...
        'memory_protected_count': memory_protected_count,
        'bars_ago_stats': {
            'avg': avg_bars_ago,
            'max': max_bars_ago
        }
...
def update_signal_scores():
    ...
    Background task olarak çalışabilir
    """
    global saved_signals
...
def get_signal_summary() -> Dict:
    ...
    # 🔥 YENİ: Memory protected count
    memory_count = len(current_data[current_data.get('memory_protected', False) == True])
