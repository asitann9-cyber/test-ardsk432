...
🔥 YENİ: VPM (Volume-Price-Momentum) UI entegrasyonu tamamlandı
"""
...
current_settings = {
    ...
    'max_zscore': 5.0  # 🔥 YENİ: Z-Score filtresi
}
...
def update_signals(n_intervals, tf, min_streak, min_pct, min_volr, min_ai_pct, max_zscore):  
    """🔥 YENİ: Z-Score + VPM filtresi dahil sinyal güncelleme"""
    ...
    if 'vpm_score' in df.columns:
        sort_columns.insert(1, 'vpm_score')  # 🔥 VPM AI'dan sonra
    ...
    for _, row in df.iterrows():
        ...
        vpm_display = f"{vpm_value:+.1f}" if not pd.isna(vpm_value) else "0.0"
...
if __name__ == "__main__":
    setup_csv_files()
    logger.info("🚀 AI Crypto Analytics + Live Trading + Z-Score + VPM Sistemi başlatılıyor...")
    ...
    if LIVE_TRADER_AVAILABLE:
        ...
        logger.info("⚠️ Z-Score ceza sistemi aktif - Aşırı şişmiş sinyaller cezalandırılacak")
        logger.info("🎯 VPM (Volume-Price-Momentum) göstergesi aktif - 30 puan ağırlık")
    else:
